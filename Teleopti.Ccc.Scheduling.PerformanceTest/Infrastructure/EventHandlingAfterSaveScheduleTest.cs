...
namespace Teleopti.Ccc.Scheduling.PerformanceTest.Infrastructure
{
	...
	[Category("EventHandlingAfterSaveScheduleTEMP")]
	[Setting("OptimizeScheduleChangedEvents_DontUseFromWeb", true)]
	[Setting("HangfireQueuePollIntervalSeconds", 15)]
	[InfrastructureTest]
	public class REMOVEME
	{
		...
		public void MeasurePerformance()
		{
			...
			WithUnitOfWork.Do(() =>
			{
				var periodPlanningPeriod = PlanningPeriodRepository.Get(AppConfigs.PlanningPeriodId).Range;
				var scenario = Scenarios.LoadDefaultScenario();
				var persons = Persons.LoadAll().Where(p => p.Period(periodPlanningPeriod.StartDate) != null).Take(500).ToArray();
