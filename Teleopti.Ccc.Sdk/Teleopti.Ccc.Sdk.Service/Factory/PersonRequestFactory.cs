    ...
    public class PersonRequestFactory : IPersonRequestFactory
    {
        ...
        public PersonRequestDto CreatePersonRequest(PersonRequestDto personRequestDto)
        {
            ...
            ShiftTradeRequestSetChecksum shiftTradeRequestSetChecksum =
                new ShiftTradeRequestSetChecksum(_scenarioRepository.LoadDefaultScenario(),
                                                 _scheduleRepository);
            ...
            else
            {
                if (_serviceBusSender.EnsureBus())
                {
                    ...
                    _serviceBusSender.NotifyServiceBus(new AcceptShiftTrade
                                                           {
                                                               BusinessUnitId =
                                                                   identity.BusinessUnit.Id.GetValueOrDefault(Guid.Empty),
                                                               Datasource =
                                                                   identity.DataSource.Application.Name,
                                                               Timestamp = DateTime.UtcNow,
                                                               PersonRequestId =
                                                                   personRequestDto.Id.GetValueOrDefault(Guid.Empty),
                                                               AcceptingPersonId =
                                                                   person.Id.GetValueOrDefault(Guid.Empty),
                ...
                else
                {
                    ...
                    using (new MessageBrokerSendEnabler())
                    {
                        IScenario scenario = _scenarioRepository.LoadDefaultScenario();
                        ShiftTradeRequestSetChecksum shiftTradeRequestSetChecksum =
