...
namespace Teleopti.Ccc.Sdk.Common.DataTransferObject.Commands
{
	...
	///
	/// The schedule changes will arrive as POST data to the endpoint you specify through the SDK. The function to get information about current subscriptions and listener endpoints is GetScheduleChangeSubscriptionsByQuery on the SchedulingService. The response on this function also contains the details needed to verify the signature of the incoming data. To add a new listener the command AddScheduleChangesListenerCommandDto should be executed. The url needs to be an absolute url (the complete address with http or https included) to the endpoint. DaysStartFromCurrentDate and DaysEndFromCurrentDate creates a filter on the range of schedule changes to listen for. A value of -1 for DaysStartFromCurrentDate means that changes from yesterday are included and a value of 1 for DaysEndFromCurrentDate means that changes for tomorrow are included. Please note that these dates are local dates in the viewpoint of the agents. DaysStartFromCurrentDate must be smaller than or equal to DaysEndFromCurrentDate. The name must be provided and will be used when the endpoint should be removed using the RevokeScheduleChangesListenerCommandDto command.
	///
