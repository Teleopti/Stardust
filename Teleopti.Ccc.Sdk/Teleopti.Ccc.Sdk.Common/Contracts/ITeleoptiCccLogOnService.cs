...
namespace Teleopti.Ccc.Sdk.Common.Contracts
{
	...
	public interface ITeleoptiCccLogOnService
	{
		...
		/// <param name="dataSource">The data source to logon to.</param>
		/// <returns></returns>
		[OperationContract, Obsolete("Use method LogOnAsApplicationUser instead")]
		AuthenticationResultDto LogOnApplicationUser(string userName, string password, DataSourceDto dataSource);

		/// <summary>
		/// Logs on using forms credentials, the correct database will be detected automaticly
		/// </summary>
		/// <param name="userName">Name of the user.</param>
		/// <param name="password">The password.</param>
		/// <returns></returns>
		[OperationContract]
		AuthenticationResultDto LogOnAsApplicationUser(string userName, string password);
