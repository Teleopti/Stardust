...
namespace Teleopti.Ccc.Sdk.WcfHost.Service
{
											...
											IDisposable
	{
		...
		{
			var dataSource = ((TeleoptiIdentity)TeleoptiPrincipalForLegacy.CurrentPrincipal.Identity).DataSource.DataSourceName;
...
#pragma warning restore 612,618
		...
		public void SaveExtendedPreferenceTemplate(ExtendedPreferenceTemplateDto extendedPreferenceTemplateDto)
		{
			...
			using (IUnitOfWork uow = UnitOfWorkFactory.Current.CreateAndOpenUnitOfWork())
			{
				var person = repositoryFactory.CreatePersonRepository(uow).Get(TeleoptiPrincipalForLegacy.CurrentPrincipal.PersonId);
		...
		public PersonRequestDto AcceptShiftTradeRequest(PersonRequestDto personRequest)
		{
			using (var inner = _lifetimeScope.BeginLifetimeScope())
			{
				...
				using (IUnitOfWork unitOfWork = UnitOfWorkFactory.Current.CreateAndOpenUnitOfWork())
				{
					var person = new PersonRepository(new ThisUnitOfWork(unitOfWork)).Get(TeleoptiPrincipalForLegacy.CurrentPrincipal.PersonId);
		...
		public TeamDto GetLoggedOnPersonTeam(DateTime utcDate)
		{
			using (IUnitOfWork unitOfWork = UnitOfWorkFactory.Current.CreateAndOpenUnitOfWork())
			{
				IPerson person = new PersonRepository(new ThisUnitOfWork(unitOfWork)).Get(TeleoptiPrincipalForLegacy.CurrentPrincipal.PersonId);
		...
		public ICollection<ApplicationFunctionDto> GetApplicationFunctionsForPerson(PersonDto person)
		{
			...
			if (TeleoptiPrincipalForLegacy.CurrentPrincipal.PersonId == person.Id.GetValueOrDefault(Guid.Empty))
			{
