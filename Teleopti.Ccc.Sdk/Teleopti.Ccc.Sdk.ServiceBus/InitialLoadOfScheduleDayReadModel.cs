...
{
	public class InitialLoadOfScheduleDayReadModel
	{
		private readonly Func<IServiceBus> _serviceBusFinder;

		public InitialLoadOfScheduleDayReadModel(Func<IServiceBus> serviceBusFinder)
		{
		...
		public void Check()
		{
			...
			foreach (var dataSource in StateHolderReader.Instance.StateReader.ApplicationScopeData.RegisteredDataSourceCollection.ToList())
			{
				...
				{
					bus.Send(new InitialLoadScheduleDay { Datasource = dataSource.DataSourceName, BusinessUnitId = businessUnitId, Timestamp = DateTime.UtcNow });
				}
			}
		}
	}
