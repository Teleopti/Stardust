...
namespace Teleopti.Ccc.Sdk.ServiceBus
{
	...
	public class NewAbsenceRequestConsumer : ConsumerOf<NewAbsenceRequestCreated>
	{
		...
		public NewAbsenceRequestConsumer(ICurrentUnitOfWorkFactory unitOfWorkFactory, IPersonAbsenceAccountProvider personAbsenceAccountProvider, ICurrentScenario scenarioRepository, IPersonRequestRepository personRequestRepository, ISchedulingResultStateHolder schedulingResultStateHolder,
										...
										IPersonAccountUpdater personAccountUpdater, IToggleManager toggleManager)
		{
		...
		public void Consume(NewAbsenceRequestCreated message)
		{
			...
			using (IUnitOfWork unitOfWork = _unitOfWorkFactory.Current().CreateAndOpenUnitOfWork())
			{
				...
				else
				{
					...
					{
						updatePersonAccountBalancesForAbsence (unitOfWork);
					}

					var extractor = _absenceRequest.Person.WorkflowControlSet.GetExtractorForAbsence(_absenceRequest.Absence);
					...
					personAccountBalanceCalculator = setupPersonAccountBalanceCalculator(message, undoRedoContainer, dateOnlyPeriod, unitOfWork);
		...
		private void updatePersonAccountBalancesForAbsence (IUnitOfWork unitOfWork)
		{
			if (_personAccountUpdater.UpdateForAbsence (
				_absenceRequest.Person,
				_absenceRequest.Absence,
				new DateOnly (_absenceRequest.Period.StartDateTime)))
			{
				unitOfWork.PersistAll();	
			};
		}

		private IPersonAccountBalanceCalculator setupPersonAccountBalanceCalculator(NewAbsenceRequestCreated message, UndoRedoContainer undoRedoContainer, DateOnlyPeriod dateOnlyPeriod, IUnitOfWork unitOfWork)
		{
			var allAccounts = _personAbsenceAccountProvider.Find(_absenceRequest.Person);
			...
			if (personAccount == null)
			{
				...
				return new EmptyPersonAccountBalanceCalculator(_absenceRequest.Absence);
			}

			var affectedAccounts = personAccount.Find(new DateOnlyPeriod(dateOnlyPeriod.StartDate, DateOnly.MaxValue)).ToList();
			
			TrackAccounts(personAccount, dateOnlyPeriod);
			
			//We must have the current and all after...
			return new PersonAccountBalanceCalculator(affectedAccounts);
		}
