...
namespace Teleopti.Ccc.Sdk.ServiceBus.AbsenceReport
{
	...
	{
		private static readonly ILog Logger = LogManager.GetLogger(typeof(NewAbsenceReportConsumer));
			...
			using (IUnitOfWork unitOfWork = _unitOfWorkFactory.Current().CreateAndOpenUnitOfWork())
			{
				...
				else
				{
					...
					else
					{
						...
						var requestApprovalServiceScheduler = _factory.GetRequestApprovalService(businessRules, _scenarioRepository.Current(), _schedulingResultStateHolder);
