...
namespace Teleopti.Ccc.DomainTest.Forecasting
{
    ...
    public class WeekOfMonthItemTest
    {
        ...
        public void VerifyProperties()
        {
            ...
            {
                target = new WeekOfMonthItem(taskOwnerPeriod, volumeYear);
                Assert.AreEqual(14,target.DailyAverageTasks);
                Assert.AreEqual(7,target.TaskIndex);
                target.TaskIndex = 14;
                Assert.AreEqual(14,target.TaskIndex);
                Assert.AreEqual(196,target.AverageTasks);

                target.AverageTasks = 98;
                Assert.AreEqual(7, target.TaskIndex);
        ...
        public void ShouldAbleToChangeAverageTasksAfterItWasZero()
        {
            ...
            using (mocks.Playback())
            {
                ...
                Assert.AreEqual(7.14d, Math.Round(target.TaskIndex, 2));
            }
        }
