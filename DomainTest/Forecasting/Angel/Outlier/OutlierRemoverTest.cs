...
namespace Teleopti.Ccc.DomainTest.Forecasting.Angel.Outlier
{
	...
	public class OutlierRemoverTest
	{
		...
		public void ShouldRemoveOutliersAndUse3SigmaInstead()
		{
			...
			taskOwner.TotalStatisticAverageTaskTime.Should().Be.EqualTo(TimeSpan.FromSeconds(1000));
			taskOwner.TotalStatisticAverageAfterTaskTime.Should().Be.EqualTo(TimeSpan.FromSeconds(1000));
		...
		[Test]
		public void ShouldRemoveOutliersAndUse3SigmaInsteadForTasksWhenConsiderTrend()
		{
			var skill = SkillFactory.CreateSkill("testSkill");
			skill.TimeZone = TimeZoneInfo.FindSystemTimeZoneById("GMT Standard Time");
			var workload = WorkloadFactory.CreateWorkload(skill);
			...
			//Math.Round(taskOwnerWithoutOutlier.TotalStatisticAverageTaskTime.TotalSeconds, 3).Should().Be.EqualTo(713.136);
			//Math.Round(taskOwnerWithoutOutlier.TotalStatisticAverageAfterTaskTime.TotalSeconds, 3).Should().Be.EqualTo(713.136);
		}
	...
	{
		public FakeTeleoptiClassic(IIndexVolumes indexVolumes) : base(indexVolumes)
		{
		}

		public override ForecastMethodType Id => ForecastMethodType.TeleoptiClassicLongTerm;
	}
	...
	public class FakeTeleoptiClassicWithTrend : TeleoptiClassicWithTrend
	{
		...
		public override ForecastMethodType Id => ForecastMethodType.TeleoptiClassicLongTermWithTrend;
	}
}