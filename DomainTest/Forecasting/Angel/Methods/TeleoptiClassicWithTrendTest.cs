...
namespace Teleopti.Ccc.DomainTest.Forecasting.Angel.Methods
{
	...
	public class TeleoptiClassicWithTrendTest
	{
		...
		public void ShouldForecastTasksWithTrend()
		{
			...
			var result = target.ForecastTasks(historicalData, new DateOnlyPeriod(new DateOnly(2014, 1, 1), new DateOnly(2014, 1, 1)));
			...
			result.First().Value.Should().Be.EqualTo(Math.Round(expected, 4));
		}
