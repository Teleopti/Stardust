...
namespace Teleopti.Ccc.DomainTest.ResourcePlanner.Validation
{
	...
	public class PersonSchedulePeriodValidatorTest : ISetup
	{
		...
		public void NoSchedulePeriodShouldReturnValidationError()
		{
			...
			var result = Target.Validate(new FullValidationInput(null, new[] { person }, planningPeriod)).InvalidResources
		...
		public void PersonWithSchedulePeriodOfOneDay()
		{
			...
			Target.Validate(new FullValidationInput(null, new[] { person }, date.ToDateOnlyPeriod())).InvalidResources
				.Where(x => x.ValidationTypes.Contains(typeof(PersonSchedulePeriodValidator)))
				.Should().Be.Empty();
		}
