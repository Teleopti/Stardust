...
namespace Teleopti.Ccc.DomainTest.ResourcePlanner.Hints
{
	...
	public class BlockSchedulingNotMatchShiftBagHintTest
	{
		...
		public void ShouldReturnShiftStartTimeNotMatchingShiftBag()
		{
			...
			result.First().ValidationErrors.First().Should().Be.EqualTo(string.Format(Resources.StartTimeNotMatchingShiftBag, personAssignment.Period.StartDateTime.TimeOfDay, personAssignment.Date.ToShortDateString(),
		...
		public void ShouldReturnShiftCategoryNotMatchingShiftBag()
		{
			...
			result.First().ValidationErrors.Count.Should().Be.EqualTo(1);
			result.First().ValidationTypes.First().Name.Should().Be.EqualTo(nameof(BlockSchedulingNotMatchShiftBagHint));
			result.First().ValidationErrors.First().Should().Be.EqualTo(string.Format(Resources.ShiftCategoryNotMatchingShiftBag, personAssignment.ShiftCategory.Description.ShortName, personAssignment.Date.ToShortDateString(),
				agent.PersonPeriodCollection.First().RuleSetBag.Description.Name));
		}
		...
		public void ShouldReturnShiftNotMatchingShiftBag()
		{
			...
			result.First().ValidationErrors.Count.Should().Be.EqualTo(1);
			result.First().ValidationTypes.First().Name.Should().Be.EqualTo(nameof(BlockSchedulingNotMatchShiftBagHint));
			result.First().ValidationErrors.First().Should().Be.EqualTo(string.Format(Resources.ShiftNotMatchingShiftBag, personAssignment.Date.ToShortDateString(),
				agent.PersonPeriodCollection.First().RuleSetBag.Description.Name));
		}
	}
}