...
namespace Teleopti.Ccc.DomainTest.SchedulingScenarios.ResourceCalculation
{
	...
	public class CascadingResourceCalculationSmallOverstaffingTests
	{
		...
		public void ShouldNotShovelIfTooSmallOverstaff()
		{
			...
			var primarySkill = new Skill("_").For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(1).WithOpenHours(8, 9);
			var primarySkillDay = primarySkill.CreateSkillDayWithDemand(scenario, dateOnly, primarySkillDemand);
			var subSkill = new Skill("_").For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(2).WithOpenHours(8, 9);
		...
		public void ShouldShovelIfSumOfOverstaffingIsEnough()
		{
			...
			var activity = new Activity("_");
			var dateOnly = DateOnly.Today;
			var primarySkill1 = new Skill("_").For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(1).WithOpenHours(8, 9);
			var primarySkill1Day = primarySkill1.CreateSkillDayWithDemand(scenario, dateOnly, primarySkillDemand);
			var primarySkill2 = new Skill("_").For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(1).WithOpenHours(8, 9);
		...
		public void ShouldStopShovelWhenLimitIsReached()
		{
			...
			var primarySkill = new Skill("_").For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(1).WithOpenHours(8, 9);
			var primarySkillDay = primarySkill.CreateSkillDayWithDemand(scenario, dateOnly, primarySkillDemand);
			var subSkill1 = new Skill("_").For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(2).WithOpenHours(8, 9);
			...
			var subSkill2 = new Skill("_").For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(2).WithOpenHours(8, 9);
