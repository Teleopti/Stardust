...
namespace Teleopti.Ccc.DomainTest.SchedulingScenarios.ResourceCalculation
{
	...
	public class CascadingResourceCalculationOverstaffedParallellSkillsTest
	{
		...
		public void ShouldMoveResourceToTwoSkillsWithSameDemand()
		{
			...
			var skillA = new Skill("A").For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(1).IsOpenBetween(8, 9);
			var skillDayA = skillA.CreateSkillDayWithDemand(scenario, dateOnly, 0.5);
			var skillB1 = new Skill("B1").For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(2).IsOpenBetween(8, 9);
			var skillDayB1 = skillB1.CreateSkillDayWithDemand(scenario, dateOnly, 1);
			var skillB2 = new Skill("B2").For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(2).IsOpenBetween(8, 9);
		...
		public void ShouldMoveAllResourcesToOneCascadingSkillIfOtherParallelSkillIsOverstaffed()
		{
			...
			var skillB2 = new Skill("B2Overstaffed").For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(2).IsOpenBetween(8, 9);
		...
		public void ShouldNotMoveResourcesFromUnderstaffedPrimarySkill()
		{
			...
			var activity = new Activity("_");
			var dateOnly = DateOnly.Today;
			var skillA1 = new Skill("skillA1").For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(1).IsOpenBetween(8, 9);
			var skillDayA1 = skillA1.CreateSkillDayWithDemand(scenario, dateOnly, 0); //-> lite mer än 1 överbemannat
			var skillA2 = new Skill("skillA2").For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(1).IsOpenBetween(8, 9);
			var skillDayA2 = skillA2.CreateSkillDayWithDemand(scenario, dateOnly, 2); //-> lite mer än 1 underbemannat
			var skillB = new Skill("skillB").For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(2).IsOpenBetween(8, 9);
		...
		public void ShouldOnlyConsiderOpenParalellPrimarySkillsWhenMovingResourcesToSubSkill()
		{
			...
			var activity = new Activity("_");
			var dateOnly = DateOnly.Today;
			var skillA1 = new Skill("skillA1").For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(1).IsOpenBetween(7, 9);
			...
			var skillB = new Skill("skillB").For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(2).IsOpenBetween(7, 9);
			var skillDayB = skillB.CreateSkillDayWithDemand(scenario, dateOnly, 1);
			var agent = new Person().InTimeZone(TimeZoneInfo.Utc);
			agent.AddPeriodWithSkills(new PersonPeriod(DateOnly.MinValue, new PersonContract(new Contract("_"), new PartTimePercentage("_"), new ContractSchedule("_")), new Team { Site = new Site("_") }), new[] { skillA1, skillA2, skillB });
			var ass = new PersonAssignment(agent, scenario, dateOnly);
		...
		public void ShouldHandleMultipleSkillgroupsContainingSamePrimarySkill()
		{
			...
			var activity = new Activity("_");
			var dateOnly = DateOnly.Today;
			var skillA1 = new Skill("A1").For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(1).IsOpenBetween(8, 9);
			var skillDayA1 = skillA1.CreateSkillDayWithDemand(scenario, dateOnly, 0);
			var skillA2 = new Skill("A2").For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(1).IsOpenBetween(8, 9);
			var skillDayA2 = skillA2.CreateSkillDayWithDemand(scenario, dateOnly, 0);
			var skillB1 = new Skill("B1").For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(2).IsOpenBetween(8, 9);
			var skillDayB1 = skillB1.CreateSkillDayWithDemand(scenario, dateOnly, 1);
			var skillB2 = new Skill("B2").For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(2).IsOpenBetween(8, 9);
		...
		public void ShouldHandleMultipleSkillgroupsContainingSamePrimarySkill_MultipleAgents()
		{
			...
			var activity = new Activity("_");
			var dateOnly = DateOnly.Today;
			var skillA1 = new Skill("A1").DefaultResolution(60).For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(1).IsOpenBetween(8, 9);
			var skillDayA1 = skillA1.CreateSkillDayWithDemand(scenario, dateOnly, 1); //one agent should be put here
			var skillA2 = new Skill("A2").DefaultResolution(60).For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(1).IsOpenBetween(8, 9);
			var skillDayA2 = skillA2.CreateSkillDayWithDemand(scenario, dateOnly, 1); //one agent should be put here
			var skillB1 = new Skill("B1").DefaultResolution(60).For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(2).IsOpenBetween(8, 9);
			var skillDayB1 = skillB1.CreateSkillDayWithDemand(scenario, dateOnly, 2); //one agent should be put here
			var skillB2 = new Skill("B2").DefaultResolution(60).For(activity).InTimeZone(TimeZoneInfo.Utc).WithId().WithCascadingIndex(2).IsOpenBetween(8, 9);
		...
		public void ShouldValueSkillGroupsWithMorePrimarySkillsHigherWhenCascadingIndexesAreEqualOnOtherSkillGroup()
		{
			...
			var skillPlatinum = new Skill("platinum").For(activity).InTimeZone(TimeZoneInfo.Utc).DefaultResolution(30).WithCascadingIndex(1).WithId().IsOpen(new TimePeriod(10, 0, 10, 30));
			var skillGold = new Skill("gold").For(activity).InTimeZone(TimeZoneInfo.Utc).DefaultResolution(30).WithCascadingIndex(1).WithId().IsOpen(new TimePeriod(10, 0, 10, 30));
			var skillPlatinum1 = new Skill("platinum1").For(activity).InTimeZone(TimeZoneInfo.Utc).DefaultResolution(30).WithCascadingIndex(1).WithId().IsOpen(new TimePeriod(10, 0, 10, 30));
			var skillSilver = new Skill("silver").For(activity).InTimeZone(TimeZoneInfo.Utc).DefaultResolution(30).WithCascadingIndex(2).WithId().IsOpen(new TimePeriod(10, 0, 10, 30));
			var skillBronze = new Skill("bronze").For(activity).InTimeZone(TimeZoneInfo.Utc).DefaultResolution(30).WithCascadingIndex(3).WithId().IsOpen(new TimePeriod(10, 0, 10, 30));
			var skillBronze1 = new Skill("bronze1").For(activity).InTimeZone(TimeZoneInfo.Utc).DefaultResolution(30).WithCascadingIndex(4).WithId().IsOpen(new TimePeriod(10, 0, 10, 30));
		...
		public void ShouldValueSkillGroupsWithMoreSubSkillsHigherWhenCascadingIndexesAreEqualOnOtherSkillGroup()
		{
			...
			var skillGold = new Skill("gold").For(activity).InTimeZone(TimeZoneInfo.Utc).DefaultResolution(30).WithCascadingIndex(1).WithId().IsOpen(new TimePeriod(10, 0, 10, 30));
			var skillBronze0 = new Skill("silver").For(activity).InTimeZone(TimeZoneInfo.Utc).DefaultResolution(30).WithCascadingIndex(2).WithId().IsOpen(new TimePeriod(10, 0, 10, 30));
			var skillBronz1 = new Skill("bronze").For(activity).InTimeZone(TimeZoneInfo.Utc).DefaultResolution(30).WithCascadingIndex(2).WithId().IsOpen(new TimePeriod(10, 0, 10, 30));
			var skillBronze2 = new Skill("bronze1").For(activity).InTimeZone(TimeZoneInfo.Utc).DefaultResolution(30).WithCascadingIndex(2).WithId().IsOpen(new TimePeriod(10, 0, 10, 30));
		...
		public void ShouldValueSkillGroupsWithLowerSubSkillIndexesHigher()
		{
			...
			var skillGold = new Skill("gold").For(activity).InTimeZone(TimeZoneInfo.Utc).DefaultResolution(30).WithCascadingIndex(1).WithId().IsOpen(new TimePeriod(10, 0, 10, 30));
			var skillSilver = new Skill("silver").For(activity).InTimeZone(TimeZoneInfo.Utc).DefaultResolution(30).WithCascadingIndex(2).WithId().IsOpen(new TimePeriod(10, 0, 10, 30));
			var skillSilver1 = new Skill("silver1").For(activity).InTimeZone(TimeZoneInfo.Utc).DefaultResolution(30).WithCascadingIndex(2).WithId().IsOpen(new TimePeriod(10, 0, 10, 30));
			var skillBronze = new Skill("bronze").For(activity).InTimeZone(TimeZoneInfo.Utc).DefaultResolution(30).WithCascadingIndex(3).WithId().IsOpen(new TimePeriod(10, 0, 10, 30));
