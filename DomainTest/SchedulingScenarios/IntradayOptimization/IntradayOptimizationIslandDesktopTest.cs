...
namespace Teleopti.Ccc.DomainTest.SchedulingScenarios.IntradayOptimization
{
	...
	public class IntradayOptimizationIslandDesktopTest : ISetup, IConfigureToggleManager
	{
		...
		public void ShouldNotPlaceSameShiftForAllAgentsInIsland()
		{
			...
			var skill = new Skill("_").For(phoneActivity).InTimeZone(TimeZoneInfo.Utc).WithId().WithFullOpenHours();
			...
			Target.Optimize(schedulerStateHolderFrom.Schedules.SchedulesForDay(dateOnly).Select(x => x.Person).Distinct(), new DateOnlyPeriod(dateOnly, dateOnly), new OptimizationPreferencesDefaultValueProvider().Fetch(), null);
		...
		public void ShouldConsiderAgentsNotPartOfAllPermittedPersons()
		{
			...
			Target.Optimize(new[] { oneAgent }, new DateOnlyPeriod(dateOnly, dateOnly), new OptimizationPreferencesDefaultValueProvider().Fetch(), null);
		...
		public void ShouldSetScheduleTag()
		{
			...
			Target.Optimize(new[] { agent }, new DateOnlyPeriod(dateOnly, dateOnly), optPref, null);
		...
		public void ShouldNotDuplicateScheduleData()
		{
			var scenario = new Scenario("_");
			var dateOnly = new DateOnly(2010, 1, 1);
			var skill = new Skill("_", "_", Color.Empty, 15, new SkillTypePhone(new Description(), ForecastSource.InboundTelephony)) { Activity = new Activity("_"), TimeZone = TimeZoneInfo.Utc }.WithId().WithFullOpenHours();
			var skillDay = skill.CreateSkillDayWithDemandPerHour(scenario, dateOnly, TimeSpan.FromMinutes(60), new Tuple<int, TimeSpan>(17, TimeSpan.FromMinutes(360)));
			var agent = new Person().WithId().InTimeZone(TimeZoneInfo.Utc);
			agent.AddPeriodWithSkill(new PersonPeriod(dateOnly, new PersonContract(new Contract("_"), new PartTimePercentage("_"), new ContractSchedule("_")), new Team { Site = new Site("_") }), skill);
			agent.AddSchedulePeriod(new SchedulePeriod(dateOnly, SchedulePeriodType.Week, 1));
			...
			using (CurrentAuthorization.ThreadlyUse(new missingPermissionsOnAgentsOverlappingSchedulePeriod()))
			{
				...
				() =>
					Target.Optimize(new[] { agent},
						new DateOnlyPeriod(dateOnly, dateOnly), new OptimizationPreferencesDefaultValueProvider().Fetch(), null));
			}
		}
