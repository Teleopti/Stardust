...
namespace Teleopti.Ccc.DomainTest.ApplicationLayer
{
	...
	public class MoveActivityCommandHandlerTest
	{
		...
		public void ShouldRaiseActivityMovedEvent()
		{
			...
			var expectedStartInUtc = TimeZoneHelper.ConvertToUtc(expectedStartInLocal, userTimeZoneInfo);
			var @event = personAssignmentRepository.Single().PopAllEvents(null).OfType<ActivityMovedEvent>().Single();
			...
			@event.LogOnBusinessUnitId.Should().Be(scenario.Current().GetOrFillWithBusinessUnit_DONTUSE().Id.GetValueOrDefault());
		}
