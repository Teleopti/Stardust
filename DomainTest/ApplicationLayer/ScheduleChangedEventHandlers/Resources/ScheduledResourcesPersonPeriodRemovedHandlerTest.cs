...
{
	[TestFixture]
	public class ScheduledResourcesPersonPeriodRemovedHandlerTest
	{
		private ScheduledResourcesPersonPeriodRemovedHandler _target;
		...
		public void Setup()
		{
			...
			_target = new ScheduledResourcesPersonPeriodRemovedHandler(new ScheduledResourcesReadModelUpdater(_storage, null, new ControllableEventSyncronization()), _readModelFinder, _skillRepository, _scenarioRepository);
		}

		[Test]
		public void ShouldHandlePersonPeriodWithSkillEfficiencyRemoved()
		{
			...
			_storage.Stub(x => x.AddResources(_activityId, false, _skill.Id.ToString(), _period.MovePeriod(TimeSpan.FromHours(10)), 1, 1)).Return(2);
			_storage.Stub(x => x.RemoveResources(_activityId, _skill.Id.ToString(), _period, 1, 1)).Return(3);
			_storage.Stub(x => x.RemoveResources(_activityId, _skill.Id.ToString(), _period.MovePeriod(TimeSpan.FromHours(48)), 1, 1)).Return(4);
			_skillRepository.Stub(x => x.MinimumResolution()).Return(15);
			_scenarioRepository.Stub(x => x.LoadDefaultScenario()).Return(_scenario);

			_target.Handle(new PersonPeriodRemovedEvent
				{
					PersonId = _personId,
			...
			_storage.AssertWasCalled(x => x.AddSkillEfficiency(2,_skill.Id.GetValueOrDefault(),0.95d));
			_storage.AssertWasCalled(x => x.RemoveSkillEfficiency(3,_skill.Id.GetValueOrDefault(),0.95d));
