...
{
	[TestFixture]
	public class ScheduledResourcesPersonSkillActivatedHandlerTest
	{
		private ScheduledResourcesPersonSkillActivatedHandler _target;
		...
		public void Setup()
		{
			...
			_target = new ScheduledResourcesPersonSkillActivatedHandler(new ScheduledResourcesReadModelUpdater(_storage, null, new ControllableEventSyncronization()), _readModelFinder, _skillRepository, _scenarioRepository);
		}
		...
		public void ShouldHandleActivatedSkillWithProficiencyWhenNoPreviousActivePersonSkill()
		{
			...
			_target.Handle(new PersonSkillActivatedEvent
				{
					PersonId = _personId,
					...
					EndDate = _date.Date,
					SkillsBefore =
						new[] {new PersonSkillDetail {Active = false, Proficiency = 0.9, SkillId = _skill.Id.GetValueOrDefault()}}
				});
						...
						new[]
							{
								...
								new PersonSkillDetail {Active = false, Proficiency = 0.8, SkillId = newSkillId}
							}
				});
