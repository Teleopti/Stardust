...
{
	[TestFixture]
	public class ScheduledResourcesPersonReactivatedHandlerTest
	{
		private ScheduledResourcesPersonReactivatedHandler _target;
		...
		public void Setup()
		{
			...
			_target = new ScheduledResourcesPersonReactivatedHandler(new ScheduledResourcesReadModelUpdater(_storage, null, new ControllableEventSyncronization()), _readModelFinder, _skillRepository, _scenarioRepository);
		}

		[Test]
		public void ShouldHandlePersonWithSkillEfficiencyReactivated()
		{
			...
			_storage.Stub(x => x.AddResources(_activityId, false, _skill.Id.ToString(), _period.MovePeriod(TimeSpan.FromHours(10)), 1, 1)).Return(3);
