...
{
	[DomainTest]
	public class AnalyticsForecastUpdaterTest : IExtendSystem
	{
		public AnalyticsForecastUpdater Target;
		...
		{
			extend.AddService<AnalyticsForecastUpdater>();
		}
		...
		public void ShouldUpdateAnalyticsWithMultipleForecastedDaysInSameEvent()
		{
			...
			AnalyticsDateRepository.HasDatesBetween(new DateTime(2018, 9, 27), new DateTime(2018, 10, 02));
			...
			var forecastedDay2 = new DateOnly(2018, 10, 02);
		...
		public void ShouldThrowWhenScenarioMissingFromAnalytics()
		{
			...
			Assert.Throws<ScenarioMissingInAnalyticsException>(() => Target.Handle(new ForecastChangedEvent
			{
				SkillDayIds = new[] { skillDay.Id.GetValueOrDefault() },
				LogOnBusinessUnitId = businessUnitId
			}));
		...
		public void ShouldThrowWhenWorkloadMissingFromAnalytics()
		{
			...
			Assert.Throws<WorkloadMissingInAnalyticsException>(() => Target.Handle(new ForecastChangedEvent
			{
				SkillDayIds = new[] { skillDay.Id.GetValueOrDefault() },
				LogOnBusinessUnitId = businessUnitId
			}));
		...
		public void ShouldAddForecastForMountainTimezone()
		{
			...
			Target.Handle(new ForecastChangedEvent
			{
				SkillDayIds = new[] { skillDay1.Id.GetValueOrDefault() },
				LogOnBusinessUnitId = businessUnitId
			});
			...
			Target.Handle(new ForecastChangedEvent
			{
				SkillDayIds = new[] { skillDay2.Id.GetValueOrDefault() },
				LogOnBusinessUnitId = businessUnitId
			});
		...
		public void ShouldOnlyDeleteWorkloadsForSpecifiedSkillDay()
		{
			...
			Target.Handle(new ForecastChangedEvent
			{
				SkillDayIds = new[] { skillDayToday.Id.GetValueOrDefault() },
				LogOnBusinessUnitId = businessUnitId
			});
			Target.Handle(new ForecastChangedEvent
			{
				SkillDayIds = new[] { skillDayYesterday.Id.GetValueOrDefault() },
				LogOnBusinessUnitId = businessUnitId
			});
