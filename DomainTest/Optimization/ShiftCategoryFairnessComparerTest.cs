...
//                    0,072111026			
		...
		public void ShouldContainCalculateStandardDeviation()
		{
			...
			var catDic = new Dictionary<IShiftCategory, int> { { cat, 100 } };
			...
			Assert.That(result.ShiftCategoryFairnessCompareValues[0].Original, Is.EqualTo(1));
			Assert.That(result.ShiftCategoryFairnessCompareValues[1].Original, Is.EqualTo(.0));
			Assert.That(result.ShiftCategoryFairnessCompareValues[2].Original, Is.EqualTo(.0));
			Assert.That(result.ShiftCategoryFairnessCompareValues[3].Original, Is.EqualTo(.0));
			...
			//TALHA fix the code so this test pass
			Assert.That(result.StandarDeviation, Is.EqualTo(0.072111026));
		}
	}
