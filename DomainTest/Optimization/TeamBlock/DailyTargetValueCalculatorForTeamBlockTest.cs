...
namespace Teleopti.Ccc.DomainTest.Optimization.TeamBlock
{
	...
	public class DailyTargetValueCalculatorForTeamBlockTest
	{
		...
		public void TestTargetValueWithOneScheduleDayOneSkill()
		{
			...
			using (_mock.Record())
			{
				Expect.Call(_groupPersonSkillAggregator.AggregatedSkills(_baseLineData.PersonList, _dateOnlyPeriod))
					.IgnoreArguments()
					.Return(_skillList);
				...
				Expect.Call(_skillStaffPeriodToSkillIntervalDataMapper.MapSkillIntervalData(_skillStaffPeriodList,
					DateOnly.Today.AddDays(1), TimeZoneGuard.Instance.TimeZone))
					.Return(new List<ISkillIntervalData> {skillIntervalData1});
				Expect.Call(
					_intervalDataDivider.SplitSkillIntervalData(new List<ISkillIntervalData> {skillIntervalData1}, 15))
					.Return(new List<ISkillIntervalData> {skillIntervalData1});
				...
				Expect.Call(_dayIntervalDataCalculator.Calculate(dateToSkillIntervalDic, DateOnly.Today))
					.IgnoreArguments()
					.Return(timeToSkillIntervalDic);
		...
		public void TestSkillWithMidnightBreak()
		{
			...
			using (_mock.Record())
			{
				...
				Expect.Call(_locateMissingIntervalsIfMidNightBreak.GetMissingSkillStaffPeriods(DateOnly.Today.AddDays(1),
					...
					.IgnoreArguments()
					.Return(new List<ISkillStaffPeriod>());
				Expect.Call(_filterOutIntervalsAfterMidNight.Filter(_skillStaffPeriodList, DateOnly.Today.AddDays(1),
					TimeZoneInfo.Utc))
					.IgnoreArguments()
					.Return(_skillStaffPeriodList);
				skillDayExpectCalls(_skillDay1, skillIntervalList);
