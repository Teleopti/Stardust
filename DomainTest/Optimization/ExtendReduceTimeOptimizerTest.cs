    ...
    public class ExtendReduceTimeOptimizerTest
    {
        ...
        public void ShouldReturnTrueIfDateToLengthenIsSuccessful()
        {
            ...
            using (_mocks.Record())
            {
				...
				Expect.Call(_scheduleService.SchedulePersonOnDay(_scheduleDay1, _schedulingOptions, _effectiveRestriction, _resourceCalculateDelayer, _rollbackService)).IgnoreArguments()
                    .Return(true);
				Expect.Call(_scheduleService.SchedulePersonOnDay(_scheduleDay2, _schedulingOptions, _effectiveRestriction, _resourceCalculateDelayer, _rollbackService)).IgnoreArguments()
                    .Return(false);
        ...
        public void ShouldReturnFalseIfNoDaysFound()
        {
            ...
            using (_mocks.Record())
            {
				Expect.Call(_optimizationLimits.OverLimitsCounts(_matrix)).Return(_overLimtitesResult);
	            Expect.Call(_optimizationLimits.MoveMaxDaysOverLimit()).Return(false).Repeat.AtLeastOnce();
                Expect.Call(_decisionMaker.Execute(_matrix, _dataExtractor)).Return(decisionMakerResult);
            }
		...
		private void commonMocks(ExtendReduceTimeDecisionMakerResult decisionMakerResult)
		{
            ...
            Expect.Call(_decisionMaker.Execute(_matrix, _dataExtractor))
            ...
            _resourceCalculateDelayer.CalculateIfNeeded(new DateOnly(2011, 1, 1), null, false);
			LastCall.Repeat.Any();
            _resourceCalculateDelayer.CalculateIfNeeded(new DateOnly(2011, 1, 2), null, false);
			LastCall.Repeat.Any();
		...
		[Test]
		public void ShouldRollbackAndReturnTrueIfMinWorktimePerWeekValidateFails()
		{
			var decisionMakerResult = new ExtendReduceTimeDecisionMakerResult();
			decisionMakerResult.DayToLengthen = new DateOnly(2011, 1, 1);
			decisionMakerResult.DayToShorten = new DateOnly(2011, 1, 2);
			...
			using (_mocks.Record())
			{
				...
				Expect.Call(_rollbackService.ModificationCollection).Return(new List<IScheduleDay>()).Repeat.AtLeastOnce();
				Expect.Call(_scheduleService.SchedulePersonOnDay(_scheduleDay1, _schedulingOptions, _effectiveRestriction, _resourceCalculateDelayer, _rollbackService)).IgnoreArguments().Return(false);
				Expect.Call(_scheduleService.SchedulePersonOnDay(_scheduleDay2, _schedulingOptions, _effectiveRestriction, _resourceCalculateDelayer, _rollbackService)).IgnoreArguments().Return(true);
