...
namespace Teleopti.Ccc.DomainTest.Optimization
{
    ...
    public class DayOffDecisionMakerExecuterTest
    {
        ...
        public void VerifyExecuteReturnsTrue()
        {
            ...
            using (_mocks.Record())
            {
				Expect.Call(_optimizationLimits.OverLimitsCounts(_scheduleMatrix)).Return(_lastOverLimitCounts);
	            Expect.Call(_optimizationLimits.HasOverLimitExceeded(_lastOverLimitCounts, _scheduleMatrix)).Return(false);
            	...
            	Expect.Call(
            		() =>
            		_resourceOptimizationHelper.ResourceCalculateDate(DateOnly.MinValue, true)).IgnoreArguments().Repeat.
            		AtLeastOnce();
				...
				Expect.Call(_scheduleService.SchedulePersonOnDay(null, schedulingOptions, null, _resourceCalculateDelayer, _rollbackService)).IgnoreArguments()
		...
		[Test]
		public void ShouldLockScheduledDayAndReturnTrueWhenMinWorkTimePerWeekIsBroken()
		{
			ISchedulingOptions schedulingOptions = new SchedulingOptions();
			...
			using (_mocks.Record())
			{
				...
				Expect.Call(_schedulingOptionsCreator.CreateSchedulingOptions(_optimizerPreferences)).Return(schedulingOptions);
				Expect.Call(_originalStateContainer.OriginalWorkTime()).Return(new TimeSpan());
				_rollbackService.ClearModificationCollection();
				...
				_rollbackService.Modify(part);
				LastCall.Repeat.Twice();
				Expect.Call(scheduleDay8.Day).Return(new DateOnly(2010, 1, 1)).Repeat.AtLeastOnce();
				Expect.Call(scheduleDay9.Day).Return(new DateOnly(2010, 1, 2)).Repeat.AtLeastOnce();
				...
				Expect.Call(_scheduleService.SchedulePersonOnDay(null, schedulingOptions, null, _resourceCalculateDelayer, _rollbackService)).IgnoreArguments().Return(true).Repeat.Twice();
				...
				Expect.Call(_decider.DecideDates(part, part)).Return(new List<DateOnly> {DateOnly.MinValue}).Repeat.AtLeastOnce();
        ...
        public void ShouldReturnFalseWhenBreakingDayOffRule()
        {
            ...
            using (_mocks.Record())
            {
	           Expect.Call(_optimizationLimits.OverLimitsCounts(_scheduleMatrix)).Return(_lastOverLimitCounts);
                ...
                Expect.Call(scheduleDay9.Day)
                    .Return(new DateOnly(2010, 1, 2)).Repeat.Twice();
				Expect.Call(() => _resourceOptimizationHelper.ResourceCalculateDate(DateOnly.MinValue, true)).Repeat.AtLeastOnce();
				Expect.Call(() => _resourceOptimizationHelper.ResourceCalculateDate(new DateOnly(2010,1,2), true)).Repeat.AtLeastOnce();
        ...
        public void ShouldReturnTrueWhenBreakingDayOffRuleButHandleConflict()
        {
            ...
            using (_mocks.Record())
            {
                ...
                Expect.Call(scheduleDay9.Day)
                    .Return(new DateOnly(2010, 1, 2)).Repeat.Twice();
				Expect.Call(() => _resourceOptimizationHelper.ResourceCalculateDate(new DateOnly(2010, 1, 1), true)).IgnoreArguments().Repeat.AtLeastOnce();
                Expect.Call(_workShiftBackToLegalStateService.Execute(_scheduleMatrix, schedulingOptions, _rollbackService))
                    .Return(true).Repeat.Once();
                Expect.Call(_workShiftBackToLegalStateService.RemovedDays)
                    .Return(new List<DateOnly>());
                ...
                Expect.Call(_dayOffOptimizerConflictHandler.HandleConflict(schedulingOptions, new DateOnly()))
                    .Return(true);
            	Expect.Call(_scheduleService.SchedulePersonOnDay(null, schedulingOptions, null, _resourceCalculateDelayer,
		...
		public void VerifyExecuteWithBackToLegalState()
		{
			...
			using (_mocks.Record())
			{
				...
				Expect.Call(
					() =>
					_resourceOptimizationHelper.ResourceCalculateDate(new DateOnly(2010, 1, 2), true));
				...
				_resourceOptimizationHelper.ResourceCalculateDate(DateOnly.Today, true);
				_resourceOptimizationHelper.ResourceCalculateDate(DateOnly.Today.AddDays(1), true);
		...
		public void VerifyExecuteWithBackToLegalStateAndFailOnMaxShift()
		{
			...
			using (_mocks.Record())
			{
				...
				Expect.Call(() => _resourceOptimizationHelper.ResourceCalculateDate(DateOnly.MinValue, true)).Repeat.Times(3);
		...
		public void ShouldRollbackAndReturnFalseIfRescheduleFails()
		{
			...
			using (_mocks.Record())
			{
				...
				Expect.Call(() => _rollbackService.Rollback());
				Expect.Call(() =>_resourceOptimizationHelper.ResourceCalculateDate(DateOnly.MinValue, true)).IgnoreArguments();
				Expect.Call(() =>_resourceOptimizationHelper.ResourceCalculateDate(DateOnly.MinValue.AddDays(1), true)).IgnoreArguments();
		...
		[Test]
		public void VerifyOverLimitExceeded()
		{

			// test criterion
			const bool overtimeLimitExceeded = true;

			ISchedulingOptions schedulingOptions = new SchedulingOptions();
			_optimizerPreferences.Advanced.UseTweakedValues = true;
			...
			using (_mocks.Record())
			{
				// mocks needed to come to the right place ...
				Expect.Call(_optimizationLimits.OverLimitsCounts(_scheduleMatrix))
					.Return(_lastOverLimitCounts);
				Expect.Call(_optimizationLimits.HasOverLimitExceeded(_lastOverLimitCounts, _scheduleMatrix))
					.Return(overtimeLimitExceeded);

				Expect.Call(_schedulingOptionsCreator.CreateSchedulingOptions(_optimizerPreferences))
					.Return(schedulingOptions);
				Expect.Call(_originalStateContainer.OriginalWorkTime())
					.Return(new TimeSpan());
				_rollbackService.ClearModificationCollection();
				Expect.Call(_scheduleMatrix.OuterWeeksPeriodDays)
					.Return(new ReadOnlyCollection<IScheduleDayPro>(outerWeekList))
					.Repeat.Times(4);
				Expect.Call(scheduleDay8.DaySchedulePart())
					.Return(part).Repeat.Twice();
				Expect.Call(scheduleDay9.DaySchedulePart())
					.Return(part).Repeat.Twice();
				Expect.Call(part.PersonAssignment())
					.Return(_personAssignment);
				Expect.Call(_personAssignment.ShiftCategory)
					.Return(new ShiftCategory("s"));
				Expect.Call(part.Clone()).Return(part).Repeat.Times(4);
				part.DeleteMainShift(part);
				part.CreateAndAddDayOff(_dayOffTemplate);
				part.DeleteDayOff();
				Expect.Call(() => _rollbackService.Modify(part)).Repeat.Twice();
				...
				Expect.Call(_dayOffOptimizerValidator.Validate(dateOnly, _scheduleMatrix))
					.Return(true);
				Expect.Call(_optimizationLimits.MoveMaxDaysOverLimit())
					.Return(false).Repeat.AtLeastOnce();
				Expect.Call(_decider.DecideDates(part, part))
					.Return(new List<DateOnly> { DateOnly.MinValue }).Repeat.Times(2);
				Expect.Call(_periodValueCalculator.PeriodValue(IterationOperationOption.DayOffOptimization))
					.Return(0);
				setExpectationsForSettingOriginalShiftCategory();

				// sepcial part for criterion expectation: rollback and lock days 
				Expect.Call(() => _rollbackService.Rollback());
				Expect.Call(() => _scheduleMatrix.LockPeriod(new DateOnlyPeriod(new DateOnly(2010, 1, 2), new DateOnly(2010, 1, 2))));
			}
			...
			{
				_target = createTarget();
				result = _target.Execute(bitArrayAfterMove, bitArrayBeforeMove, _scheduleMatrix, _originalStateContainer, true, true, false);
			}
