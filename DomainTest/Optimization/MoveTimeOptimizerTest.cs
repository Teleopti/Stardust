    ...
    public class MoveTimeOptimizerTest
    {
		...
		private void tryScheduleSecondDate(bool success)
		{
			...
			Expect.Call(_scheduleService.SchedulePersonOnDay(_mostOverStaffSchedulePart, _schedulingOptions,
			                                                 _effectiveRestriction, _resourceCalculateDelayer, _rollbackService))
				.IgnoreArguments()
				.Return(success);
    	...
    	private void tryScheduleFirstDate(bool success, bool sameContractTime)
		{
			...
			Expect.Call(
				() => _mainShiftOptimizeActivitySpecificationSetter.SetMainShiftOptimizeActivitySpecification(null, null, null, DateOnly.MinValue)).IgnoreArguments();
			Expect.Call(_scheduleService.SchedulePersonOnDay(_mostUnderStaffSchedulePart, _schedulingOptions, _effectiveRestriction, _resourceCalculateDelayer, _rollbackService)).IgnoreArguments()
				.Return(success);
			if(success)
			{
				Expect.Call(_mostUnderStaffSchedulePart.ProjectionService()).Return(_projectionService);
		...
		{
            Expect.Call(() => _resourceOptimizationHelper.ResourceCalculate(_mostUnderStaffDate, new ResourceOptimizationData(new FakeSchedulingResultStateHolder(), false, false))).IgnoreArguments();
            Expect.Call(() => _resourceOptimizationHelper.ResourceCalculate(_mostOverStaffDate, new ResourceOptimizationData(new FakeSchedulingResultStateHolder(), false, false))).IgnoreArguments();
		}
    }
}
