...
namespace Teleopti.Ccc.DomainTest.Optimization
{
    public class DayOffOptimizerConflictHandlerTest
    {
        ...
        public void Setup()
        {
            ...
            _target = new DayOffOptimizerConflictHandler(_scheduleMatrixPro, _scheduleService, _effectiveRestrictionCreator, _schedulingOptions, _rollbackService);
        ...
        public void ShouldReturnTrueWhenDayBeforeRescheduled()
        {
            ...
            {
                var result = _target.HandleConflict(_dateOnly);
        ...
        public void ShouldNotTryToRescheduleLockedDates()
        {
            using (_mock.Record())
            {
                //Expect.Call(_scheduleMatrixPro.UserLockedDates).Return(new List<DateOnly>{_dateOnly.AddDays(-1)});
                //Expect.Call(_scheduleMatrixPro.UserLockedDates).Return(new List<DateOnly> { _dateOnly.AddDays(1) });
                Expect.Call(_scheduleMatrixPro.GetScheduleDayByKey(_dateOnly.AddDays(-1))).Return(_scheduleDayPro);
                Expect.Call(_scheduleMatrixPro.GetScheduleDayByKey(_dateOnly.AddDays(1))).Return(_scheduleDayPro);
