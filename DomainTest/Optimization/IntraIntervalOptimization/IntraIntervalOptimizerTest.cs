	...
	public class IntraIntervalOptimizerTest
	{
		...
		public void ShouldReturnIssuesAfterWhenResultGetsBetterOnDayAndNotWorseOnDayAfter()
		{
			...
			using (_mock.Record())
			{
				...
				Expect.Call(() => _deleteAndResourceCalculateService.DeleteWithResourceCalculation(new[] { _scheduleDay }, _rollbackService, true, true));
				...
				Expect.Call(_teamScheduling.ExecutePerDayPerPerson(_person, _dateOnly, _teamBlockInfo, _shiftProjectionCache, _rollbackService,_resourceCalculateDelayer, true, null)).Return(false);
				Expect.Call(_scheduleRange.ScheduledDay(_dateOnly)).Return(_scheduleDay);
