...
namespace Teleopti.Ccc.DomainTest.Optimization.ShiftCategoryFairness
{
	...
	public class ShiftCategoryFairnessSwapperTest
	{
		...
		public void ShouldReturnFalseIfWrongCategory()
		{
			...
			_mocks.ReplayAll();
			_target = new ShiftCategoryFairnessSwapper(_swapService, _resultState, _fairnessReScheduler, _shiftCatChecker, _deleteService, _swappableChecker, _optimizationOverLimitByRestrictionDeciderCreator);
			Assert.That(_target.TrySwap(suggestion, dateOnly, matrixes, rollbackService, new BackgroundWorker(), _optimizationPreferences), Is.False);
			_mocks.VerifyAll();
		}
		...
		public void ShouldCallRescheduleIfMoreMembers()
		{
			...
			_mocks.ReplayAll();
			_target = new ShiftCategoryFairnessSwapper(_swapService, _resultState, _fairnessReScheduler, _shiftCatChecker, _deleteService, _swappableChecker, _optimizationOverLimitByRestrictionDeciderCreator);
			Assert.That(_target.TrySwap(suggestion, dateOnly, matrixes, rollbackService, new BackgroundWorker(), _optimizationPreferences), Is.True);
            _mocks.VerifyAll();
