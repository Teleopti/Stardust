...
namespace Teleopti.Ccc.DomainTest.Collection
{
    ...
    public class SkillStaffPeriodDictionaryTest
    {
        ...
        public void Setup()
        {
			...
			_ssp1 = new SkillStaffPeriod(_dtp1, new Task(), new ServiceAgreement(), new StaffingCalculatorServiceFacade());
			_ssp2 = new SkillStaffPeriod(_dtp2, new Task(), new ServiceAgreement(), new StaffingCalculatorServiceFacade());
            _target.Add(_ssp1);
            _target.Add(_ssp2);
        ...
        public void VerifyOpenHoursCollection()
        {
			...
			ISkillStaffPeriod ssp = new SkillStaffPeriod(dtp, new Task(), new ServiceAgreement(), new StaffingCalculatorServiceFacade());
            Assert.AreEqual(2, _target.SkillOpenHoursCollection.Count);
            _target.Add(ssp);
            Assert.AreEqual(2, _target.SkillOpenHoursCollection.Count);
            dtp = new DateTimePeriod(1998, 1, 1, 1999, 1, 1);
			ssp = new SkillStaffPeriod(dtp, new Task(), new ServiceAgreement(), new StaffingCalculatorServiceFacade());
            _target.Add(ssp);
