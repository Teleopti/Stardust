...
namespace Teleopti.Ccc.DomainTest.AgentInfo.Requests
{
    ...
    public class ShiftTradeRequestSetChecksumTest
    {
        ...
        {
			Expect.Call(_scheduleRepository.FindSchedulesOnlyForGivenPeriodAndPersons(new[] { _person2, _person1 }, new ScheduleDictionaryLoadOptions(true, true), new DateOnlyPeriod(new DateOnly(_personRequest2.Request.Period.StartDateTime.AddDays(-1)), new DateOnly(_personRequest2.Request.Period.EndDateTime.AddDays(1))), _scenario)).Return(_scheduleDictionary).IgnoreArguments();
