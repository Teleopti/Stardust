...
namespace Teleopti.Ccc.DomainTest.ResourceCalculation
{
    ...
    public class ScheduleServiceTest
    {
        ...
        public void Setup()
        {
			...
			_target = new ScheduleService(() =>MockRepository.GenerateMock<ISchedulerStateHolder>(), _workShiftFinder, _matrixListFactory, _shiftCatLimitChecker, _restrictionCreator);
        ...
        public void ShouldReturnFalseIfMatrixCountIsZero()
        {
			...
			Expect.Call(_matrixListFactory.CreateMatrixListForSelection(null, new List<IScheduleDay> { _part })).Return(new List<IScheduleMatrixPro>());
            _mocks.ReplayAll();
			Assert.That(_target.SchedulePersonOnDay(_part, _options, _effectiveRestriction, _resourceCalculateDelayer, _rollbackService), Is.False);
            _mocks.VerifyAll();
        }
        ...
        public void ShouldReturnFalseIfNotFindShift()
        {
            ...
            Expect.Call(
                () => _shiftCatLimitChecker.SetBlockedShiftCategories(_options, _person, (new DateOnly(2011, 4, 18))));
			Expect.Call(_matrixListFactory.CreateMatrixListForSelection(null, new List<IScheduleDay> { _part })).Return(new List<IScheduleMatrixPro> { _scheduleMatrixPro });
        ...
        public void ShouldResetSchedulingOptionsShiftCategoryIfUsingSpecificCategory()
        {
        	...
        	_resourceCalculateDelayer.CalculateIfNeeded(new DateOnly(2011, 04, 18), period, false);
