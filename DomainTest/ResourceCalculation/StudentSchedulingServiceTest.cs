...
#pragma warning restore 612,618
		...
		public void ShouldUserCancel()
		{
			...
			using (_mocks.Record())
			{
				Expect.Call(_scheduleService.SchedulePersonOnDay(null, _schedulingOptions, _effectiveRestriction, _resourceCalculateDelayer, null, _rollbackService)).IgnoreArguments().Return(true).Repeat.AtLeastOnce();
				...
				Expect.Call(personSkill.Skill).Return(_skill1).Repeat.AtLeastOnce();
				Expect.Call(personSkill2.Skill).Return(_skill2).Repeat.AtLeastOnce();
				Expect.Call(_effectiveRestrictionCreator.GetEffectiveRestriction(part1, preferences.SchedulingOptions)).Return(_effectiveRestriction).IgnoreArguments().Repeat.AtLeastOnce();
				Expect.Call(_schedulingResultStateHolder.SkipResourceCalculation).Return(false).Repeat.Any();
				Expect.Call(_schedulingResultStateHolder.Skills).Return(new List<ISkill> { _skill1 });
				Expect.Call(_schedulingResultStateHolder.Schedules).Return(schedules);
#pragma warning disable 612,618
...
#pragma warning restore 612,618
			...
			using (_mocks.Playback())
			{
				_studentSchedulingService.DayScheduled += (sender, e) => { e.UserCancel = true; };
				_studentSchedulingService.DoTheScheduling(new List<IScheduleDay> { part2, part1 }, preferences.SchedulingOptions, true, false, _rollbackService);
			}
		}
