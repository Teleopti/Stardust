...
namespace Teleopti.Ccc.DomainTest.ResourceCalculation
{
    ...
    public class WorkShiftCalculatorsManagerTest
    {
        ...
        public void ShouldCallAllCalculatorsWithSkillStaffPeriods()
        {
			...
			var proj1 = _mocks.StrictMock<IEnumerable<IWorkShiftCalculatableLayer>>();
			var proj2 = _mocks.StrictMock<IEnumerable<IWorkShiftCalculatableLayer>>();
			var proj1v = _mocks.StrictMock<IVisualLayerCollection>();
			var proj2v = _mocks.StrictMock<IVisualLayerCollection>();
            ...
            Expect.Call(cache1.WorkShiftCalculatableLayers).Return(proj1).Repeat.Twice();
            Expect.Call(cache2.WorkShiftCalculatableLayers).Return(proj2).Repeat.Twice();
			...
			Expect.Call(cache1.MainShiftProjection).Return(proj1v).Repeat.Twice();
			Expect.Call(cache2.MainShiftProjection).Return(proj2v).Repeat.Twice();
