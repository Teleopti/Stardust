    ...
    public class MissingDaysOffSchedulerTest
    {
        ...
        public void ShouldAddOnAllMatrixDataIfInSteadyState()
        {
            using (_mocks.Record())
            {
				...
				Expect.Call(_scheduleDay.PersonDayOffCollection()).Return(new ReadOnlyCollection<IPersonDayOff>(new List<IPersonDayOff> { _personDayOff })).Repeat.AtLeastOnce();
				Expect.Call(_personDayOff.FunctionPath).Return("functionPath").Repeat.AtLeastOnce();
        ...
        public void ShouldCancelAndReturnFalseIfCanceled()
        {
            ...
            using (_mocks.Record())
            {
                ...
                Expect.Call(() => _scheduleDay.CreateAndAddDayOff(_schedulingOptions.DayOffTemplate));
                Expect.Call(() => _rollbackService.Modify(_scheduleDay));
				Expect.Call(_scheduleDay.PersonDayOffCollection()).Return(new ReadOnlyCollection<IPersonDayOff>(new List<IPersonDayOff> { _personDayOff }));
