...
{
	[TestFixture]
	public class TeamBlockSteadyStateDictionaryCreatorTest
	{
		private TeamBlockSteadyStateDictionaryCreator _target;
		...
		public void Setup()
		{
			...
			var dateOnlyPeriod1 = new DateOnlyPeriod(_dateOnly, _dateOnly.AddDays(30));
			var dateOnlyPeriod2 = new DateOnlyPeriod(_dateOnly.AddDays(1), _dateOnly.AddDays(31));
			...
			_groupPerson1 = new GroupPerson(new List<IPerson> { _person1 }, _dateOnly, "groupPerson1", _guid1);
			_groupPerson2 = new GroupPerson(new List<IPerson> { _person2 }, _dateOnly.AddDays(1), "groupPerson2", _guid2);

			setupStateHolder();

			_scheduleMatrixPro1 = ScheduleMatrixProFactory.Create(dateOnlyPeriod1, _stateHolder, _person1, _person1.VirtualSchedulePeriod(_dateOnly));
			_scheduleMatrixPro2 = ScheduleMatrixProFactory.Create(dateOnlyPeriod2, _stateHolder, _person2, _person2.VirtualSchedulePeriod(_dateOnly.AddDays(1)));
			...
			_target = new TeamBlockSteadyStateDictionaryCreator(_teamSteadyStateRunner, _matrixes, _groupPersonsBuilder, _schedulingOptions);
		...
		public void ShouldCreateDictionary()
		{
			using(_mocks.Record())
			{
				Expect.Call(_groupPersonsBuilder.BuildListOfGroupPersons(_dateOnly, new List<IPerson> {_person1}, true,
				                                                         _schedulingOptions))
				      .Return(new List<IGroupPerson> {_groupPerson1});
				Expect.Call(_groupPersonsBuilder.BuildListOfGroupPersons(_dateOnly.AddDays(1), new List<IPerson> {_person2}, true,
				                                                         _schedulingOptions))
				      .Return(new List<IGroupPerson> {_groupPerson2});
				Expect.Call(_teamSteadyStateRunner.Run(_groupPerson1, _dateOnly)).Return(new KeyValuePair<Guid, bool>(_guid1, true));
				Expect.Call(_teamSteadyStateRunner.Run(_groupPerson2, _dateOnly.AddDays(1))).Return(new KeyValuePair<Guid, bool>(_guid2, true));
			}
