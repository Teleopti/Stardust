...
namespace Teleopti.Ccc.DomainTest.Scheduling.TeamBlock.WorkShiftCalculation
{
	...
	public class WorkShiftSelectorTest
	{
		...
		public void ShouldSelectShiftWithHighestValue()
		{
			...
			var cache1 = new ShiftProjectionCache(workShift1, new DateOnlyAsDateTimePeriod(date, TimeZoneGuard.Instance.CurrentTimeZone()));
			var cache2 = new ShiftProjectionCache(workShift2, new DateOnlyAsDateTimePeriod(date, TimeZoneGuard.Instance.CurrentTimeZone()));
			var cache3 = new ShiftProjectionCache(workShift3, new DateOnlyAsDateTimePeriod(date, TimeZoneGuard.Instance.CurrentTimeZone()));
			...
			var scheduleDictionary = new ScheduleDictionaryForTest(scenario, new ScheduleDateTimePeriod(new DateOnlyPeriodAsDateTimePeriod(selectedPeriod, TimeZoneGuard.Instance.CurrentTimeZone()).Period(), new[] { agent1 }).VisiblePeriod);
			...
			var result = Target.SelectShiftProjectionCache(GroupPersonSkillAggregator, date, caches,
				new List<ISkillDay> {skillDay}, teamBlockInfo, new SchedulingOptions(), TimeZoneGuard.Instance.CurrentTimeZone(), true,
				agent1);
