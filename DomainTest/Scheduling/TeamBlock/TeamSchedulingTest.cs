    ...
    public class TeamSchedulingTest
    {
        ...
        public void ShouldSchedulePerDayWithOffset()
        {
            ...
            using (_mock.Record())
            {
                ...
                someExpectCallForShouldSchedulePerDayWithOffset();
            }

            using (_mock.Playback())
            {
                _target.DayScheduled += targetDayScheduledCanceled;
                _target.ExecutePerDayPerPerson(_baseLineData.Person1, dateOnly, _teamBlockInfo, _shiftProjectionCache, true, _dateOnlyPeriod);
                _target.DayScheduled += targetDayScheduledCanceled;
            }

        }

        private void someExpectCallForShouldSchedulePerDayWithOffset()
        {
            Expect.Call(_shiftProjectionCache.TheMainShift).Return(_mainShift);
            Expect.Call(() => _schedulePartModifyAndRollbackService.Modify(_scheduleDay)).IgnoreArguments();
            Expect.Call(() => _scheduleDay.AddMainShift(_mainShift)).IgnoreArguments();
        ...
        public void ShouldSchedulePerDayWithoutOffset()
        {
            ...
            {
                _target.DayScheduled += targetDayScheduledCanceled;
                _target.ExecutePerDayPerPerson(_baseLineData.Person1, dateOnly, _teamBlockInfo, _shiftProjectionCache, false, _dateOnlyPeriod);
                _target.DayScheduled += targetDayScheduledCanceled;
            }
        ...
        [Test]
        public void ShouldNotSchedulePerDayWithContractDayOffWithSchedulePeriodStartDate()
        {
            DateOnly dateOnly = DateOnly.Today;
            using (_mock.Record())
            {
            ...
            {
                _target.DayScheduled += targetDayScheduledCanceled;
                _target.ExecutePerDayPerPerson(_baseLineData.Person1, dateOnly, new TeamBlockInfo(_teaminfo, new BlockInfo(new DateOnlyPeriod(_startDateOfBlock.AddDays(2), _startDateOfBlock.AddDays(2)))), _shiftProjectionCache, false, _dateOnlyPeriod);
                _target.DayScheduled += targetDayScheduledCanceled;
            }
        ...
        {
            Assert.Throws<ArgumentNullException >(() => _target.ExecutePerDayPerPerson(_baseLineData.Person1, new DateOnly(), null,
                                                              _shiftProjectionCache,
                                                              false, _dateOnlyPeriod));
        }

        [Test]
        public void ShouldReturnIfShiftProjectionCacheNull()
        {
            Assert.Throws<ArgumentNullException>(() => _target.ExecutePerDayPerPerson(_baseLineData.Person1, new DateOnly(), _teamBlockInfo ,
                                                              null,
                                                              false, _dateOnlyPeriod));
        }
    }
