...
namespace Teleopti.Wfm.Administration.IntegrationTest.Core
{
	[WfmAdminTest]
	[AllTogglesOn]
	public class RestorePersonInfoOnDetachTest
	{
		public IPersonInfoPersister PersonInfoPersister;
		public RestorePersonInfoOnDetach Target;
		...
		[Test]
		public void ShouldPutBackPersonInfos()
		{
			...
			{
				var allTenants = LoadAllTenants.Tenants().ToList();
				tenant = allTenants.Single(t => t.Name.Equals("NewFineTenant"));
			...
			using (uow.EnsureUnitOfWorkIsStarted())
			{
				...
				personInfos.Count.Should().Be.EqualTo(14);
			}
				...
				TenantUnitOfWorkManager.Create(tenant.DataSourceConfiguration.ApplicationConnectionString))
			{
				...
				var allInClient = loadAllPersonInfos.PersonInfos();
				allInClient.Count().Should().Be(0);
			}

			using (uow.EnsureUnitOfWorkIsStarted())
			{
				Target.Restore(tenant);
			}
