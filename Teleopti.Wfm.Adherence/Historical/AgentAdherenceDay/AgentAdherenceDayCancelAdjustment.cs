...
{
	public class AgentAdherenceDayCancelAdjustment : IAgentAdherenceDay
	{
		...
		public AgentAdherenceDayCancelAdjustment(
			DateTime now,
		...
		public void Apply(PeriodApprovalAsInAdherenceCanceledEvent @event) =>
		...
		)
		{
			...
			_adjustedToNeutralAdherences = _collectedAdjustedToNeutralPeriods
				.Intersects(new OpenPeriod(_displayPeriod.Value.StartDateTime, _displayPeriod.Value.EndDateTime))
				.ToAdherencePeriods();
		...
		private static IEnumerable<OpenPeriod> buildPeriods(
			HistoricalChangeAdherence adherence,
