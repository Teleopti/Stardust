...
namespace Teleopti.Ccc.AgentPortal.Requests
{
	public partial class PersonAccountView : BaseUserControl, IPersonAccountView
	{
		...
		public PersonAccountView(ITeleoptiOrganizationService service, PersonDto personDto)
			: this()
		{
		...
		private void InitializeGrid()
		{
			...
			gridControlPersonAccounts.CellModels.Add("DescriptionNameCell",
			                                         new TextReadOnlyCellModel(gridControlPersonAccounts.Model));
			...
			_sfGridColumnGridHelper = new SFGridColumnGridHelper<PersonAccountModel>(gridControlPersonAccounts,
			                                                                         new ReadOnlyCollection
				                                                                         <SFGridColumnBase<PersonAccountModel>>(
				                                                                         gridColumns), _presenter.Items);
			gridControlPersonAccounts.ResizeColsBehavior = ((GridResizeCellsBehavior.ResizeSingle |
			                                                 GridResizeCellsBehavior.InsideGrid) |
			                                                GridResizeCellsBehavior.OutlineHeaders);
		...
		private void dateTimePickerAdvSelectedDate_Leave(object sender, EventArgs e)
		{
			setMinMAx();
		}

		private void dateTimePickerAdvSelectedDate_Calendar_Click(object sender, EventArgs e)
		{
			setMinMAx();
		}
		...
		{
			if (dateTimePickerAdvSelectedDate.Value < dateTimePickerAdvSelectedDate.MinValue)
				dateTimePickerAdvSelectedDate.Value = dateTimePickerAdvSelectedDate.MinValue;

			if (dateTimePickerAdvSelectedDate.Value > dateTimePickerAdvSelectedDate.MaxValue)
				dateTimePickerAdvSelectedDate.Value = dateTimePickerAdvSelectedDate.MaxValue;
		}
	}
}
