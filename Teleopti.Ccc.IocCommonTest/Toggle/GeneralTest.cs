...
namespace Teleopti.Ccc.IocCommonTest.Toggle
{
	public class GeneralTest
	{
		...
		public void ShouldUseToggleQuerierIfStartsWithHttp()
		{
			...
			containerBuilder.RegisterModule(new CommonModule(new IocConfiguration(new IocArgs { FeatureToggle = "http://tralala" }, null)));
		...
		{
			var containerBuilder = new ContainerBuilder();
			containerBuilder.RegisterModule(new CommonModule(new IocConfiguration(new IocArgs { FeatureToggle = "https://hejsan" }, null)));
			using (var container = containerBuilder.Build())
			{
				var toggleChecker = container.Resolve<IToggleManager>();
				toggleChecker.Should().Be.OfType<ToggleQuerier>();
		...
		{
			var containerBuilder = new ContainerBuilder();
			containerBuilder.RegisterModule(new CommonModule(new IocConfiguration(new IocArgs { FeatureToggle = "http://something" }, null)));
			using (var container = containerBuilder.Build())
			{
		...
		{
			var containerBuilder = new ContainerBuilder();
			containerBuilder.RegisterModule(new CommonModule(new IocConfiguration(new IocArgs { FeatureToggle = "" }, null)));
			using (var container = containerBuilder.Build())
			{
		...
		{
			var containerBuilder = new ContainerBuilder();
			containerBuilder.RegisterModule(new CommonModule(new IocConfiguration(new IocArgs { FeatureToggle = null }, null)));
			using (var container = containerBuilder.Build())
			{
				var toggleManager = container.Resolve<IToggleManager>();
				toggleManager.IsEnabled(Toggles.TestToggle)
