...
namespace Teleopti.Ccc.IocCommon.Configuration
{
	public class MbCacheLog4NetListener : IEventListener
	{
		...
		public void OnCacheHit(CachedItem cachedItem)
		{
			...
			{
				log.DebugFormat(cacheHitLogMessage, cachedItem.EventInformation.Method?.ToString() ?? nullReplacer, cachedItem.EventInformation.CacheKey);
			}
		}
		...
		public void OnCacheRemoval(CachedItem cachedItem)
		{
			if (log.IsDebugEnabled)
			{
				log.DebugFormat(deleteMessage, cachedItem.EventInformation.Method?.ToString() ?? nullReplacer, cachedItem.EventInformation.CacheKey);
			}
		}
		...
		public void OnCacheMiss(CachedItem cachedItem)
		{
			if (log.IsDebugEnabled)
			{
				log.DebugFormat(putMessage, cachedItem.CachedValue, cachedItem.EventInformation.Method?.ToString() ?? nullReplacer, cachedItem.EventInformation.CacheKey);
			}
		}
