...
declare @ModuleString table(ModuleString varchar(20))

--Handle @scheduling_type_id
IF @scheduling_type_id = 1
BEGIN
	INSERT INTO @ModuleString
	SELECT 'Scheduling'
	UNION ALL
	SELECT 'Optimization'
END
...
SELECT
CASE
	WHEN @scheduling_type_id = 2 THEN 'Scheduling'
	WHEN @scheduling_type_id = 3 THEN 'Optimization'
END
...
where [Message] in (SELECT ModuleString FROM @ModuleString)
GROUP BY
CASE
	WHEN @interval_type <=4 THEN convert(varchar(10), m.LogDate, 120) --day
	WHEN @interval_type = 5 THEN 'Week ' +cast(datepart(week,m.LogDate) as varchar(2))
