...
-- 2008-08-13    DaJo    Added columns: employment_number, windows_logon_name, windows_domain_name, password, email, note,
--                       language_id, language_name, time_zone_id, ui_culture, contract_code, contract_name, parttime_code,
--                       parttime_percentage
...
-- Split the update into 2 parts since when a person period start earlier than earliest date in mart.dim_date,
-- in such scenario the record in mart.dim_person will not be updated. (Bug #47602)
-------------------------

UPDATE mart.dim_person
SET
	valid_to_date                 = s.valid_to_date,
	valid_to_date_id              = ISNULL(validToDate.date_id, @maxdateid),
	...
	valid_to_date_id_maxDate      = CASE WHEN validToDate.date_id =-2 THEN @maxdateid-1 ELSE ISNULL(validToDate.date_id, @maxdateid-1) END,
	valid_to_interval_id_maxDate  = CASE WHEN validToDate.date_id =-2 THEN @maxInterval WHEN validToDate.date_id IS NULL THEN @maxInterval ELSE ISNULL(s.valid_to_interval_id, @maxInterval) END,
	valid_to_date_local           = CASE WHEN s.valid_to_date_local=@eternityDate THEN @maxdate ELSE ISNULL(validToLocalDate.date_date, @maxdate) END,
	valid_to_date_id_local        = CASE WHEN validToLocalDate.date_id=-2 THEN @maxdateid ELSE ISNULL(validToLocalDate.date_id, @maxdateid)  END
FROM
	Stage.stg_person s
LEFT JOIN
	mart.dim_team dt
...
LEFT JOIN
	mart.dim_date validToDate
ON
	CONVERT(smalldatetime, CONVERT(int, (CONVERT(float, s.valid_to_interval_start)))) = validToDate.date_date -- Remove time part from date
LEFT JOIN
	mart.dim_date validToLocalDate
ON
...
SET
	...
	valid_from_date_id            = validFromDate.date_id,
	valid_from_interval_id        = s.valid_from_interval_id,
	valid_from_date_local         = s.valid_from_date_local,
	valid_from_date_id_local      = validFromLocalDate.date_id
FROM
	Stage.stg_person s
INNER JOIN
	mart.dim_date validFromDate
ON
	CONVERT(smalldatetime, CONVERT(int, (CONVERT(float, s.valid_from_date)))) = validFromDate.date_date -- Remove time part from date
INNER JOIN
	mart.dim_date validFromLocalDate
ON
...
WHERE
	NOT EXISTS (SELECT person_id FROM mart.dim_person p WHERE s.person_period_code = p.person_period_code)
