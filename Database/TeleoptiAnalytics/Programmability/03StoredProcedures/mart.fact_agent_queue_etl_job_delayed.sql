IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[mart].[fact_agent_queue_etl_job_delayed]') AND type in (N'P', N'PC'))
DROP PROCEDURE [mart].[fact_agent_queue_etl_job_delayed]
GO

--exec [mart].[fact_agent_queue_etl_job_delayed]
CREATE PROCEDURE [mart].[fact_agent_queue_etl_job_delayed]

AS
SET NOCOUNT ON
DECLARE @stored_procedure nvarchar(300)
SET @stored_procedure=N'mart.etl_fact_agent_queue_load'

--data convert already happened
IF EXISTS (select date_id FROM mart.fact_agent_queue)
RETURN

--fact_agent_queue_old exists
IF (NOT EXISTS (SELECT * FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'mart' AND  TABLE_NAME = 'fact_agent_queue_old'))
RETURN
...
WHERE date_id in (
	SELECT
	min(date_id)-1
	FROM mart.fact_agent_queue_old --previous data
)
