IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[mart].[etl_fact_schedule_preference_update_unlinked_personids]') AND type in (N'P', N'PC'))
DROP PROCEDURE [mart].[etl_fact_schedule_preference_update_unlinked_personids]
GO

-- example: exec [mart].[etl_fact_schedule_preference_update_unlinked_personids] '275,276,548,549'
CREATE PROCEDURE [mart].[etl_fact_schedule_preference_update_unlinked_personids]
...
CREATE TABLE #schedule_preference(date_id int)
...
-- select fact_schedule_preference which need to be updated
INSERT #schedule_preference(date_id)
...
-- update fact_schedule_preference with correct person period id
...
and f.date_id in (SELECT date_id FROM #schedule_preference)

drop table #person, #schedule_preference
