IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[mart].[yearweek]') AND type in (N'FN', N'IF', N'TF', N'FS', N'FT'))
DROP FUNCTION [mart].[yearweek]
GO


CREATE FUNCTION [mart].[yearweek] (@start_week int, @DATE datetime)
	RETURNS varchar(6)
	AS
	BEGIN
	/*
	Created by Anders Forsberg Teleopti AB
	
	Returns a string YYYYWW containing year and week.
	Example: 20050101 will return:
	200453 (if first four day week is used) or
	200501 (if 1st jan is used)
