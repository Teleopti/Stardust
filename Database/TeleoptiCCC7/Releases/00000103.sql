...
--Date: 2009-05-18  
--Desc: Should not only rely on CreatedBy 
--Changed name to Reply instead of message
----------------  
alter table dbo.Conversation add Sender UNIQUEIDENTIFIER
alter table dbo.Conversation add constraint FK_Conversation_Person_Sender foreign key (Sender) references dbo.Person
create table dbo.Reply (Id UNIQUEIDENTIFIER not null, Created DATETIME not null, Text NVARCHAR(255) not null, Parent UNIQUEIDENTIFIER not null, primary key (Id))
drop table dbo.Message

----------------  
--Name: David Jonsson
--Date: 2009-05-20
--Desc: Rename wrong PK name
---------------- 
DECLARE @PKName nvarchar(1000)

SELECT @PKName= '[dbo].[Reply].['+name+']' FROM sys.indexes
WHERE OBJECT_NAME(object_id) = 'Reply'
AND index_id = 1
AND is_primary_key = 1

SELECT @PKName

EXEC sp_rename @PKName, N'PK_Reply', N'INDEX'

----------------  
--Name: David Jonsson
--Date: 2009-05-20
--Desc: Refactor tables names for [ConversationDialogue]
--Note: tables are not used yet by any code, only avaible under develop license
----------------
DROP TABLE [dbo].[ConversationDialogue]
GO
DROP TABLE [dbo].[Conversation]
GO

CREATE TABLE dbo.PushMessageDialogue (Id UNIQUEIDENTIFIER not null, Version INT not null, CreatedBy UNIQUEIDENTIFIER not null, UpdatedBy UNIQUEIDENTIFIER null, CreatedOn DATETIME not null, UpdatedOn DATETIME null, Receiver UNIQUEIDENTIFIER not null, PushMessage UNIQUEIDENTIFIER not null, BusinessUnit UNIQUEIDENTIFIER not null, IsDeleted BIT not null)
ALTER TABLE dbo.PushMessageDialogue ADD CONSTRAINT
	PK_PushMessageDialogue PRIMARY KEY CLUSTERED 
	(
	Id
	)
GO

CREATE TABLE dbo.PushMessage (Id UNIQUEIDENTIFIER not null, Version INT not null, CreatedBy UNIQUEIDENTIFIER not null, UpdatedBy UNIQUEIDENTIFIER null, CreatedOn DATETIME not null, UpdatedOn DATETIME null, Title NVARCHAR(255) not null, Message NVARCHAR(255) not null, Sender UNIQUEIDENTIFIER null, BusinessUnit UNIQUEIDENTIFIER not null, IsDeleted BIT not null)
ALTER TABLE dbo.PushMessage ADD CONSTRAINT
	PK_PushMessage PRIMARY KEY CLUSTERED 
	(
	Id
	)
GO

ALTER TABLE dbo.Reply add constraint FK_PushMessageDialogue_Reply foreign key (Parent) references PushMessageDialogue
ALTER TABLE dbo.PushMessageDialogue add constraint FK_PushMessageDialogue_Person_CreatedBy foreign key (CreatedBy) references Person
ALTER TABLE dbo.PushMessageDialogue add constraint FK_PushMessageDialogue_Person_UpdatedBy foreign key (UpdatedBy) references Person
ALTER TABLE dbo.PushMessageDialogue add constraint FK_PushMessageDialogue_Receiver foreign key (Receiver) references Person
ALTER TABLE dbo.PushMessageDialogue add constraint FK_PushMessageDialogue_PushMessage foreign key (PushMessage) references PushMessage
ALTER TABLE dbo.PushMessageDialogue add constraint FK_PushMessageDialogue_BusinessUnit foreign key (BusinessUnit) references BusinessUnit
ALTER TABLE dbo.PushMessage add constraint FK_PushMessage_Person_CreatedBy foreign key (CreatedBy) references Person
ALTER TABLE dbo.PushMessage add constraint FK_PushMessage_Person_UpdatedBy foreign key (UpdatedBy) references Person
ALTER TABLE dbo.PushMessage add constraint FK_PushMessage_Person_Sender foreign key (Sender) references Person
ALTER TABLE dbo.PushMessage add constraint FK_PushMessage_BusinessUnit foreign key (BusinessUnit) references BusinessUnit

----------------  
--Name: Henrik Andersson
--Date: 2009-05-25
--Desc: Drop IsDeleted on PushMessage/Dialogue (Changed to RemovableAggregateRoot)
--Note: tables are not used yet by any code, only avaible under develop license
----------------

ALTER TABLE dbo.PushMessageDialogue
...
GO
CREATE TABLE dbo.DialogueMessage (Id UNIQUEIDENTIFIER not null, Created DATETIME not null, Text NVARCHAR(255) not null, Parent UNIQUEIDENTIFIER not null)
ALTER TABLE dbo.DialogueMessage ADD CONSTRAINT
	PK_DialogueMessage PRIMARY KEY CLUSTERED 
	(
	Id
	)
GO
alter table dbo.DialogueMessage add constraint FK_PushMessageDialogue_DialogueReply foreign key (Parent) references PushMessageDialogue
GO

----------------  
--Name: Henrik Andersson
--Date: 2009-05-18  
--Desc: Added sender on  DialogueMessage
----------------
alter table dbo.DialogueMessage add Sender UNIQUEIDENTIFIER
GO
alter table dbo.DialogueMessage add constraint FK_DialogueMessage_Person_Sender foreign key (Sender) references Person
GO

----------------  
--Name: Henrik Andersson
--Date: 2009-05-18  
--Desc: Added options on  DialogueMessage
----------------

create table dbo.ReplyOptions (id UNIQUEIDENTIFIER not null, elt NVARCHAR(255) null)
GO
alter table dbo.ReplyOptions add constraint FK_PushMessage_ReplyOption foreign key (id) references PushMessage
GO
alter table dbo.PushMessageDialogue add IsReplied BIT
GO
alter table dbo.PushMessageDialogue add Reply NVARCHAR(255)
GO
