...
----------------  
--Name: Henry Greijer, ÷sten Petersson
--Date: 2009-03-02  
--Desc: Payroll Export stuff  
----------------  
create table dbo.PayrollExport (Id UNIQUEIDENTIFIER not null, Version INT not null, CreatedBy UNIQUEIDENTIFIER not null, UpdatedBy UNIQUEIDENTIFIER null, CreatedOn DATETIME not null, UpdatedOn DATETIME null, FileFormat INT not null, PayrollFormatName NVARCHAR(50) not null, PayrollFormatId UNIQUEIDENTIFIER not null, Minimum DATETIME not null, Maximum DATETIME not null, BusinessUnit UNIQUEIDENTIFIER not null, IsDeleted BIT not null, primary key (Id))
create table dbo.PersonsInPayrollExport (PersonId UNIQUEIDENTIFIER not null, Person UNIQUEIDENTIFIER null, collection_id UNIQUEIDENTIFIER not null, primary key (collection_id))
alter table dbo.PayrollExport add constraint FK_PayrollExport_Person_CreatedBy foreign key (CreatedBy) references Person
alter table dbo.PayrollExport add constraint FK_PayrollExport_Person_UpdatedBy foreign key (UpdatedBy) references Person
alter table dbo.PayrollExport add constraint FK_PayrollExport_BusinessUnit foreign key (BusinessUnit) references BusinessUnit
alter table dbo.PersonsInPayrollExport add constraint FK_PersonsInPayrollExport_Person foreign key (Person) references Person
alter table dbo.PersonsInPayrollExport add constraint FK_PersonsInPayrollExport_PersonID foreign key (PersonId) references PayrollExport

----------------  
--Name: Ola HÂkansson  
--Date: 2009-03-03  
--Desc: Locking of the schedule  
---------------- 
ALTER TABLE dbo.Team ADD
	WriteProtection int NOT NULL CONSTRAINT DF_Team_DaysLockedAfter DEFAULT 28
	

----------------  
--Name: Roger Kratz
--Date: 2009-03-03  
--Desc: Write protection for person
---------------- 	
alter table dbo.Person add WriteProtectionUpdatedOn DATETIME
alter table dbo.Person add PersonWriteProtectedDate DATETIME
alter table dbo.Person add WriteProtectionUpdatedBy UNIQUEIDENTIFIER
alter table dbo.Person add constraint FK_Person_WriteProtection_UpdatedBy foreign key (WriteProtectionUpdatedBy) references Person


----------------  
--Name: ÷sten Petersson 
--Date: 2009-03-03  
--Desc: added support for payroll system unique payrollcode on absence  
----------------  
alter table dbo.Absence add PayrollCode NVARCHAR(20) 
GO 
...
INSERT INTO dbo.DatabaseVersion(BuildNumber, SystemVersion) VALUES (75,'7.0.75') 
