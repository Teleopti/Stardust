...
--Name: Jonas Nordh
--Date: 2009-10-20 
--Desc: Add property to Scenario
----------------
ALTER TABLE dbo.Scenario ADD
	EnableReporting bit NOT NULL CONSTRAINT DF_Scenario_EnableReporting DEFAULT 0
GO
...
--Name: Ola HÂkansson
--Date: 2009-10-21 
--Desc: changes to Student Availability
----------------

/****** Object:  Table [dbo].[StudentAvailabilityDay]    Script Date: 10/20/2009 12:16:00 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  Table [dbo].[StudentAvailabilityRestriction]    Script Date: 10/20/2009 12:16:22 ******/
SET ANSI_NULLS ON
GO
...
ALTER TABLE [dbo].[StudentAvailabilityRestriction]  WITH CHECK ADD  CONSTRAINT [FK_StudentAvailabilityRestriction_RestrictionBase] FOREIGN KEY([RestrictionBase])
REFERENCES [dbo].[RestrictionBase] ([Id])
GO

ALTER TABLE [dbo].[StudentAvailabilityRestriction] CHECK CONSTRAINT [FK_StudentAvailabilityRestriction_RestrictionBase]
GO

-- insert dbo.StudentAvailabilityDay
INSERT INTO StudentAvailabilityDay
SELECT   
MAX(Cast(pr.Id as char(36))), MAX(Version), MAX(Cast(CreatedBy as char(36))), MAX(Cast(UpdatedBy as char(36))), 
MIN(CreatedOn), MAX(UpdatedOn), Person, 0,RestrictionDate, BusinessUnit
  FROM AvailabilityRestrictionOld ar
  INNER JOIN ScheduleRestriction r
  ON r.Id = ar.ScheduleRestriction
  INNER JOIN PersonRestriction pr
  ON pr.Id = r.Parent
  GROUP BY Person, 
	RestrictionDate, BusinessUnit
	
  --  insert dbo.RestrictionBase
  INSERT INTO RestrictionBase
  SELECT  r.Id, StartTimeMinimum, StartTimeMaximum, EndTimeMinimum, EndTimeMaximum, WorkTimeMinimum, WorkTimeMaximum
  FROM AvailabilityRestrictionOld ar
  INNER JOIN ScheduleRestriction r
  ON r.Id = ar.ScheduleRestriction
  INNER JOIN PersonRestriction pr
  ...
  -- insert dbo.StudentAvailabilityRestriction in correct order
  
  INSERT INTO StudentAvailabilityRestriction
  SELECT  r.Id, sd.Id, 0 
  FROM AvailabilityRestrictionOld ar
  INNER JOIN ScheduleRestriction r
  ON r.Id = ar.ScheduleRestriction
  INNER JOIN PersonRestriction pr
  ON pr.Id = r.Parent
  INNER JOIN StudentAvailabilityDay sd
  ON sd.Person = pr.Person and sd.RestrictionDate = pr.RestrictionDate
  ORDER BY sd.Person,sd.RestrictionDate,  StartTimeMinimum, EndTimeMaximum
