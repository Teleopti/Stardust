IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[ReadModel].[PersonFinderWithDataPermission]') AND type in (N'P', N'PC'))
DROP PROCEDURE [ReadModel].[PersonFinderWithDataPermission]
GO
...
AND (af.ForeignId = @perm_foreignId OR af.ForeignId = '0000')
