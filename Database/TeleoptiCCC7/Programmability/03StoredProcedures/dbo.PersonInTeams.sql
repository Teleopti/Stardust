...
WHERE ISNULL(p.TerminalDate, '2100-01-01') >= @start_date 
AND p.IsDeleted = 0
AND (pp.StartDate IS NULL OR pp.StartDate <= @start_date) 
AND pp.StartDate <= @end_date AND (pp.EndDate IS NULL OR EndDate >= @start_date)
