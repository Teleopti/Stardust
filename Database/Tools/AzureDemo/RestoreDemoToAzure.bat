...
IF "%1"=="" (
SET /P DESTUSER=Provide a new SQL Login for the application:
SET /P DESTPWD=Provide a new SQL pwd for the application:
) ELSE (
...
SQLCMD -Stcp:%DESTSERVER% -U%DESTSERVERADMINUSER% -P%AZUREADMINPWD% -b -d%DESTCCC% -Q"DROP VIEW [dbo].[v_PersonOrganizationData]"
...
SQLCMD -Stcp:%DESTSERVER% -U%DESTSERVERADMINUSER% -P%AZUREADMINPWD% -b -d%DESTCCC% -i"%ROOTDIR_DBMANAGER%\TeleoptiCCC7\Programmability\01Views\dbo.v_PersonOrganizationData.sql"
...
SQLCMD -Stcp:%DESTSERVER% -U%DESTSERVERADMINUSER% -P%AZUREADMINPWD% -b -d%DESTCCC% -i"%ROOTDIR_LOCAL%\DropExistingUsersAddContainedUser.sql" -v DESTANALYTICS = "%DESTANALYTICS%" DESTUSER = "%DESTUSER%" DESTPWD = "%DESTPWD%"
IF %ERRORLEVEL% NEQ 0 SET /A ERRORLEV=25 & GOTO :error
...
SQLCMD -Stcp:%DESTSERVER% -U%DESTSERVERADMINUSER% -P%AZUREADMINPWD% -b -d%DESTANALYTICS% -i"%ROOTDIR_LOCAL%\DropExistingUsersAddContainedUser.sql" -v DESTANALYTICS = "%DESTANALYTICS%" DESTUSER = "%DESTUSER%" DESTPWD = "%DESTPWD%"
IF %ERRORLEVEL% NEQ 0 SET /A ERRORLEV=32 & GOTO :error
