...
namespace DotNetOpenAuth.OpenId.Provider
{
	...
	public sealed class OpenIdProvider : IDisposable, IOpenIdHost {
		...
		/// </summary>
		SecuritySettings IOpenIdHost.SecuritySettings => this.SecuritySettings;

		/// <summary>
		/// Gets the extension factories.
		/// </summary>
		public IList<IOpenIdExtensionFactory> ExtensionFactories => this.Channel.GetExtensionFactories();
		...
		/// </remarks>
		public ICollection<IProviderBehavior> Behaviors => this.behaviors;
		...
		/// </summary>
		IDirectWebRequestHandler IOpenIdHost.WebRequestHandler => this.Channel.WebRequestHandler;
		...
		/// Gets the channel.
		/// </summary>
		internal OpenIdChannel OpenIdChannel => (OpenIdChannel)this.Channel;

		/// <summary>
		/// Gets the list of services that can perform discovery on identifiers given.
		/// </summary>
		internal IList<IIdentifierDiscoveryService> DiscoveryServices => this.discoveryServices.DiscoveryServices;

		/// <summary>
		/// Gets the web request handler to use for discovery and the part of
		/// authentication where direct messages are sent to an untrusted remote party.
		/// </summary>
		internal IDirectWebRequestHandler WebRequestHandler => this.Channel.WebRequestHandler;
		...
		private class SwitchingAssociationStore : IProviderAssociationStore {
			...
			/// </summary>
			internal IProviderAssociationStore AssociationStore => this.securitySettings.EncodeAssociationSecretsInHandles ? this.associationHandleEncoder : this.associationSecretStorage;
