...
namespace ManagerTest
{
    ...
    public class ManagerOperationsTest : DatabaseTest
    {
        ...
        private readonly Uri _nodeUri = new Uri("http://localhost:9050/");
        ...
        {
            Target.NodeInitialized(_nodeUri);
            NodeRepository.LoadAll()
                .First()
                .Url.Should()
                .Be.EqualTo(_nodeUri.ToString());
        }
        ...
        public void ShouldBeAbleToSendNewJobToAvailableNode()
        {
            ...
            Target.NodeInitialized(_nodeUri);
            Target.DoThisJob(job);
            Target.Heartbeat(_nodeUri);
            HttpSender.CalledNodes.Keys.First()
                .Should()
                .Contain(_nodeUri.ToString());
        }
        ...
        {
            var job = new JobRequestModel() {Name = "ShouldBeAbleToSendNewJobToAvailableNode", Serialized = "ngt", Type = "bra", UserName = "ManagerTests"};
            thisNodeIsBusy(_nodeUri.ToString());
        ...
        public void ShouldNotRemoveARunningJobFromRepo()
        {
            ...
            var job = new JobDefinition() {Name = " ", Serialized = " ", Type = " ", UserName = "ManagerTests", Id = jobId};
            JobRepository.Add(job);
            JobRepository.CheckAndAssignNextJob(new List<WorkerNode>() {new WorkerNode() {Url = _nodeUri.ToString() } },
                                                HttpSender);
            thisNodeIsBusy(_nodeUri.ToString());
            Target.CancelThisJob(jobId);
            JobRepository.LoadAll()
        ...
        public void ShouldBeAbleToCancelJobOnNode()
        {
            ...
            JobRepository.CheckAndAssignNextJob(new List<WorkerNode>() {new WorkerNode() {Url = _nodeUri.ToString() }, new WorkerNode() {Url = "http://localhost:9051/" } },
                                                HttpSender);
        ...
        {
            Guid jobId = Guid.NewGuid();
            var job = new JobDefinition() {Id = jobId, AssignedNode = _nodeUri.ToString(), Name = "job", Serialized = "", Type = "", UserName = "ShouldRemoveTheJobWhenItsFinished"};
            JobRepository.Add(job);
        ...
        {
            Guid jobId = Guid.NewGuid();
            var job = new JobDefinition() {Id = jobId, AssignedNode = _nodeUri.ToString(), Name = "job", Serialized = "", Type = "", UserName = "ShouldSendOkWhenJobDoneSignalReceived"};
            JobRepository.Add(job);
