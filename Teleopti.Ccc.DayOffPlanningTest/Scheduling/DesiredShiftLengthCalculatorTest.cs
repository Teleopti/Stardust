...
namespace Teleopti.Ccc.DayOffPlanningTest.Scheduling
{
	...
	public class DesiredShiftLengthCalculatorTest
	{
		...
		public void Setup()
		{
			_target = new DesiredShiftLengthCalculator();
		...
		public void ShouldReturnAverageWorkTimeWhenAllDaysAreSameAndIncludesCurrentAverage()
		{
			...
			using(_mocks.Record())
			{
				Expect.Call(_workShiftMinMaxCalculator.PossibleMinMaxWorkShiftLengths(_matrix)).Return(minMaxDic);
				...
				Expect.Call(_schedulePeriod.PeriodTarget()).Return(TimeSpan.FromHours(16));
			}
			...
			{
				result = _target.FindAverageLength(_workShiftMinMaxCalculator, _matrix);
			}
