...
{
    [TestFixture]
    public class GridlockAllPreferencesFulfilledAbsenceCommandTest
    {
        private GridlockAllPreferencesFulfilledAbsenceCommand _target;
        ...
        public void Setup()
        {
            ...
            _target = new GridlockAllPreferencesFulfilledAbsenceCommand(_gridScheduleExtractor, _restrictionChecker, _scheduleDayPreferenceRestrictionExtractor, _gridlockManager);
        }

        [Test]
        public void ShouldLockFulfilledAbsencePreferences()
        {
            using (_mock.Record())
            {
                Expect.Call(_gridScheduleExtractor.Extract()).Return(_scheduleDays);
				Expect.Call(_scheduleDayPreferenceRestrictionExtractor.RestrictionFulfilledAbsence(_restrictionChecker, _day1)).Return(_day1);
                Expect.Call(_scheduleDayPreferenceRestrictionExtractor.RestrictionFulfilledAbsence(_restrictionChecker, _day2)).Return(null);
