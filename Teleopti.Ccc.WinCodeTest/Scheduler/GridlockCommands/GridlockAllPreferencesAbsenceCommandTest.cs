...
{
    [TestFixture]
    public class GridlockAllPreferencesAbsenceCommandTest
    {
        private GridlockAllPreferencesAbsenceCommand _target;
        ...
        public void Setup()
        {
            ...
            _target = new GridlockAllPreferencesAbsenceCommand(_gridScheduleExtractor, _scheduleDayPreferenceRestrictionExtractor, _gridlockManager);
        }

        [Test]
        public void ShouldLocAbsencePreferences()
        {
            using (_mock.Record())
            {
                Expect.Call(_gridScheduleExtractor.Extract()).Return(_scheduleDays);
                Expect.Call(_scheduleDayPreferenceRestrictionExtractor.AllRestrictedAbsences(_scheduleDays)).Return(_scheduleDays);
