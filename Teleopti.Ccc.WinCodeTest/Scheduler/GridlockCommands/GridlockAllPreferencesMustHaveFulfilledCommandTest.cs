...
{
    [TestFixture]
    public class GridlockAllPreferencesMustHaveFulfilledCommandTest
    {
        private GridlockAllPreferencesMustHaveFulfilledCommand _target;
        ...
        public void Setup()
        {
            ...
            _target = new GridlockAllPreferencesMustHaveFulfilledCommand(_gridScheduleExtractor, _restrictionChecker, _scheduleDayPreferenceRestrictionExtractor, _gridlockManager);
        }

        [Test]
        public void ShouldLockFulfilledPreferences()
        {
            using (_mock.Record())
            {
                Expect.Call(_gridScheduleExtractor.Extract()).Return(_scheduleDays);
                Expect.Call(_scheduleDayPreferenceRestrictionExtractor.RestrictionFulfilledMustHave(_restrictionChecker, _day1)).Return(_day1);
                Expect.Call(_scheduleDayPreferenceRestrictionExtractor.RestrictionFulfilledMustHave(_restrictionChecker, _day2)).Return(null);
