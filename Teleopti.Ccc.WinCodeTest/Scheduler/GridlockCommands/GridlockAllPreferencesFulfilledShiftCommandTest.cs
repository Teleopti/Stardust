...
{
    [TestFixture]
    public class GridlockAllPreferencesFulfilledShiftCommandTest
    {
        private GridlockAllPreferencesFulfilledShiftCommand _target;
        ...
        public void Setup()
        {
            ...
            _target = new GridlockAllPreferencesFulfilledShiftCommand(_gridScheduleExtractor, _restrictionChecker, _scheduleDayPreferenceRestrictionExtractor, _gridlockManager);
        }

        [Test]
        public void ShouldLockFulfilledShiftPreferences()
        {
            using (_mock.Record())
            {
                Expect.Call(_gridScheduleExtractor.Extract()).Return(_scheduleDays);
                Expect.Call(_scheduleDayPreferenceRestrictionExtractor.RestrictionFulfilledShift(_restrictionChecker, _day1)).Return(_day1);
                Expect.Call(_scheduleDayPreferenceRestrictionExtractor.RestrictionFulfilledShift(_restrictionChecker, _day2)).Return(null);
