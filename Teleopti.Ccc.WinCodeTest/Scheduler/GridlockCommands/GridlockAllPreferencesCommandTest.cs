...
{
    [TestFixture]
    public class GridlockAllPreferencesCommandTest
    {
        private GridlockAllPreferencesCommand _target;
        ...
        public void Setup()
        {
            ...
            _target = new GridlockAllPreferencesCommand(_gridScheduleExtractor, _scheduleDayPreferenceRestrictionExtractor, _gridlockManager);
        }

        [Test]
        public void ShouldLockAllPreferences()
        {
            using(_mock.Record())
            {
                Expect.Call(_gridScheduleExtractor.Extract()).Return(_scheduleDays);
                Expect.Call(_scheduleDayPreferenceRestrictionExtractor.AllRestrictedDays(_scheduleDays)).Return(_scheduleDays);
