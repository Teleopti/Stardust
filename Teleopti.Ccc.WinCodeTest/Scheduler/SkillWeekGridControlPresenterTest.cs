	...
	public class SkillWeekGridControlPresenterTest
	{
		...
		public void ShouldCreateDataSourceOnVirtualSkill()
		{
			...
			using (_mocks.Record())
			{
				...
				Expect.Call(_stateHolder.TimeZoneInfo).Return(StateHolderReader.Instance.StateReader.UserTimeZone).Repeat.AtLeastOnce();
				...
				Expect.Call(_skillStaffPeriodHolder.SkillStaffPeriodList(_skill, dateOnlyPeriod.ToDateTimePeriod(StateHolderReader.Instance.StateReader.UserTimeZone))).IgnoreArguments().Return(new List<ISkillStaffPeriod>());
			}
		...
		public void ShouldCreateDataSourceOnNonVirtualSkill()
		{
			...
			using (_mocks.Record())
			{
				...
				Expect.Call(_stateHolder.SchedulingResultState).Return(_schedulingResultStateHolder);
				Expect.Call(_schedulingResultStateHolder.SkillStaffPeriodHolder).Return(_skillStaffPeriodHolder);
				Expect.Call(_skillStaffPeriodHolder.SkillStaffPeriodList(new List<ISkill> { _skill }, dateOnlyPeriod.ToDateTimePeriod(StateHolderReader.Instance.StateReader.UserTimeZone))).IgnoreArguments().Return(new List<ISkillStaffPeriod>());
			}
