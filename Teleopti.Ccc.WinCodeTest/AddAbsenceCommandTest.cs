...
namespace Teleopti.Ccc.WinCodeTest
{
    ...
    public class AddAbsenceCommandTest
    {
        ...
        public void Setup()
        {
            ...
            _schedulerStateHolder = new SchedulerStateHolder(_scenario, _period, new List<IPerson>());
		...
		private IScheduleDictionary CreateExpectationForModifyScheduleParts(IEnumerable<IScheduleDay> scheduleParts, IPerson person)
		{
			...
			{
				Expect.Call(schedulePart.DateOnlyAsPeriod).Return(new DateOnlyAsDateTimePeriod(new DateOnly(startDate), new CccTimeZoneInfo(TimeZoneInfo.Utc))).Repeat.AtLeastOnce();
				Expect.Call(schedulePart.Person).Return(person).Repeat.AtLeastOnce();
