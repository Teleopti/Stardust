...
#pragma warning restore 618
			_schedulerMeetingHelper = new SchedulerMeetingHelper(_schedulerMessageBrokerHandler, _schedulerState, _container.Resolve<IResourceOptimization>(), _container.Resolve<ISkillPriorityProvider>(), _container.Resolve<IScheduleStorageFactory>());
		...
		private void _backgroundWorkerOvertimeScheduling_ProgressChanged(object sender, ProgressChangedEventArgs e)
		{
			...
			if (InvokeRequired)
				BeginInvoke(new EventHandler<ProgressChangedEventArgs>(_backgroundWorkerOvertimeScheduling_ProgressChanged), sender,
					e);
			else
			{
				var progress = e.UserState as SchedulingServiceBaseEventArgs;
			...
			Action<ILoaderDeciderResult> setDeciderResult, Func<ILoaderDeciderResult> getDeciderResult)
		{
			...
			using (PerformanceOutput.ForOperation("Loading schedules " + period.LoadedPeriod()))
			{
				...
				stateHolder.LoadSchedules(new ScheduleStorage(new ThisUnitOfWork(uow), new RepositoryFactory(), new PersistableScheduleDataPermissionChecker(), _container.Resolve<IToggleManager>()), personsInOrganizationProvider, scheduleDictionaryLoadOptions, period);
		...
		private void menuItemClick(object sender, EventArgs e)
		{
			...
			var toggleManager = _container.Resolve<IToggleManager>();
			var scheduleRepository = new ScheduleStorage(new FromFactory(() => uowFactory), new RepositoryFactory(), new PersistableScheduleDataPermissionChecker(), toggleManager);
		...
		private void displayTimeZoneInfo()
		{
			...
			toolStripStatusLabelTimeZone.Text = string.Concat(LanguageResourceHelper.Translate("XXViewPointTimeZone"),
				Resources.Colon,
