...
namespace Teleopti.Ccc.Win.Scheduling
{
	...
	public partial class SchedulingScreen : BaseRibbonForm
	{
		...
		private readonly ICollection<IPersonWriteProtectionInfo> _modifiedWriteProtections = new HashSet<IPersonWriteProtectionInfo>(); //shouldn't be here, but GridHelper is static... I hate those static classes
		...
		private class LoaderMethod
		{
			...
			public LoaderMethod(Action<IUnitOfWork, ISchedulerStateHolder, IPeopleAndSkillLoaderDecider> action, string statusStripString)
			{
				_action = action;
        ...
        protected SchedulingScreen()
		{
			...
			notesEditor = new NotesEditor(TeleoptiPrincipal.Current.PrincipalAuthorization.IsPermitted(DefinedRaptorApplicationFunctionPaths.ModifyPersonAssignment));
        ...
        void _tmpTimer_Tick(object sender, EventArgs e)
        {
            _tmpTimer.Enabled = false;
            _grid.Focus();
        }

		void dateNavigateControlSelectedDateChanged(object sender, CustomEventArgs<DateOnly> e)
		{
		...
		void _schedulerMeetingHelper_ModificationOccured(object sender, Meetings.ModifyMeetingEventArgs e)
		{
		...
		void contextMenuViews_Opened(object sender, EventArgs e)
		{
			if (_scheduleView != null)
		...
		void _editControl_DeleteSpecialClicked(object sender, ToolStripItemClickedEventArgs e)
		{
		...
		void _editControl_DeleteClicked(object sender, EventArgs e)
		{
			deleteSwitch();
		}
		...
		void _editControl_NewSpecialClicked(object sender, ToolStripItemClickedEventArgs e)
		{
		...
		void _editControl_NewClicked(object sender, EventArgs e)
		{
			if (TeleoptiPrincipal.Current.PrincipalAuthorization.IsPermitted(DefinedRaptorApplicationFunctionPaths.ModifyPersonAssignment))
		...
		private void toggleCalculation()
		{
			...
			{
				foreach (var date in _schedulerState.RequestedPeriod.ToDateOnlyPeriod(TeleoptiPrincipal.Current.Regional.TimeZone).DayCollection())
				{
		...
		private void instantiateClipboardControl()
		{
			...
			_clipboardControl.CutSpecialItems.Add(new ToolStripButton { Text = Resources.CutPersonalShift, Tag = ClipboardItems.PersonalShift });
			...
			_clipboardControl.PasteSpecialItems.Add(new ToolStripButton { Text = Resources.PastePersonalShift, Tag = ClipboardItems.PersonalShift });
			_clipboardControl.PasteSpecialItems.Add(new ToolStripButton { Text = Resources.PasteSpecial, Tag = ClipboardItems.Special });
			_clipboardControl.PasteSpecialItems.Add(new ToolStripButton { Text = Resources.PasteShiftFromShifts, Tag = ClipboardItems.ShiftFromShifts });
		...
		private void setPermissionOnClipboardControl()
		{
			if (_clipboardControl != null)
			{
				...
				_clipboardControl.SetButtonDropDownItemState(ClipboardAction.Cut, ClipboardItems.DayOff.ToString(),
					authorization.IsPermitted(DefinedRaptorApplicationFunctionPaths.ModifyPersonDayOff));
				_clipboardControl.SetButtonDropDownItemState(ClipboardAction.Cut, ClipboardItems.PersonalShift.ToString(),
					authorization.IsPermitted(DefinedRaptorApplicationFunctionPaths.ModifyPersonAssignment));
				...
				_clipboardControl.SetButtonDropDownItemState(ClipboardAction.Paste, ClipboardItems.DayOff.ToString(),
					authorization.IsPermitted(DefinedRaptorApplicationFunctionPaths.ModifyPersonDayOff));
				_clipboardControl.SetButtonDropDownItemState(ClipboardAction.Paste, ClipboardItems.PersonalShift.ToString(),
					authorization.IsPermitted(DefinedRaptorApplicationFunctionPaths.ModifyPersonAssignment));
		...
		void _clipboardControl_PasteSpecialClicked(object sender, ToolStripItemClickedEventArgs e)
		{
			switch ((ClipboardItems)e.ClickedItem.Tag)
			{
				case ClipboardItems.Shift:
		...
		void _clipboardControl_CutSpecialClicked(object sender, ToolStripItemClickedEventArgs e)
		{
			switch ((ClipboardItems)e.ClickedItem.Tag)
			{
				case ClipboardItems.Shift:
		...
		void _clipboardControl_CopySpecialClicked(object sender, ToolStripItemClickedEventArgs e)
		{
			switch ((ClipboardItems)e.ClickedItem.Tag)
			{
				case ClipboardItems.Special:
		...
		private void loadQuickAccessState()
		{
			...
			if (readSettingList.Count == 1 && readSettingList[0] == "NotSavedBefore")
			{
				...
				ribbonControlAdv1.Header.AddQuickItem(new QuickSplitButtonReflectable(toolStripSplitButtonQuickAccessUndo));
		...
		void _agentInfo_FormClosed(object sender, FormClosedEventArgs e)
		{
			_agentInfo = null;
		}
		...
		private void toolStripMenuItemSwapAndReschedule_Click(object sender, EventArgs e)
		{
			if (_scheduleView != null)
			{
				...
				_groupPagePerDateHolder.ShiftCategoryFairnessGroupPagePerDate = ScheduleOptimizerHelper.CreateGroupPagePerDate(_scheduleView, _container.Resolve<GroupScheduleGroupPageDataProvider>(),
		...
		private void swapSelectedSchedules()
		{
			...
			if (selectedSchedules.Count > 1)
			{
				...
				try
				{
					...
					lstBusinessRuleResponse = swapAndModifyServiceNew.Swap(personList[0], personList[1], dates, _scheduleView.LockedDates(_gridLockManager), 
						_schedulerState.Schedules, newRules, new ScheduleTagSetter(_defaultScheduleTag));
				}
				catch (ValidationException ex)
				{
					_undoRedo.RollbackBatch();
		...
		void ToolStripMenuItemDayOffLockRmMouseUp(object sender, MouseEventArgs e)
		{
			if (e.Button != MouseButtons.Left) return;
			Cursor = Cursors.WaitCursor;
			GridHelper.GridlockFreeDays(_grid, LockManager);
		...
		void ToolStripMenuItemAbsenceLockRmMouseUp(object sender, MouseEventArgs e)
		{
			if (e.Button != MouseButtons.Left) return;
			Cursor = Cursors.WaitCursor;
			var absence = (Absence)(((ToolStripMenuItem)(sender)).Tag);
		...
		private void ToolStripMenuItemLockAllRestrictionsMouseUp(object sender, MouseEventArgs e)
		{
			...
			var gridlockAllRestrictionsCommand = new GridlockAllRestrictionsCommand(gridSchedulesExtractor, scheduleDayRestrictionExtractor, LockManager);
		...
		private void ToolStripMenuItemAllPreferencesMouseUp(object sender, MouseEventArgs e)
		{
			...
			var gridlockAllPreferencesCommand = new GridlockAllPreferencesCommand(gridSchedulesExtractor, scheduleDayPreferenceRestrictionExtractor, LockManager);
		...
		private void ToolStripMenuItemAllDaysOffMouseUp(object sender, MouseEventArgs e)
		{
			...
			IRestrictionExtractor restrictionExtractor = new RestrictionExtractor(_schedulerState.SchedulingResultState);
			IScheduleDayPreferenceRestrictionExtractor scheduleDayPreferenceRestrictionExtractor = new ScheduleDayPreferenceRestrictionExtractor(restrictionExtractor);
			var gridlockAllPreferencesDayOffCommand = new GridlockAllPreferencesDayOffCommand(gridSchedulesExtractor, scheduleDayPreferenceRestrictionExtractor, LockManager);
		...
		private void ToolStripMenuItemAllShiftsPreferencesMouseUp(object sender, MouseEventArgs e)
		{
			...
			IRestrictionExtractor restrictionExtractor = new RestrictionExtractor(_schedulerState.SchedulingResultState);
			IScheduleDayPreferenceRestrictionExtractor scheduleDayPreferenceRestrictionExtractor = new ScheduleDayPreferenceRestrictionExtractor(restrictionExtractor);
			var gridlockAllPreferencesShiftCommand = new GridlockAllPreferencesShiftCommand(gridSchedulesExtractor, scheduleDayPreferenceRestrictionExtractor, LockManager);
		...
		private void ToolStripMenuItemAllMustHaveMouseUp(object sender, MouseEventArgs e)
		{
			...
			IRestrictionExtractor restrictionExtractor = new RestrictionExtractor(_schedulerState.SchedulingResultState);
			IScheduleDayPreferenceRestrictionExtractor scheduleDayPreferenceRestrictionExtractor = new ScheduleDayPreferenceRestrictionExtractor(restrictionExtractor);
			var gridlockAllPreferencesMustHaveCommand = new GridlockAllPreferencesMustHaveCommand(gridSchedulesExtractor, scheduleDayPreferenceRestrictionExtractor, LockManager);
		...
		private void ToolStripMenuItemAllFulfilledMustHaveMouseUp(object sender, MouseEventArgs e)
		{
			...
			var gridlockAllMustHaveFulfilledCommand = new GridlockAllPreferencesMustHaveFulfilledCommand(gridSchedulesExtractor, restrictionChecker, scheduleDayPreferenceRestrictionExtractor, LockManager);
		...
		private void ToolStripMenuItemAllFulFilledPreferencesMouseUp(object sender, MouseEventArgs e)
		{
			...
			IScheduleDayPreferenceRestrictionExtractor scheduleDayPreferenceRestrictionExtractor = new ScheduleDayPreferenceRestrictionExtractor(restrictionExtractor);
			ICheckerRestriction restrictionChecker = new RestrictionChecker(null);
			var gridlockAllPreferencesFulfilledCommand = new GridlockAllPreferencesFulfilledCommand(gridSchedulesExtractor, restrictionChecker, scheduleDayPreferenceRestrictionExtractor, LockManager);
		...
		private void ToolStripMenuItemAllAbsencePreferenceMouseUp(object sender, MouseEventArgs e)
		{
			...
			IRestrictionExtractor restrictionExtractor = new RestrictionExtractor(_schedulerState.SchedulingResultState);
			IScheduleDayPreferenceRestrictionExtractor scheduleDayPreferenceRestrictionExtractor = new ScheduleDayPreferenceRestrictionExtractor(restrictionExtractor);
			var gridlockAllPreferencesAbsenceCommand = new GridlockAllPreferencesAbsenceCommand(gridSchedulesExtractor, scheduleDayPreferenceRestrictionExtractor, LockManager);
		...
		private void ToolStripMenuItemAllFulFilledAbsencesPreferencesMouseUp(object sender, MouseEventArgs e)
		{
			...
			IScheduleDayPreferenceRestrictionExtractor scheduleDayPreferenceRestrictionExtractor = new ScheduleDayPreferenceRestrictionExtractor(restrictionExtractor);
			ICheckerRestriction restrictionChecker = new RestrictionChecker(null);
			var gridlockAllPreferencesFulfilledAbsenceCommand = new GridlockAllPreferencesFulfilledAbsenceCommand(gridSchedulesExtractor, restrictionChecker, scheduleDayPreferenceRestrictionExtractor, LockManager);
		...
		private void ToolStripMenuItemAllFulFilledDaysOffPreferencesMouseUp(object sender, MouseEventArgs e)
		{
			...
			IScheduleDayPreferenceRestrictionExtractor scheduleDayPreferenceRestrictionExtractor = new ScheduleDayPreferenceRestrictionExtractor(restrictionExtractor);
			ICheckerRestriction restrictionChecker = new RestrictionChecker(null);
			var gridlockAllPreferencesFulfilledDaysOffCommand = new GridlockAllPreferencesFulfilledDayOffCommand(gridSchedulesExtractor, restrictionChecker, scheduleDayPreferenceRestrictionExtractor, LockManager);
		...
		private void ToolStripMenuItemAllFulFilledShiftsPreferencesMouseUp(object sender, MouseEventArgs e)
		{
			...
			IScheduleDayPreferenceRestrictionExtractor scheduleDayPreferenceRestrictionExtractor = new ScheduleDayPreferenceRestrictionExtractor(restrictionExtractor);
			ICheckerRestriction restrictionChecker = new RestrictionChecker(null);
			var gridlockAllPreferencesFulfilledShiftCommand = new GridlockAllPreferencesFulfilledShiftCommand(gridSchedulesExtractor, restrictionChecker, scheduleDayPreferenceRestrictionExtractor, LockManager);
		...
		private void ToolStripMenuItemAllRotationsMouseUp(object sender, MouseEventArgs e)
		{
			...
			var gridlockAllRotationsCommand = new GridlockAllRotationsCommand(gridSchedulesExtractor, scheduleDayRotationExtractor, LockManager);
		...
		private void ToolStripMenuItemAllDaysOffRotationsMouseUp(object sender, MouseEventArgs e)
		{
			...
			IRestrictionExtractor restrictionExtractor = new RestrictionExtractor(_schedulerState.SchedulingResultState);
			IScheduleDayRotationRestrictionExtractor scheduleDayRotationExtractor = new ScheduleDayRotationRestrictionExtractor(restrictionExtractor);
			var gridlockAllRotationsDayOffCommand = new GridlockAllRotationsDayOffCommand(gridSchedulesExtractor, scheduleDayRotationExtractor, LockManager);
		...
		private void ToolStripMenuItemAllShiftsRotationsMouseUp(object sender, MouseEventArgs e)
		{
			...
			IRestrictionExtractor restrictionExtractor = new RestrictionExtractor(_schedulerState.SchedulingResultState);
			IScheduleDayRotationRestrictionExtractor scheduleDayRotationExtractor = new ScheduleDayRotationRestrictionExtractor(restrictionExtractor);
			var gridlockAllRotationsShiftCommand = new GridlockAllRotationsShiftCommand(gridSchedulesExtractor, scheduleDayRotationExtractor, LockManager);
		...
		private void ToolStripMenuItemAllFulFilledRotationsMouseUp(object sender, MouseEventArgs e)
		{
			...
			ICheckerRestriction restrictionChecker = new RestrictionChecker(null);
			var gridlockAllRotationsFulfilledCommand = new GridlockAllRotationsFulfilledCommand(gridSchedulesExtractor, restrictionChecker, scheduleDayRotationRestrictionExtractor, LockManager);
		...
		private void ToolStripMenuItemAllFulFilledDaysOffRotationsMouseUp(object sender, MouseEventArgs e)
		{
			...
			IScheduleDayRotationRestrictionExtractor scheduleDayRotationRestrictionExtractor = new ScheduleDayRotationRestrictionExtractor(restrictionExtractor);
			ICheckerRestriction restrictionChecker = new RestrictionChecker(null);
			var gridlockAllRotationsFulfilledDayOffCommand = new GridlockAllRotationsFulfilledDayOffCommand(gridSchedulesExtractor, restrictionChecker, scheduleDayRotationRestrictionExtractor, LockManager);
		...
		private void ToolStripMenuItemAllFulFilledShiftsRotationsMouseUp(object sender, MouseEventArgs e)
		{
			...
			IScheduleDayRotationRestrictionExtractor scheduleDayRotationRestrictionExtractor = new ScheduleDayRotationRestrictionExtractor(restrictionExtractor);
			ICheckerRestriction restrictionChecker = new RestrictionChecker(null);
			var gridlockAllRotationsFulfilledShiftCommand = new GridlockAllRotationsFulfilledShiftCommand(gridSchedulesExtractor, restrictionChecker, scheduleDayRotationRestrictionExtractor, LockManager);
		...
		private void ToolStripMenuItemAllUnavailableStudentAvailabilityMouseUp(object sender, MouseEventArgs e)
		{
			...
			var gridlockAllStudentAvailabilityUnavailableCommand = new GridlockAllStudentAvailabilityUnavailableCommand(gridSchedulesExtractor, scheduleDayStudentAvailabilityExtractor, LockManager);
		...
		private void ToolStripMenuItemAllAvailableStudentAvailabilityMouseUp(object sender, MouseEventArgs e)
		{
			...
			IRestrictionExtractor restrictionExtractor = new RestrictionExtractor(_schedulerState.SchedulingResultState);
			IScheduleDayStudentAvailabilityRestrictionExtractor scheduleDayStudentAvailabilityExtractor = new ScheduleDayStudentAvailabilityRestrictionExtractor(restrictionExtractor);
			var gridlockAllStudentAvailabilityAvailableCommand = new GridlockAllStudentAvailabilityAvailableCommand(gridSchedulesExtractor, scheduleDayStudentAvailabilityExtractor, LockManager);
		...
		private void ToolStripMenuItemAllFulFilledStudentAvailabilityMouseUp(object sender, MouseEventArgs e)
		{
			...
			ICheckerRestriction restrictionChecker = new RestrictionChecker(null);
			var gridlockAllStudentAvailabilityFulfilledCommand = new GridlockAllStudentAvailabilityFulfilledCommand(gridSchedulesExtractor, restrictionChecker, scheduleDayStudentAvailabilityRestrictionExtractor, LockManager);
		...
		private void ToolStripMenuItemAllUnavailableAvailabilityMouseUp(object sender, MouseEventArgs e)
		{
			...
			var gridlockAllAvailabilityUnavailableCommand = new GridlockAllAvailabilityUnavailableCommand(gridSchedulesExtractor, scheduleDayAvailabilityExtractor, LockManager);
		...
		private void ToolStripMenuItemAllAvailableAvailabilityMouseUp(object sender, MouseEventArgs e)
		{
			...
			IRestrictionExtractor restrictionExtractor = new RestrictionExtractor(_schedulerState.SchedulingResultState);
			IScheduleDayAvailabilityRestrictionExtractor scheduleDayAvailabilityExtractor = new ScheduleDayAvailabilityRestrictionExtractor(restrictionExtractor);
			var gridlockAllAvailabilityAvailableCommand = new GridlockAllAvailabilityAvailableCommand(gridSchedulesExtractor, scheduleDayAvailabilityExtractor, LockManager);
		...
		private void ToolStripMenuItemAllFulFilledAvailabilityMouseUp(object sender, MouseEventArgs e)
		{
			...
			ICheckerRestriction restrictionChecker = new RestrictionChecker(null);
			var gridlockAllAvailabilityFulfilledCommand = new GridlockAllAvailabilityFulfilledCommand(gridSchedulesExtractor, restrictionChecker, scheduleDayAvailabilityRestrictionExtractor, LockManager);
		...
		{
			return new PersonRequestAuthorization(TeleoptiPrincipal.Current.PrincipalAuthorization).IsPermittedRequestApprove(model.RequestType);
		}

		private static bool isPermittedViewRequest()
		{
			return new PersonRequestAuthorization(TeleoptiPrincipal.Current.PrincipalAuthorization).IsPermittedRequestView();
		}
		...
		void skillGridMenuItemDay_Click(object sender, EventArgs e)
		{
			_intradayMode = false;
			((ToolStripMenuItem)_contextMenuSkillGrid.Items["IntraDay"]).Checked = _intradayMode;
			((ToolStripMenuItem)_contextMenuSkillGrid.Items["Day"]).Checked = !_intradayMode;
		...
		void skillGridMenuItemIntraDay_Click(object sender, EventArgs e)
		{
			_intradayMode = true;
			((ToolStripMenuItem)_contextMenuSkillGrid.Items["IntraDay"]).Checked = _intradayMode;
			((ToolStripMenuItem)_contextMenuSkillGrid.Items["Day"]).Checked = !_intradayMode;
		...
		void skillGridMenuItem_Click(object sender, EventArgs e)
		{
		...
		void skillGridMenuItemEdit_Click(object sender, EventArgs e)
		{
		...
		void skillGridMenuItemDelete_Click(object sender, EventArgs e)
		{
			var menuItem = (ToolStripMenuItem)sender;
		...
		void _backgroundWorkerResourceCalculator_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
		{
			if (Disposing)
				return;
		...
		void _backgroundWorkerResourceCalculator_ProgressChanged(object sender, ProgressChangedEventArgs e)
		{
			if (Disposing)
				return;
		...
		void _backgroundWorkerResourceCalculator_DoWork(object sender, DoWorkEventArgs e)
		{
			setThreadCulture();
			_optimizationHelperWin.ResourceCalculateMarkedDays(e, _backgroundWorkerResourceCalculator, _optimizerOriginalPreferences.SchedulingOptions.ConsiderShortBreaks, true);
		}
		...
		void _backgroundWorkerValidatePersons_DoWork(object sender, DoWorkEventArgs e)
		{
			setThreadCulture();
			if (_scheduleView != null)
			{
		...
		void _backgroundWorkerValidatePersons_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
		{
			if (Disposing)
				return;
		...
		void backgroundWorkerLoadData_ProgressChanged(object sender, ProgressChangedEventArgs e)
		{
			if (Disposing)
				return;
			toolStripProgressBar1.PerformStep();
		...
		void backgroundWorkerLoadData_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
		{
			_backgroundWorkerRunning = false;
			...
			DateOnly dateOnly = new DateOnly(SchedulerState.RequestedPeriod.StartDateTimeLocal(TeleoptiPrincipal.Current.Regional.TimeZone));
		...
		void backgroundWorkerLoadData_DoWork(object sender, DoWorkEventArgs e)
		{
		...
		private void editMeeting()
		{
			if (_scheduleView != null)
			{
				...
				if (dest != null)
				{
					...
					if (meeting != null)
					{
						meeting = meeting.EntityClone(); //We don't want to work with the actual meeting, that will be a bad idea!
						IList<ITeam> meetingPersonsTeams = getDistinctTeamList(meeting);
						bool editPermission = hasFunctionPermissionForTeams(meetingPersonsTeams, DefinedRaptorApplicationFunctionPaths.ModifyMeetings);
		...
		void _optimizationHelper_ResourcesChanged(object sender, ResourceChangedEventArgs e)
		{
			if (InvokeRequired)
			{
		...
		void _backgroundWorkerDelete_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
		{
			if (Disposing)
				return;
		...
		void _backgroundWorkerDelete_DoWork(object sender, DoWorkEventArgs e)
		{
			setThreadCulture();
		...
		static void chartControlSkillData_ChartRegionMouseEnter(object sender, ChartRegionMouseEventArgs e)
		{
			//GridChartManager.SetChartToolTip(e.Region, _chartControlSkillData);
		}

		void chartControlSkillData_ChartRegionMouseHover(object sender, ChartRegionMouseEventArgs e)
		{
			GridChartManager.SetChartToolTip(e.Region, _chartControlSkillData);
		}

		void chartControlSkillData_ChartRegionClick(object sender, ChartRegionMouseEventArgs e)
		{
		...
		void _backgroundWorkerScheduling_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
		{
			if (Disposing)
				return;
			if (_undoRedo.InUndoRedo)
		...
		void _backgroundWorkerScheduling_ProgressChanged(object sender, ProgressChangedEventArgs e)
		{
			if (Disposing)
				return;
		...
		[System.Diagnostics.CodeAnalysis.SuppressMessage("Microsoft.Maintainability", "CA1506:AvoidExcessiveClassCoupling")]
		void _backgroundWorkerScheduling_DoWork(object sender, DoWorkEventArgs e)
		{
			setThreadCulture();
		...
		void _backgroundWorkerOptimization_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
		{
		...
		void _backgroundWorkerOptimization_ProgressChanged(object sender, ProgressChangedEventArgs e)
		{
			if (Disposing)
				return;
			if (InvokeRequired)
		...
		private void releaseUserInterface(bool canceled)
		{
			using (PerformanceOutput.ForOperation("ReleaseUserInterface"))
			{
				...
				schedulerSplitters1.ElementHost1.Enabled = true; //shifteditor                                               
				if (_scheduleView != null)
				{
		...
		void _backgroundWorkerOptimization_DoWork(object sender, DoWorkEventArgs e)
		{
			...
			switch (options.OptimizationMethod)
			{
				case OptimizationMethod.BackToLegalState:
					...
					_optimizationHelperWin.ResourceCalculateMarkedDays(e, null, optimizerPreferences.SchedulingOptions.ConsiderShortBreaks, true);
				...
				case OptimizationMethod.ReOptimize:
					...
					if (optimizerPreferences.SchedulingOptions.UseBlockScheduling == BlockFinderType.BetweenDayOff || optimizerPreferences.SchedulingOptions.UseBlockScheduling == BlockFinderType.SchedulePeriod)
					{
		...
		{
			bool permitted = TeleoptiPrincipal.Current.PrincipalAuthorization.IsPermitted(DefinedRaptorApplicationFunctionPaths.ModifyPersonAssignment);
		...
		private void setPermissionOnScheduleControl()
		{
			...
			toolStripSplitButtonSchedule.Enabled = TeleoptiPrincipal.Current.PrincipalAuthorization.IsPermitted(DefinedRaptorApplicationFunctionPaths.AutomaticScheduling);
			if (_scheduleView != null)
		...
		private void setPermissionOnContextMenuItems()
		{
			...
			toolStripMenuItemDeleteSpecial.Enabled = authorization.IsPermitted(DefinedRaptorApplicationFunctionPaths.ModifyPersonAssignment);
			toolStripMenuItemWriteProtectSchedule.Enabled =
				toolStripMenuItemWriteProtectSchedule2.Enabled = authorization.IsPermitted(DefinedRaptorApplicationFunctionPaths.SetWriteProtection);
		...
		private void createMaxSeatSkills(ISkillDayRepository skillDayRepository)
		{
			...
			DateTimePeriod extendedPeriod = SchedulerState.RequestedPeriod.ChangeStartTime(new TimeSpan(-8, -1, 0, 0)).ChangeEndTime(new TimeSpan(8, 1, 0, 0));
			...
			var schedulerSkillDayHelper = new SchedulerSkillDayHelper(SchedulerState.SchedulingResultState, extendedPeriod.ToDateOnlyPeriod(timeZoneInfo), skillDayRepository, SchedulerState.RequestedScenario);
		...
		private void nonBlendSkills()
		{
			...
			int demand;
			using (var options = new TempNonBlendSchedulingPreferences(_optimizerOriginalPreferences.SchedulingOptions, _scheduleOptimizerHelper.CreateGroupPages(_scheduleView, _schedulerState),
			...
			DateTimePeriod extendedPeriod = SchedulerState.RequestedPeriod.ChangeStartTime(new TimeSpan(-8, -1, 0, 0)).ChangeEndTime(new TimeSpan(8, 1, 0, 0));
			var schedulerSkillDayHelper = new SchedulerSkillDayHelper(SchedulerState.SchedulingResultState, extendedPeriod.ToDateOnlyPeriod(SchedulerState.TimeZoneInfo), skillDayRepository, SchedulerState.RequestedScenario);
			...
			IGroupPageDataProvider groupPageDataProvider = new GroupScheduleGroupPageDataProvider(_schedulerState, new RepositoryFactory(),
																									  UnitOfWorkFactory.Current);
			schedulerHelper.CreateNonBlendSkillsFromGrouping(groupPageDataProvider, _optimizerOriginalPreferences.SchedulingOptions.GroupOnGroupPage, selectedDates.First(),
		...
		private void filteringPeopleAndSkills(IUnitOfWork uow, ISchedulerStateHolder stateHolder, IPeopleAndSkillLoaderDecider decider)
		{
			using (PerformanceOutput.ForOperation("Executing and filtering loader decider"))
			{
				...
				decider.Execute(_schedulerState.RequestedScenario, _schedulerState.RequestedPeriod, SchedulerState.AllPermittedPersons);
		...
		{
			ICollection<ISkill> skills = new SkillRepository(uow).FindAllWithSkillDays(stateHolder.RequestedPeriod.ToDateOnlyPeriod(new CccTimeZoneInfo(TimeZoneInfo.Utc)));
		...
		void _schedulerMessageBrokerHandler_RequestInsertedFromBroker(object sender, CustomEventArgs<IPersonRequest> e)
		{
		...
		void _schedulerMessageBrokerHandler_RequestDeletedFromBroker(object sender, CustomEventArgs<IPersonRequest> e)
		{
		...
		void _schedules_PartModified(object sender, ModifyEventArgs e)
		{
			...
			using (PerformanceOutput.ForOperation("Loading people"))
			{
				...
				if (_teamLeaderMode)
				{
					...
					new SelectedEntitiesForPeriod(_temporarySelectedEntitiesFromTreeView,
						_schedulerState.RequestedPeriod), new RepositoryFactory());
				}
				else
				{
					loader = new PeopleLoader(personRep, new ContractRepository(uow), SchedulerState,
					new SelectedEntitiesForPeriod(_temporarySelectedEntitiesFromTreeView,
						_schedulerState.RequestedPeriod), new SkillRepository(uow));
				}
		...
		private bool save()
		{
			if (_scheduleView != null)
			{
				if (restrictionEditor.RestrictionIsAltered) restrictionEditor.LoadRestriction(restrictionEditor.SchedulePart);
		...
		private void disableAllExceptCancelInRibbon()
		{
			...
			using (PerformanceOutput.ForOperation("disableAllExceptCancelInRibbon"))
			{
				...
				if (toolStripSpinningProgressControl1.SpinningProgressControl == null) //Why is this null in some cases?!?
		...
		{
			_currentIntraDayDate = new DateOnly(_schedulerState.RequestedPeriod.StartDateTimeLocal(_schedulerState.TimeZoneInfo));
		...
		private void showFilterDialog()
		{
			using (IUnitOfWork uow = UnitOfWorkFactory.Current.CreateAndOpenUnitOfWork())
			{
				...
				using (var scheduleFilterView = new ScheduleFilterView(_scheduleFilterModelCached))
				{
					...
					Point pointToScreen = toolStripExScheduleViews.PointToScreen(new Point(toolStripButtonFilterAgents.Bounds.X + 63, toolStripButtonFilterAgents.Bounds.Y + toolStripButtonFilterAgents.Height));
					...
					if (scheduleFilterView.ShowDialog() == DialogResult.OK)
					{
						...
						if (page == null)
							uniquePersons = new HashSet<IPerson>(_scheduleFilterModelCached.SelectedPersonDictionary.Values);
						else
		...
		private void zoom(ZoomLevel level)
		{
			...
			switch (level)
			{
				case ZoomLevel.Level1:
					...
					_scheduleView = new DayViewNew(_grid, SchedulerState, _gridLockManager, SchedulePartFilter, _clipHandlerSchedule, _overriddenBusinessRulesHolder, 
				...
				case ZoomLevel.Level7:
					...
					_scheduleView = new RestrictionSummaryView(_grid, SchedulerState, _gridLockManager,
															   ...
															   _singleAgentRestrictionPresenter, _ruleSetProjectionService, _overriddenBusinessRulesHolder, callback, _defaultScheduleTag);
			...
			if (level == ZoomLevel.Level1)
			{
				...
				_grid.Model.Selections.SelectRange(GridRangeInfo.Cell(_grid.CurrentCell.RowIndex, _grid.CurrentCell.ColIndex), true);
		...
		void _requestView_PropertyChanged(object sender, PropertyChangedEventArgs e)
		{
			enableSave();
		}
		...
		void scenarioMenuItem_Click(object sender, EventArgs e)
		{
			...
			var exportForm = new ExportToScenarioResultView(uowFactory, scheduleRepository,
															   new MoveDataBetweenSchedules(allNewRules, new SchedulerStateScheduleDayChangedCallback(new ResourceCalculateDaysDecider(), SchedulerState)),
		...
		private void enableSwapButtons(IList<IScheduleDay> selectedSchedules)
		{
			...
			if (selectedSchedules.Count <= 1 || ScheduleViewBase.AllSelectedPersons(selectedSchedules).Count() != 2) return;
		...
		private void updateSelectionInfo(IList<IScheduleDay> selectedSchedules)
		{
			if (_scheduleView != null)
			{
				...
				if (_agentInfo != null)
					_agentInfo.UpdateData(personDic, dateList, _schedulerState.SchedulingResultState, SchedulerState.SchedulingResultState.AllPersonAccounts);

				toolStripStatusLabelContractTime.Text = string.Concat(Resources.ContractScheduledTime, " ",
																	  DateHelper.HourMinutesString(totalTime.TotalMinutes));
			...
			if (aggregateSkillSkill.IsVirtual)
			{
				skillStaffPeriods = SchedulerState.SchedulingResultState.SkillStaffPeriodHolder.SkillStaffPeriodList(
						aggregateSkillSkill, TimeZoneHelper.NewUtcDateTimePeriodFromLocalDateTime(_currentIntraDayDate,
																			 _currentIntraDayDate.AddDays(1), _schedulerState.TimeZoneInfo));
			}
			else
			{
		...
		private void loadSkillDays(IUnitOfWork uow, ISchedulerStateHolder stateHolder, IPeopleAndSkillLoaderDecider decider)
		{
			...
			using (PerformanceOutput.ForOperation("Loading skill days"))
			{
				...
				stateHolder.SchedulingResultState.SkillDays = new SkillDayLoadHelper(skillDayRepository, multisiteDayRepository).LoadSchedulerSkillDays(
																						   stateHolder.RequestedPeriod.ChangeStartTime(new TimeSpan(-8, -1, 0, 0)).ChangeEndTime(new TimeSpan(8, 1, 0, 0)).ToDateOnlyPeriod(stateHolder.TimeZoneInfo),
				...
				IList<ISkillStaffPeriod> skillStaffPeriods =
					stateHolder.SchedulingResultState.SkillStaffPeriodHolder.SkillStaffPeriodList(stateHolder.SchedulingResultState.Skills, stateHolder.LoadedPeriod.Value);
		...
		void _grid_StartAutoScrolling(object sender, StartAutoScrollingEventArgs e)
		{
			if(e.Reason == AutoScrollReason.MouseDragging)
				_grid.SupportsPrepareViewStyleInfo = false;     
		}

		void _grid_ScrollControlMouseUp(object sender, CancelMouseEventArgs e)
		{
		...
		void wpfShiftEditor1_EditMeeting(object sender, CustomEventArgs<IPersonMeeting> e)
		{
		...
		void wpfShiftEditor1_CreateMeeting(object sender, CustomEventArgs<IPersonMeeting> e)
		{
			bool viewSchedulesPermission = isPermittedToViewSchedules();
			_schedulerMeetingHelper.MeetingComposerStart(null, _scheduleView, true, viewSchedulesPermission);
		}

		void restrictionEditor_RestrictionChanged(object sender, System.Windows.RoutedEventArgs e)
		{
			if (_scheduleView != null)
			{
		...
		void notesEditor_NotesChanged(object sender, System.Windows.RoutedEventArgs e)
		{
			if (_scheduleView != null)
			{
		...
		void notesEditor_PublicNotesChanged(object sender, System.Windows.RoutedEventArgs e)
		{
			if (_scheduleView != null)
			{
				_scheduleView.Presenter.LastUnsavedSchedulePart = notesEditor.SchedulePart;
		...
		{
			if (_scheduleView == null) return;
			_scheduleView.Presenter.AddOvertime(new List<IScheduleDay> { e.SchedulePart }, e.Period, MultiplicatorDefinitionSet);
		}
		...
		private void toolStripButtonApproveRequestClick(object sender, EventArgs e)
		{
			...
			changeRequestStatus(
				new ApprovePersonRequestCommand(this, _schedulerState.Schedules, _schedulerState.RequestedScenario,
												...
												_personRequestAuthorizationChecker, allNewBusinessRules, _overriddenBusinessRulesHolder, new SchedulerStateScheduleDayChangedCallback(new ResourceCalculateDaysDecider(), SchedulerState)), _requestView.SelectedAdapters());
			if (_requestView != null)
				_requestView.NeedUpdate = true;
		...
		void filterbox_FilterClicked(object sender, FilterBoxAdvancedEventArgs e)
		{
		...
		private void toolStripButtonReplyAndApprove_Click(object sender, EventArgs e)
		{
			...
			replyAndChangeStatus(new ApprovePersonRequestCommand(this, _schedulerState.Schedules,
																 ...
																 _personRequestAuthorizationChecker, businessRules, _overriddenBusinessRulesHolder, new SchedulerStateScheduleDayChangedCallback(new ResourceCalculateDaysDecider(), SchedulerState)));
			if (_requestView != null)
				_requestView.NeedUpdate = true;
		...
		private void ToolStripMenuItemExportToPdfGraphicalMouseUp(object sender, MouseEventArgs e)
		{
			...
			if (model.Team)
				ScheduleToPdfManager.ExportShiftPerDayTeamViewGraphical(culture, personDic, period, SchedulerState.SchedulingResultState, rightToLeft, this, path, model);
			else
				ScheduleToPdfManager.ExportShiftPerDayAgentViewGraphical(culture, personDic, period, SchedulerState.SchedulingResultState, rightToLeft, this, path, model);
		}
		...
		{
			Cursor = Cursors.WaitCursor;
			IList<ISkillStaffPeriod> skillStaffPeriods = _schedulerState.SchedulingResultState.SkillStaffPeriodHolder.SkillStaffPeriodList(_schedulerState.SchedulingResultState.Skills,
        ...
        private void toolStripMenuItemViewHistory_Click(object sender, EventArgs e)
        {
            ...
            using(var auditHistoryView = new AuditHistoryView(selected, this))
            {
                ...
                if (auditHistoryView.DialogResult != DialogResult.OK || auditHistoryView.SelectedScheduleDay == null || isLocked) return;
