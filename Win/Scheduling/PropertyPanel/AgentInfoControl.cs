    ...
    public partial class AgentInfoControl : BaseUserControl
    {
		...
		private void resetGroupPageData()
		{
			_groupPagePerDate = _container.Resolve<IGroupPageCreator>()
				.CreateGroupPagePerDate(_stateHolder.AllPermittedPersons, _dateOnlyPeriod.DayCollection(), _container.Resolve<IGroupScheduleGroupPageDataProvider>(),
        ...
        private void updateRestrictionData(IPerson person, DateOnly dateOnly, ISchedulingResultStateHolder state)
        {
	        ...
	        if (PrincipalAuthorization.Current().IsPermitted(DefinedRaptorApplicationFunctionPaths.ModifyAvailabilities))
	        {
