...
namespace Teleopti.Ccc.Win.PeopleAdmin.Controls.Columns
{
    public class EditableTimeSpanInSchedulePeriodColumn<T> : ColumnBase<T>
    {
        ...
        public EditableTimeSpanInSchedulePeriodColumn(string bindingProperty, string headerText)
        {
        ...
        public override void SaveCellInfo(GridSaveCellInfoEventArgs e, ReadOnlyCollection<T> dataItems)
        {
            if (e.ColIndex > 0 && e.RowIndex > 0)
            {
                ...
                {
                    _propertyReflector.SetValue(dataItem, _bindingProperty, TimeSpan.Zero);
                }
