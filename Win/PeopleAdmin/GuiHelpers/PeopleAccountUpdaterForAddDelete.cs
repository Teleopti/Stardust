using Teleopti.Ccc.Domain.Tracking;
using Teleopti.Interfaces.Domain;

namespace Teleopti.Ccc.Win.PeopleAdmin.GuiHelpers
{
	/// <summary>
	/// Person account updater that works with the Win project's FilteredPeopleHolder's domain data
	/// </summary>
	/// <remarks>
	/// Used for the win fat client where all the data is already loaded in the FilteredPeopleHolder
	/// </remarks>
	public class PeopleAccountUpdaterForAddDelete : IPersonAccountUpdater
	{
		private readonly IPersonAccountCollection _personAccountCollection;
		private readonly ITraceableRefreshService _traceableRefreshService;

		public PeopleAccountUpdaterForAddDelete(IPersonAccountCollection personAccountCollection, ITraceableRefreshService traceableRefreshService)
		{
			_personAccountCollection = personAccountCollection;
			_traceableRefreshService = traceableRefreshService;
		}

		public void Update(IPerson person)
		{
			var personAccounts = _personAccountCollection.PersonAbsenceAccounts();

			foreach (var personAccount in personAccounts)
			{
				foreach (var account in personAccount.AccountCollection())
				{
					_traceableRefreshService.Refresh(account);
				}
			}
		}
	}
}