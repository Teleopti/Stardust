...
namespace Teleopti.Ccc.InfrastructureTest.Intraday
{
	...
	public class ResourceCalculationReadModelPersisterTest
	{
		...
		[Test]
		public void ShouldUpdateTimeStampForABu()
		{
			Now.Is("2016-06-16 03:15");
			...
			Now.Is("2016-06-16 06:15");
			CurrentUnitOfWork.Current().PersistAll();
			Target.UpdateInsertedDateTime(bu1.Id.GetValueOrDefault());
			
			var result = Target.GetLastCalculatedTime();
			result.First().Value.Should().Be.EqualTo(new DateTime(2016, 06, 16, 06, 15, 0, DateTimeKind.Utc));
		}

		[Test]
		public void ShouldUpdateTimeStampForRespectiveBu()
		{
			Now.Is("2016-06-16 03:15");

			var bu1 = BusinessUnitFactory.CreateSimpleBusinessUnit("1");
			var bu2 = BusinessUnitFactory.CreateSimpleBusinessUnit("1");
			...
			var skill = new Skill("S1", "asdf", Color.AliceBlue, 15, skillType);
			var skill2 = new Skill("S2", "asdf2", Color.AliceBlue, 15, skillType);
			skill.Activity = activity;
			skill.SetBusinessUnit(bu1);
			skill.TimeZone = TimeZoneInfo.Utc;
			...
			var bu1Data = Target.GetLastCalculatedTime().Where(x=>x.Key==bu1.Id.GetValueOrDefault());
			bu1Data.First().Value.Should().Be.EqualTo(new DateTime(2016, 06, 16, 06, 15, 0, DateTimeKind.Utc));
			var bu2Data = Target.GetLastCalculatedTime().Where(x => x.Key == bu2.Id.GetValueOrDefault());
			bu2Data.First().Value.Should().Be.EqualTo(new DateTime(2016, 06, 16, 03, 15, 0, DateTimeKind.Utc));
		}
