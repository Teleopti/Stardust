	...
	public class ScheduleHistoryRepositoryTest : AuditTest
	{
		...
		public void ShouldFindRevisionForModifiedAssignment()
		{
			...
			using (var uow = UnitOfWorkFactory.Current.CreateAndOpenUnitOfWork())
			{
				...
				PersonAssignment.AddActivity(PersonAssignment.MainLayers().First().Payload, PersonAssignment.MainLayers().First().Period);
				uow.PersistAll();
			}
			using (UnitOfWorkFactory.Current.CreateAndOpenUnitOfWork())
			{
		...
		public void ShouldFindAbsenceNextDayDueToPossibleNightShifts()
		{
			...
			using (var uow = UnitOfWorkFactory.Current.CreateAndOpenUnitOfWork())
			{
				PersonAssignment.AddActivity(PersonAssignment.MainLayers().First().Payload, new DateTimePeriod(Today.AddHours(23), Today.AddHours(30)));
