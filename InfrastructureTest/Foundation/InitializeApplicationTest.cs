...
namespace Teleopti.Ccc.InfrastructureTest.Foundation
{
	...
	public class InitializeApplicationTest
	{
		...
		{
			createDummyXml();
			var ds1 = mocks.DynamicMock<IDataSource>();
			var ds2 = mocks.DynamicMock<IDataSource>();
			...
			using (mocks.Record())
			{
				...
				Expect.Call(ds1.OriginalFileName).PropertyBehavior();
				Expect.Call(ds2.AuthenticationTypeOption).PropertyBehavior();
				Expect.Call(ds2.OriginalFileName).PropertyBehavior();
			...
			using (new StateHolderModificationContext())
			{
				using (mocks.Playback())
				{
					...
					((ds1.AuthenticationTypeOption & AuthenticationTypeOption.Windows) == AuthenticationTypeOption.Windows).Should().Be.True();
					((ds1.AuthenticationTypeOption & AuthenticationTypeOption.Application) == AuthenticationTypeOption.Application).Should().Be.True();

					((ds2.AuthenticationTypeOption & AuthenticationTypeOption.Windows) == AuthenticationTypeOption.Windows).Should().Be.True();
					((ds2.AuthenticationTypeOption & AuthenticationTypeOption.Application) == AuthenticationTypeOption.Application).Should().Be.True();

					ds1.OriginalFileName.Should().Be.EqualTo(xmlFile1);
					ds2.OriginalFileName.Should().Be.EqualTo(xmlFile2);
				}
		...
		[Test]
		public void VerifyStartWhenMessageBrokerThrows()
		{
			createDummyXml();
			var ds1 = mocks.DynamicMock<IDataSource>();
			var ds2 = mocks.DynamicMock<IDataSource>();
		...
		/// <summary>
		/// Tests the start method with settings in a dictionary. 
		/// </summary>
		[Test]
		public void TestStartWithDictionary()
		{
			var ds = mocks.StrictMock<IDataSource>();
			const string matrixConnstring = "blabla";
		...
		[Test]
		public void TestStartWithDictionaryWhenMessageBrokerThrows()
		{
			var ds = mocks.StrictMock<IDataSource>();
			const string matrixConnstring = "blabla";
		...
		[Test]
		public void TestStartWithDictionaryWhenMessageBrokerDisabled()
		{

			var ds = mocks.StrictMock<IDataSource>();
			const string matrixConnstring = "blabla";
			target.MessageBrokerDisabled = true;
			using (mocks.Record())
			{
				Expect.Call(datasourcesFactory.Create(nHibSettings(), matrixConnstring)).Return(ds);
			}
