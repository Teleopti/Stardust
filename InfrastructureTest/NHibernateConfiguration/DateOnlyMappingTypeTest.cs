...
namespace Teleopti.Ccc.InfrastructureTest.NHibernateConfiguration
{
    ...
    public class DateOnlyMappingTypeTest
    {
        ...
        {
            DateOnly firstDateOnly = new DateOnly(DateTime.Now);
        ...
        public void CanNullSafeGet()
        {
            ...
            Expect.Call(dataReader[0]).Return(new DateOnly(DateTime.MinValue));
            mocks.ReplayAll();
            ...
            Assert.IsTrue(gotDateOnly.Equals(DateOnly.MinValue));

            dataReader = mocks.StrictMock<IDataReader>();
            Expect.Call(dataReader.GetOrdinal("Kalle")).Return(0);
            Expect.Call(dataReader[0]).Return(DBNull.Value);
            mocks.ReplayAll();
        ...
        public void CanNullSafeSet()
        {
            ...
            _target.NullSafeSet(dbCommand, new DateOnly(DateTime.MinValue), 0);

            Assert.IsNotNull(_target);
            Assert.AreEqual(new DateOnly(DateTime.MinValue).ToString(), dataParameter.Value);
