(function() {
    ...
    angular.module('wfm.rta').controller('RtaOverviewController', RtaOverviewController);
    ...
    function RtaOverviewController(
        ...
        (function() {
            if (angular.isDefined(vm.urlParams.skillAreaId)) {
                vm.agentsState =
                    'rta-agents({siteId	s: card.site.Id, skillAreaId: "' + vm.urlParams.skillAreaId + '"})';
                vm.agentsStateForTeam =
                    'rta-agents({teamIds: team.Id, skillAreaId: "' + vm.urlParams.skillAreaId + '"})';
                ...
                vm.agentsStateForTeam =
                    'rta-agents({teamIds: team.Id, skillIds: ["' + vm.urlParams.skillIds[0] + '"]})';
            } else {
                vm.agentsState = 'rta-agents({siteIds: card.site.Id})';
                vm.agentsStateForTeam = 'rta-agents({teamIds: team.Id})';
            }
        ...
        var poller = rtaPollingService.create(function() {
            return getSites().then(getTeamsForSites);
        });
        vm.siteCards = [];
        poller.start();
        ...
        function getSites() {
            return rtaService.getOverviewModelFor(vm.skillIds).then(function(sites) {
        ...
        function getTeamsForSite(s) {
            return rtaService.getTeamCardsFor({siteIds: s.site.Id, skillIds: vm.skillIds}).then(function(teams) {
