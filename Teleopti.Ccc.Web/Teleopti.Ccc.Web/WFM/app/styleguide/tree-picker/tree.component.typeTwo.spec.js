describe('TreeDataTwoController', function() {
	...
	function setupPicker(attrs, scope, optCompileOpts) {
		var el;
		var template = '' + '<tree-data-two ' + (attrs || '') + '>' + '</tree-data-two>';
	...
	it('should select the clicked node and its parents', function() {
		...
		expect(
			data.nodes[mapParents[0]].nodes[mapParents[1]].nodes[mapParents[2]].nodes[mapParents[3]].selected
		).toEqual(true);
		expect(
			data.nodes[mapParents[0]].nodes[mapParents[1]].nodes[mapParents[2]].nodes[mapParents[3]].nodes[
				mapParents[4]
			].selected
		).toEqual(true);
	});
	...
	it('should not select siblings', function() {
		...
		expect(data.nodes[mapParents[0]].nodes[mapParents[1]].nodes[mapParents[2]].nodes[1].nodes[0].selected).toEqual(
			false
		);
	});
});
