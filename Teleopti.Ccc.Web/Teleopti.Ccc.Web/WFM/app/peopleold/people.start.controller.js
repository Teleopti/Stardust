...
angular
	...
	.controller('PeopleStartCtrl', [
		...
		'PeopleService',
		'NoticeService',
		PeopleStartController
	]);

function PeopleStartController(
	$scope,
	$filter,
	$state,
	$stateParams,
	...
	$scope.paginationOptions = {
		...
		totalPages: 0,
		sortColumns:
			stateParamsPagination.sortColumns !== undefined
				? stateParamsPagination.sortColumns
				: [
						{
							ColumnName: 'LastName',
							SortASC: true
	...
	var selectPeople = function(rows) {
		...
		var allRowsInCurrentPageSelected =
			$scope.gridApi.selection.getSelectedRows().length === $scope.getCurrentPageSelectedRowsLength();
		...
		$scope.selectAllVisible =
			$scope.paginationOptions.totalPages > 1 && rowInCurrentPageSelected && !allRowsSelected;
	};
	...
	$scope.defautKeyword = function() {
		if (
			...
			$scope.getCurrentPageSelectedRowsLength() > 0
		) {
	...
	if ($state.current.name === 'peopleold.start' && toggleSvc.Wfm_PeopleWeb_PrepareForRelease_76666) {
		var message = $translate
			...
			.replace(
				'{1}',
				"<a href=' http://www.teleopti.com/wfm/customer-feedback.aspx' target='_blank' target='_blank'>"
			)
			.replace('{2}', '</a>');
