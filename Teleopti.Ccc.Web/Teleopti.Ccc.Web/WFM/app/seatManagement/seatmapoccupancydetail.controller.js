(function() {
	angular.module('wfm.seatPlan').controller('SeatMapOccupancyController', seatMapOccupancyDirectiveController);
	seatMapOccupancyDirectiveController.$inject = [
		'seatMapCanvasUtilsService',
		'$translate',
		'seatPlanService',
		'seatMapService',
		'NoticeService',
		'seatMapTranslatorFactory'
	];

	function seatMapOccupancyDirectiveController(
		utils,
		$translate,
		seatPlanService,
		seatMapService,
		...
		vm.deleteSeatBooking = function(booking) {
			seatMapService.occupancy.remove({ Id: booking.BookingId }).$promise.then(function() {
				vm.refreshSeatMap();
				var deleteSuccessMessage = seatmapTranslator.TranslatedStrings['SeatBookingDeletedSuccessfully']
					.replace('{0}', booking.SeatName)
					.replace('{1}', booking.FirstName + ' ' + booking.LastName);
		...
		function onSeatPlanCompleted(seatPlanResultMessage) {
			var seatPlanResultDetailMessage = $translate
				.instant('SeatPlanResultDetailMessage')
			...
			if (vm.previousSelectedSeatIds.length > 0) {
				if (vm.previousSelectedSeatIds.length == 1) {
					...
					vm.previousSelectedSeatIds.forEach(function(seatId) {
						...
						if (seatObj != null) seatObjects.push(seatObj);
					});
