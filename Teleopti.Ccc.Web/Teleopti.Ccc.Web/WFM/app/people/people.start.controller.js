...
var fetchJSON = function(input, init) {
...
// Array.sort
var sortRolesByName = function(role1, role2) {
    return role1.Name >= role2.Name;
};
var sortPeopleByName = function(person1, person2) {
    ...
    return __awaiter(_this, void 0, void 0, function() {
        return __generator(this, function(_a) {
            return [
                2 /*return*/,
                fetchJSON(
                    '../api/Search/People/Keyword?currentPageIndex=1&keyword=' +
                        keyword +
                        '&pageSize=10&sortColumns=LastName:true',
                    fetchDefaultOptions
                ).then(function(_a) {
...
var getPersons = function(_a) {
    var _b = _a.Date,
        Date = _b === void 0 ? '2017-02-08' : _b,
        PersonIdList = _a.PersonIdList;
    return __awaiter(_this, void 0, void 0, function() {
        return __generator(this, function(_c) {
            return [
                2 /*return*/,
                fetchJSON(
                    '../api/PeopleData/fetchPersons',
                    __assign({}, fetchDefaultOptions, {
                        method: 'POST',
                        body: JSON.stringify({ Date: Date, PersonIdList: PersonIdList })
                    })
                )
            ];
        });
            ...
            switch (_a.label) {
                ...
                case 2:
                    ...
                    return [
                        ...
                        persons
                            .map(function(person) {
                                return __assign({}, person, { selected: false });
                            })
    ...
    function PeopleStartController($scope) {
        ...
        $scope.toggleSelectedPerson = function(person) {
            if ($scope.selectedPeopleIds.includes(person.Id))
                $scope.selectedPeopleIds = $scope.selectedPeopleIds.filter(function(id) {
                    return id !== person.Id;
                });
            else $scope.selectedPeopleIds = $scope.selectedPeopleIds.concat([person.Id]);
            console.log($scope.selectedPeopleIds);
        };
        $scope.isSelectedPerson = function(person) {
            return $scope.selectedPeopleIds.includes(person.Id);
        };
        $scope.toggleSelectedRole = function(roleId) {
            if ($scope.selectedRoleIds.includes(roleId))
                $scope.selectedRoleIds = $scope.selectedRoleIds.filter(function(id) {
                    return id !== roleId;
                });
            else $scope.selectedRoleIds = $scope.selectedRoleIds.concat([roleId]);
        };
        $scope.isSelectedRole = function(roleId) {
            return $scope.selectedRoleIds.includes(roleId);
        };
        $scope.getPerson = function(id) {
            return $scope.people.find(function(person) {
                return person.Id === id;
            });
        };
        $scope.getRole = function(id) {
            return $scope.roles.find(function(role) {
                return role.Id === id;
            });
        };
        $scope.$watch('selectedPeopleIds', function() {
            $scope.selectedPeople = $scope.people.filter(function(person) {
                return $scope.selectedPeopleIds.includes(person.Id);
            });
            var roleIds = {};
            $scope.selectedPeople.forEach(function(person) {
                person.Roles.forEach(function(role) {
                    return (roleIds[role.Id] = null);
                });
            });
        ...
        $scope.clearSelectedPeople = function() {
            return ($scope.selectedPeopleIds.length = 0);
        };
        $scope.clearSelectedRoles = function() {
            return ($scope.selectedRoleIds.length = 0);
        };
