'use strict';
describe('Requests - overtime controller tests',
	function () {
		var $rootScope,
			$filter,
			$compile,
		...
		it('should list overtime types', function () {
			...
			expect(results.some(function (item) {
				return angular.element(item).html().indexOf('overtime paid');
			})).toBeTruthy();

			expect(results.some(function (item) {
				return angular.element(item).html().indexOf('overtime time');
			})).toBeTruthy();
		});
			...
			this.getOvertimeTypes = function () {
				return {
					then: function (cb) {
						cb({
							data: [
								{ Id: '00', Name: 'overtime paid' },
								{ Id: '01', Name: 'overtime time' }
							]
						});
					}
				}
