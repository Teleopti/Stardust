	...
	describe('<requests-shift-detail>', function () {
		...
		beforeEach(function () {
			...
			module(function ($provide) {
				...
				$provide.service('CurrentUserInfo', function () { return fakeCurrentUserInfo; });
			});
		});
		...
		function setUp(personIds, date, targetTimezone, showShiftDetail) {
			...
			var element = $compile('<div requests-shift-detail date="date" target-timezone="targetTimezone" person-ids="personIds" show-shift-detail="showShiftDetail(params)"></div>')(
				scope
			);
			scope.$apply();
