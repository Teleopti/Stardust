...
describe('PermissionsController', function() {
	...
	describe('PermissionsController - filters', function() {
		...
		xit('should show correct selected org data when switching role after filtering', function() {
	...
	inject(function($filter) {
		...
		vm.nodeClick(vm.filteredOrganizationSelection.BusinessUnit.ChildNodes[0].ChildNodes[0]);
		...
		vm.selectRole(vm.roles[0]);
		$httpBackend.flush();
		expect(vm.selectedOrgData['3333cfb-6f7d-4128-a014-0c8f82209d68']).toEqual(true);
	});
});

	xit('should show correct selected org data when unselecting top node while filtering', function() {
		$httpBackend.whenPOST('../api/Permissions/Roles/e7f360d3-c4b6-41fc-9b2d-9b5e015aae64/AvailableData').respond(function(method, url, data, headers) {
		...
		inject(function($filter) {
			...
			vm.nodeClick(vm.filteredOrganizationSelection.BusinessUnit);
			...
			expect(vm.selectedOrgData['111dd0bc-bf40-412e-b970-9b5e015aadea']).toEqual(undefined);
			expect(vm.selectedOrgData['22225804-7c51-4f43-a1ae-ecb2a17a2177']).toEqual(undefined);
			expect(vm.selectedOrgData['3333cfb-6f7d-4128-a014-0c8f82209d68']).toEqual(undefined);
	});
});
	});
});
