	...
	angular.module('wfm.rta').service('RtaGridService',
		function() {
			...
			this.createAgentsGridOptions = function() {
				...
				var rowTemplate = '<div ng-click="checked=!checked" class="agent-state"><div ng-class="{testclass:checked}" style=" background-color: {{grid.appScope.hexToRgb(row.entity.AlarmColor)}} !important;" ng-repeat="(colRenderIndex, col) in colContainer.renderedColumns track by col.colDef.name" class="ui-grid-cell agent-row" ng-attr-agentid="{{row.entity.PersonId}}" ng-click="grid.appScope.getAdherenceForAgent(row.entity.PersonId)" ng-class="{ \'ui-grid-row-header-cell\': col.isRowHeader }" ui-grid-cell></div></div><div ng-show="checked"><div class="agent-menu historical-adherence"><a href="{{grid.appScope.changeScheduleUrl(row.entity.TeamId, row.entity.PersonId)}}" class="change-schedule">Change schedule</a><a href="{{grid.appScope.agentDetailsUrl(row.entity.PersonId)}}">Agent adherence: {{grid.appScope.adherencePercent}}% Latest updated: {{grid.appScope.timeStamp}}</a></div></div>';
				return {
