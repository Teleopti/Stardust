	...
	public class TeamScheduleController : ApiController
	{
		...
		public virtual JsonResult<PermissionsViewModel> GetPermissions()
		{
			var permissions = new PermissionsViewModel
			{
				...
				HasSaveFavoriteSearchPermission = _authorization.IsPermitted(DefinedRaptorApplicationFunctionPaths.SaveFavoriteSearch)
			};
		...
		public virtual JsonResult<GroupScheduleViewModel> SearchSchedules(SearchDaySchedulesFormData input)
		{
			...
			var result =
				_teamScheduleViewModelFactory.CreateViewModel(input.SelectedTeamIds ?? new Guid[] { },criteriaDictionary,currentDate,input.PageSize,input.CurrentPageIndex,input.IsOnlyAbsences);
