$(document).ready(function() {
		...
		for (var i = 0; i < (24 * 60) / intervalLengthInMinute; i++) {
			result.push({
				...
				StartTime: moment(formattedDate)
					.startOf('day')
					.add(intervalLengthInMinute * i, 'minutes')
					.format('YYYY-MM-DDTHH:mm:ss'),
				EndTime: moment(formattedDate)
					.startOf('day')
					.add(intervalLengthInMinute * (i + 1), 'minutes')
					.format('YYYY-MM-DDTHH:mm:ss'),
	...
	test('should read date', function() {
		var weekViewModel = new Teleopti.MyTimeWeb.Schedule.MobileWeekViewModel(
			fakeUserText,
			null,
			null,
			stopFetchProbabilityData
		);
		var vm = new Teleopti.MyTimeWeb.Schedule.MobileDayViewModel(
			getFakeScheduleData().Days[0],
			false,
			true,
			weekViewModel
		);
