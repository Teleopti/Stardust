...
namespace Teleopti.Ccc.WebTest.Areas.Anywhere.Hubs
{
	...
	{
		[Test]
		public void ShouldConvertToUsersTimeZone()
		{
			var groupScheduleProvider = MockRepository.GenerateMock<IGroupScheduleViewModelFactory>();
			var groupId = Guid.NewGuid();
			var dateTime = new DateTime(2013, 3, 4, 0, 0, 0, DateTimeKind.Unspecified);
			var userTimeZone = MockRepository.GenerateMock<IUserTimeZone>();
			userTimeZone.Stub(x => x.TimeZone()).Return(TimeZoneInfoFactory.SingaporeTimeZoneInfo());
			var target = new GroupScheduleHub(groupScheduleProvider, userTimeZone);
			var hubBuilder = new TestHubBuilder();
			hubBuilder.SetupHub(target, hubBuilder.FakeClient<IEnumerable<dynamic>>("incomingGroupSchedule", a => { }));

			target.SubscribeGroupSchedule(groupId, dateTime);

			groupScheduleProvider.AssertWasCalled(
				x => x.CreateViewModel(groupId, TimeZoneInfo.ConvertTime(dateTime, userTimeZone.TimeZone(), TimeZoneInfo.Utc)));
		}
