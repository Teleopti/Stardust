...
namespace Teleopti.Ccc.WebTest.Core
{
	[MyTimeWebTest]
	[TestFixture]
	public class WeekScheduleDomainDataProviderDayScheduleNoMockTest : IIsolateSystem
	{
		...
		public void ShouldMapScheduleDayOnGetDaySchedule()
		{
			...
			var result = Target.GetDaySchedule(date);
			result.ScheduleDay.ScheduleDay.PersonAssignment().Period.StartDateTime.Should().Be(date.Date.AddHours(1));
			result.ScheduleDay.ScheduleDay.PersonAssignment().Period.EndDateTime.Should().Be(date.Date.AddHours(2));
		}
		...
		public void ShouldMapProjectionOnGetDaySchedule()
		{
			...
			var result = Target.GetDaySchedule(date);
			result.ScheduleDay.Projection.ElementAt(0).Period.StartDateTime.Should().Be(date.Date.AddHours(1));
			result.ScheduleDay.Projection.ElementAt(0).Period.EndDateTime.Should().Be(date.Date.AddHours(2));
		}
		...
		public void ShouldMapProjectionIncludingTheDayBeforeCurrentWeekOnGetDaySchedule()
		{
			...
			result.ScheduleDay.ProjectionYesterday.Period().Value.StartDateTime.Should().Be(date.AddDays(-1).Date.AddHours(2));
			result.ScheduleDay.ProjectionYesterday.Period().Value.EndDateTime.Should().Be(date.AddDays(-1).Date.AddHours(3));
		}
		...
		public void ShouldMapOvertimeAvailabilityOnGetDaySchedule()
		{
			...
			result.ScheduleDay.OvertimeAvailability.Should().Be(overtimeAvailability);
		}
		...
		public void ShouldMapClassOnGetDaySchedule()
		{
			...
			var result = Target.GetDaySchedule(date);
			result.ScheduleDay.ProbabilityClass.Should().Be.EqualTo("red");
		}
		...
		public void ShouldMapTextOnGetDaySchedule()
		{
			...
			var result = Target.GetDaySchedule(date);
			result.ScheduleDay.ProbabilityText.Should().Be.EqualTo("Poor");
		}
		...
		public void ShouldMapAvailabilityOnGetDaySchedule()
		{
			...
			var result = Target.GetDaySchedule(date);
			result.ScheduleDay.Availability.Should().Be.EqualTo(true);
		}
		...
		{
			var date = DateOnly.Today;
			addPersonSchedule(date, TimeSpan.FromHours(1), TimeSpan.FromHours(3));
			var result = Target.GetDaySchedule(date);

			result.ColorSource.Projections.Single().ElementAt(0).Period.StartDateTime.Should().Be(date.Date.AddHours(1));
			result.ColorSource.Projections.Single().ElementAt(0).Period.EndDateTime.Should().Be(date.Date.AddHours(3));
		}
