...
namespace Teleopti.Ccc.WinCode.Backlog
{
	public class BacklogModel
	{
		...
		private DateOnly _productPlanStart;
		...
		public DateOnly ProductPlanStart
		{
			get { return _productPlanStart; }
		}
		...
		public void Load(BackgroundWorker backgroundWorker, DateOnly productPlanStart)
		{
			...
			_productPlanStart = productPlanStart;
					...
					if (taskOnDate.StartDate < productPlanStart && nextTask.StartDate >= productPlanStart)
						nextTask.BacklogProductPlanTask.TransferedBacklog =
							nextTask.BacklogProductPlanTask.TransferedBacklog.Add(
								taskOnDate.BacklogScheduledTask.ScheduledBacklogTimeOnTask(_productPlanStart));
				}
			}
		}
		...
		private void setScheduledTime(DateOnlyPeriod period)
		{
			foreach (var dateOnly in period.DayCollection())
			{
				foreach (var skillDay in _stateHolder.SchedulingResultState.SkillDaysOnDateOnly(new List<DateOnly> {dateOnly}))
				{
					...
					TimeSpan time;
					time = TimeSpan.FromHours(SkillStaffPeriodHelper.ScheduledHours(skillDay.SkillStaffPeriodCollection).GetValueOrDefault(0));					
		...
		public TimeSpan? GetScheduledTimeOnIndex(int colIndex, ISkill skill)
		{
			var date = _stateHolder.RequestedPeriod.DateOnlyPeriod.DayCollection()[colIndex - 1];
			
			var skillDay = _stateHolder.SchedulingResultState.SkillDayOnSkillAndDateOnly(_skillPairsEmailBackOffice[skill], date);

			return
				TimeSpan.FromHours(SkillStaffPeriodHelper.ScheduledHours(skillDay.SkillStaffPeriodCollection).GetValueOrDefault(0));
		}

		public double GetScheduledWorkOnIndex(int colIndex, ISkill skill)
		{
			return -1;
		}
		...
		private void createBacklogTasks(DateOnlyPeriod period)
		{
			...
			foreach (var date in period.DayCollection())
			{
				foreach (var skillDay in _stateHolder.SchedulingResultState.SkillDaysOnDateOnly(new List<DateOnly>{date}))
				{
					...
					var task = new BacklogTask(skillDay.TotalTasks, skillDay.TotalAverageTaskTime, skillDay.CurrentDate, serviceLevel);
		...
		public TimeSpan? GetIncomingTimeForIndex(int colIndex, ISkill skill)
		{
			var date = _stateHolder.RequestedPeriod.DateOnlyPeriod.DayCollection()[colIndex-1];
			if (!_taskDic[skill].ContainsKey(date))
				return TimeSpan.Zero;

			return _taskDic[skill][date].IncomingTime;
		}

		public double GetIncomingWorkForIndex(int colIndex, ISkill skill)
		{
			var date = _stateHolder.RequestedPeriod.DateOnlyPeriod.DayCollection()[colIndex - 1];
			if (!_taskDic[skill].ContainsKey(date))
				return 0;

			return _taskDic[skill][date].IncomingWork;
		}
		...
		public TimeSpan? GetPlannedOverstaffingTimeForIndex(int colIndex, ISkill skill)
		{
			var date = _stateHolder.RequestedPeriod.DateOnlyPeriod.DayCollection()[colIndex - 1];
			if (!_taskDic[skill].ContainsKey(date))
				return TimeSpan.Zero;
		...
		public TimeSpan GetForecastedBacklogForIndex(int colIndex, ISkill skill)
		{
			...
			{
				time = time.Add(task.BacklogProductPlanTask.PlannedBacklogTimeOnDate(date, _productPlanStart));
			}
		...
		public bool IsClosedOnIndex(int colIndex, ISkill skill)
		{
			...
			return affectedTasks.First().Value.ClosedDays.Contains(date);
		}
		...
		{
			var date = _stateHolder.RequestedPeriod.DateOnlyPeriod.DayCollection()[colIndex - 1];
			SetManualEntryOnDate(date, skill, time);
		}
		...
		private TimeSpan setManualEntryOnBacklogTasks(ISkill skill, TimeSpan time, DateOnly date)
		{
			...
			{
				var maxTime = affectedTask.BacklogProductPlanTask.PlannedBacklogTimeOnDate(date.AddDays(-1),_productPlanStart);
				if (affectedTask.StartDate == date)
					maxTime = affectedTask.BacklogProductPlanTask.TotalIncomingTime();
			...
			affectedTasks.Values.Last().BacklogProductPlanTask.OverStaff = TimeSpan.Zero;
			if(time > bookedTime)
				affectedTasks.Values.Last().BacklogProductPlanTask.OverStaff = time.Subtract(bookedTime);
		...
		private void setScheduledTimeOnBacklogTasks(ISkill skill, TimeSpan time, DateOnly date)
		{
			...
			foreach (var affectedTask in affectedTasks.Values)
			{
				var maxTime = affectedTask.BacklogScheduledTask.ScheduledBackLogTimeOnDate(date);
				if (timeToBook <= maxTime)
				{
		...
		{
			ISkill targetSkill;
			if (!_skillPairsEmailBackOffice.TryGetValue(skill, out targetSkill))
				return;

			var dirtySkillDays = new List<ISkillDay>();
			foreach (var dateOnly in _stateHolder.RequestedPeriod.DateOnlyPeriod.DayCollection())
			{
				var skilldays = _stateHolder.SchedulingResultState.SkillDaysOnDateOnly(new List<DateOnly> { dateOnly });
		...
		public TimeSpan? GetScheduledTimeOnIncomingIndex(int colIndex, ISkill skill)
		{
			var date = _stateHolder.RequestedPeriod.DateOnlyPeriod.DayCollection()[colIndex - 1];
			if (!_taskDic[skill].ContainsKey(date))
				return TimeSpan.Zero;

			var task = _taskDic[skill][date];
			return task.BacklogScheduledTask.ScheduledTimeOnTask(_productPlanStart);
		}

		public double GetScheduledWorkOnIncomingIndex(int colIndex, ISkill skill)
		{
			var date = _stateHolder.RequestedPeriod.DateOnlyPeriod.DayCollection()[colIndex - 1];
			if (!_taskDic[skill].ContainsKey(date))
				return 0;
			...
			var task = _taskDic[skill][date];
			return task.BacklogScheduledTask.ScheduledWorkOnTask(_productPlanStart);
		}

		public TimeSpan? GetScheduledBacklogTimeOnIncomingIndex(int colIndex, ISkill skill)
		{
			var date = _stateHolder.RequestedPeriod.DateOnlyPeriod.DayCollection()[colIndex - 1];
			if (!_taskDic[skill].ContainsKey(date))
				return TimeSpan.Zero;

			return _taskDic[skill][date].BacklogScheduledTask.ScheduledBacklogTimeOnTask(_productPlanStart);
		}

		public double GetScheduledBacklogWorkOnIncomingIndex(int colIndex, ISkill skill)
		{
			var date = _stateHolder.RequestedPeriod.DateOnlyPeriod.DayCollection()[colIndex - 1];
			if (!_taskDic[skill].ContainsKey(date))
				return 0;

			return _taskDic[skill][date].BacklogScheduledTask.ScheduledBacklogWorkOnTask(_productPlanStart);
		}
		...
		public Percent GetScheduledServiceLevelOnIncomingIndex(int colIndex, ISkill skill)
		{
			...
			return _taskDic[skill][date].BacklogScheduledTask.ScheduledServiceLevelOnTask(_productPlanStart);
		}
		...
		public TimeSpan? GetScheduledBacklogTimeOnIndex(int colIndex, ISkill skill)
		{
			...
			foreach (var task in _taskDic[skill].Values)
			{
				time = time.Add(task.BacklogScheduledTask.ScheduledBackLogTimeOnDate(date));
			}

			return time;
		}

		public double GetScheduledBacklogWorkOnIndex(int colIndex, ISkill skill)
		{
			var date = _stateHolder.RequestedPeriod.DateOnlyPeriod.DayCollection()[colIndex - 1];
			var work = 0d;
			foreach (var task in _taskDic[skill].Values)
			{
				work += task.BacklogScheduledTask.ScheduledBacklogWorkOnDate(date);
			}

			return work;
		}

		public double GetAccumulatedPlannedBacklogWorkForIndex(int colIndex, ISkill skill)
		{
			var date = _stateHolder.RequestedPeriod.DateOnlyPeriod.DayCollection()[colIndex - 1];
			var work = 0d;
			foreach (var task in _taskDic[skill].Values)
			{
				work += task.BacklogProductPlanTask.PlannedBacklogWorkOnDate(date, _productPlanStart);
			}

			return work;
		}

		public TimeSpan? GetAccumulatedPlannedBacklogTimeForIndex(int colIndex, ISkill skill)
		{
			var date = _stateHolder.RequestedPeriod.DateOnlyPeriod.DayCollection()[colIndex - 1];
			var time = TimeSpan.Zero;
			foreach (var task in _taskDic[skill].Values)
		...
		public void SetProductPlanStart(DateOnly date)
		{
			_productPlanStart = date;
		}
