...
namespace Teleopti.Ccc.WinCode.Common.ExceptionHandling
{
	...
	public interface ISomethingFromRoger
	{
		IDictionary<Toggles, bool> Features { get; }  	
	}
    ...
    public class ExceptionHandlerModel
    {
	    ...
	    private readonly ISomethingFromRoger _somethingFromRoger;
	    public const string ToggleFeaturesUnknown = "ToggleFeatures unknown";

	    public ExceptionHandlerModel(Exception exception, string defaultEmail, IMapiMailMessage mapiMessage, IWriteToFile fileWriter,ISomethingFromRoger somethingFromRoger)
        {
            _exception = exception;
            _defaultEmail = defaultEmail;
            _mapiMessage = mapiMessage;
            _fileWriter = fileWriter;
	        _somethingFromRoger = somethingFromRoger;
        }
        ...
        public string CompleteStackAndAssemblyText()
        {
	        ...
	        return extractFeatureInfo(exceptionInfoText,_somethingFromRoger);
        }

		private static string extractFeatureInfo(string exceptionText, ISomethingFromRoger somethingFromRoger)
	    {
			var text = new StringBuilder(exceptionText);
		    try
		    {
			    foreach (var entry in somethingFromRoger.Features)
			    {
