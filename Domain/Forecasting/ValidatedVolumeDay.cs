...
namespace Teleopti.Ccc.Domain.Forecasting
{
    ...
    public class ValidatedVolumeDay : VersionedAggregateRootWithBusinessUnit, IValidatedVolumeDay
    {
        ...
        /// </remarks>
        public virtual void AddPercentageToTasks(Percent percent)
        {
            double value = getValue(percent);

            _validatedTasks *= value;
        }

        private static double getValue(Percent percent)
        {
            double value;
            if (percent.Value >= 0)
                value = percent.Value + 1d;
            else
                value = 1d + percent.Value;
            return value;
        }
        ...
        /// Created date: 2008-06-27
        /// </remarks>
        public virtual void AddPercentageToValidatedAfterTaskTime(Percent percent)
        {
            validatedAfterTaskTime = TimeSpan.FromMilliseconds(validatedAfterTaskTime.Value.TotalMilliseconds * (getValue(percent)));
        }
