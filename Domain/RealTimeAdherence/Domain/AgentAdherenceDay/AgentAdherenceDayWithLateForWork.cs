...
namespace Teleopti.Ccc.Domain.RealTimeAdherence.Domain.AgentAdherenceDay
{
	public class AgentAdherenceDayWithLateForWork : IAgentAdherenceDay
	{
		...
		public void Apply(PersonArrivedLateForWorkEvent @event)
		{
			...
			if (model != null)
				model.LateForWork = string.Format(UserTexts.Resources.LateXMinutes, Math.Floor(new DateTimePeriod(@event.ShiftStart, @event.Timestamp).ElapsedTime().TotalMinutes));
		}
