...
namespace Teleopti.Ccc.Domain.Optimization.WeeklyRestSolver
{
	...
	public class ShiftNudgeEarlier : IShiftNudgeEarlier
	{
		...
		public ShiftNudgeEarlier(ITeamBlockClearer teamBlockClearer, ITeamBlockRestrictionAggregator teamBlockRestrictionAggregator, ITeamBlockScheduler teamBlockScheduler)
		{
			...
			DateOnlyPeriod selectedPeriod, IList<IPerson> selectedPersons)
		{
			...
			if(!result)
			{
				//this does not work for team and or block need to think how to solve
				//rollbackService.RollbackLast();
				//resourceCalculateDelayer.CalculateIfNeeded(shiftDate, null);
			}

			//rollback if breaking max% of restrictions
