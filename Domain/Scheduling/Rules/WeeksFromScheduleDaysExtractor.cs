...
namespace Teleopti.Ccc.Domain.Scheduling.Rules
{
    public interface IWeeksFromScheduleDaysExtractor
    {
        ...
        IEnumerable<PersonWeek> CreateWeeksFromScheduleDaysExtractor(IEnumerable<IScheduleDay> scheduleDays,
                                                                     bool
                                                                         addWeekBeforeAndAfterIfFirstOrLastDayInWeekIsIncluded);
    }

    public class WeeksFromScheduleDaysExtractor : IWeeksFromScheduleDaysExtractor
    {
        public IEnumerable<PersonWeek> CreateWeeksFromScheduleDaysExtractor(IEnumerable<IScheduleDay> scheduleDays, bool addWeekBeforeAndAfterIfFirstOrLastDayInWeekIsIncluded)
        {
            ...
            foreach (IScheduleDay day in scheduleDays)
            {
                ...
                if (addWeekBeforeAndAfterIfFirstOrLastDayInWeekIsIncluded && dayDateOnly.Equals(firstDateInPeriodLocal))
                {
                    ...
                    personWeeks.Add(new PersonWeek(day.Person,
                                                   new DateOnlyPeriod(firstDateInPeriodLocal.AddDays(-7),
                                                                      firstDateInPeriodLocal.AddDays(-1))));
                }
                ...
                if (addWeekBeforeAndAfterIfFirstOrLastDayInWeekIsIncluded && dayDateOnly.Equals(period.EndDate))
                {
                    ...
                    if (personPeriod == null)
                        continue;
                    personWeeks.Add(new PersonWeek(day.Person,
                                                   new DateOnlyPeriod(period.EndDate.AddDays(1),
                                                                      period.EndDate.AddDays(7))));
                }
            }
