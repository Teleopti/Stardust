...
namespace Teleopti.Ccc.Domain.Common.TimeLogger
{
	...
	{
		private const string output = "Method '{0}' on type '{1}' took {2} seconds to run.";
		...
		public void OnAfterInvocation(Exception exception, IInvocationInfo invocation)
		{
			_logger.GetLogger(loggerName)
				.DebugFormat(output, invocation.Method.Name, invocation.Method.DeclaringType, _stopwatch.Elapsed.TotalSeconds);
		}
	}
}