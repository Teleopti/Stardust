...
namespace Teleopti.Ccc.Domain.ApplicationLayer.AbsenceRequests
{
		...
		IRunOnStardust
	{
		...
		public MultiAbsenceRequestsHandlerRobustToggleOn(IPersonRequestRepository personRequestRepository,
		...
		private List<IPersonRequest> checkPersonRequest(NewMultiAbsenceRequestsCreatedEvent @event)
		{
			...
			if (wfcs.Any(x => x.AbsenceRequestWaitlistEnabled))
			{
				...
				_feedback.SendProgress($"Period after inflating using queuedRequests: {min} - {max}. Number of queuedRequest:{queuedRequests.Count}");
			}
		...
		IRunOnStardust
	{
		...
		public MultiAbsenceRequestsHandlerRobustToggleOff(IPersonRequestRepository personRequestRepository,
			ICurrentUnitOfWorkFactory currentUnitOfWorkFactory,
			IStardustJobFeedback stardustJobFeedback, IWorkflowControlSetRepository workflowControlSetRepository,
			IQueuedAbsenceRequestRepository queuedAbsenceRequestRepository, IMultiAbsenceRequestsUpdater multiAbsenceRequestsUpdater,
			IAbsenceRequestValidatorProvider absenceRequestValidatorProvider, IFilterRequests filterRequests, IAbsenceRequestWaitlistProvider absenceRequestWaitlistProvider)
		...
		private List<IPersonRequest> checkPersonRequest(NewMultiAbsenceRequestsCreatedEvent @event)
		{
			...
			{
				atleastOneWaitlistedWfc = true;
				var truncatedSent = @event.Sent.Truncate(TimeSpan.FromSeconds(1));

				var queuedRequests = _queuedAbsenceRequestRepository.LoadAll().Where(x => x.Sent.HasValue && DateTime.Compare(x.Sent.Value, truncatedSent) == 0);
				foreach (var request in queuedRequests)
				{
					if (request.StartDateTime < min)
						min = request.StartDateTime;
				...
				_feedback.SendProgress($"Number of queuedRequest:{queuedRequests.Count()} with sent value:{@event.Sent} (truncated sent: {truncatedSent})");
				_feedback.SendProgress($"Period after inflating using queuedRequests: {min} - {max}");
			}
