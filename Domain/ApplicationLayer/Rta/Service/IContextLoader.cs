...
{
	public class StateAndSchedule
	{
		public AgentState AgentState { get; set; }
		public IList<ScheduledActivity> Schedule { get; set; }
	}

	public interface IDatabaseOptimizer
	{
		StateAndSchedule LoadFor(Guid personId);
	}
	...
	public class OptimizedContextLoader : IContextLoader
	{
		...
		private readonly IDatabaseOptimizer _optimizer;

		public OptimizedContextLoader(
		...
		public virtual void For(ExternalUserStateInputModel input, Action<Context> action)
		{
			...
			WithUnitOfWork(() =>
			{
				_databaseReader.LoadPersonOrganizationData(dataSourceId, userCode)
					.ForEach(x =>
					{
						var stateAndSchedule = _optimizer.LoadFor(x.PersonId);
						...
						action.Invoke(new Context(
							...
							x.TeamId,
							x.SiteId,
							() => stateAndSchedule.AgentState,
							() => stateAndSchedule.Schedule,
							s =>
							{
