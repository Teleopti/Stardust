	...
	public class OrganizationViewModelBuilder
	{
				...
				.Where(
					s =>
						_authorization.Current()
							.IsPermitted(DefinedRaptorApplicationFunctionPaths.RealTimeAdherenceOverview, _now.ServerDate_DontUse(), s))
		...
		public IEnumerable<OrganizationSiteViewModel> BuildForSkills(Guid[] skillIds)
		{
			...
			var org = from num in numberOfAgents
					  from site in sites
					  let teamIdsBySkill = site.TeamCollection.Any() ? _numberOfAgentsInTeamReader.Read(site.TeamCollection.Select(t => t.Id.Value), skillIds) : new Dictionary<Guid, int>() 
