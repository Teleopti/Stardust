...
{
	public class WorkShiftCategorizeFinderService : IWorkShiftFinderService
	{
		...
		public WorkShiftCategorizeFinderService(Func<ISchedulingResultStateHolder> resultStateHolder, Func<IPreSchedulingStatusChecker> preSchedulingStatusChecker
		...
		{
			if (schedulingOptions.IsOptimization)
				return fullShiftSelection(shiftProjectionCaches, dataHolders, nonBlendSkillPeriods, currentSchedulePeriod,
					schedulingOptions, workShiftFinderResult);

			return categorizedShiftSelection(shiftProjectionCaches, dataHolders, nonBlendSkillPeriods, currentSchedulePeriod,
					schedulingOptions, workShiftFinderResult);
		}

		private IWorkShiftCalculationResultHolder fullShiftSelection(IList<ShiftProjectionCache> shiftProjectionCaches,
			IWorkShiftCalculatorSkillStaffPeriodData dataHolders, IDictionary<ISkill, ISkillStaffPeriodDictionary> nonBlendSkillPeriods,
			IVirtualSchedulePeriod currentSchedulePeriod, SchedulingOptions schedulingOptions,
			IWorkShiftFinderResult workShiftFinderResult)
		{
			...
			foundValues =
				WorkShiftCalculator.CalculateListForBestImprovementAfterAssignment(foundValues, dataHolders)
		...
		private IWorkShiftCalculationResultHolder categorizedShiftSelection(IList<ShiftProjectionCache> shiftProjectionCaches,
			IWorkShiftCalculatorSkillStaffPeriodData dataHolders, IDictionary<ISkill, ISkillStaffPeriodDictionary> nonBlendSkillPeriods,
			IVirtualSchedulePeriod currentSchedulePeriod, SchedulingOptions schedulingOptions,
			IWorkShiftFinderResult workShiftFinderResult)
		{
			...
			allValues = _workShiftCalculatorsManager.RunCalculators(person,
				...
				nonBlendSkillPeriods,
				schedulingOptions);
			foundValues = _fairnessAndMaxSeatCalculatorsManager.RecalculateFoundValues(allValues).ToArray();
			foundValues =
				WorkShiftCalculator.CalculateListForBestImprovementAfterAssignment(foundValues, dataHolders)
					.OfType<IWorkShiftCalculationResultHolder>()
					.ToArray();
