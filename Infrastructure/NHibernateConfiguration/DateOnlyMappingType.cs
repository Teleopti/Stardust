...
namespace Teleopti.Ccc.Infrastructure.NHibernateConfiguration
{
    ...
    public class DateOnlyMappingType : IUserType
    {
        ...
        {
            string text;
            object value = rs[rs.GetOrdinal(names[0])];

            if (value == DBNull.Value)
                return null;

            text = value.ToString();
            var parsedDateTime = DateTime.Parse(text, CultureInfo.CurrentCulture);
            if (parsedDateTime < DateOnly.MinValue)
                parsedDateTime = DateOnly.MinValue.Date;
            DateOnly dateOnly = new DateOnly(parsedDateTime);
            return dateOnly;
        }
        ...
        public void NullSafeSet(IDbCommand cmd, object value, int index)
        {
            if (value != null)
            {
                ...
                if (str.Length > 0)
                {
                    IDataParameter dataParameter = (IDataParameter) cmd.Parameters[index];
                    dataParameter.Value = str;
                    return;
                }
            }
