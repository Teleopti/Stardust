...
namespace Teleopti.Ccc.Infrastructure.NHibernateConfiguration.TransientErrorHandling
{
	...
	public class ResilientSqlClientBatchingBatcher : SqlClientBatchingBatcher
	{
		...
		public ResilientSqlClientBatchingBatcher(ConnectionManager connectionManager, IInterceptor interceptor)
		...
		public override void AddToBatch(IExpectation expectation)
		{
			...
			#endregion
			_currentBatch.Append((System.Data.SqlClient.SqlCommand)(ResilientSqlCommand)batchUpdate);
			#region NHibernate code
		...
		protected new void Prepare(DbCommand cmd)
		{
			try
			{
				var sessionConnection = (ResilientSqlDbConnection)_connectionManager.GetConnection();
		...
		{
			var connection = (ResilientSqlDbConnection)_connectionManager.GetConnection();
			ResilientAdoTransaction.ExecuteWithRetry(connection, () => ExecuteBatch(ps));
		}
	}
}