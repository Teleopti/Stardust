...
namespace Teleopti.Analytics.Etl.TransformerTest
{
	...
	public class OvertimeTransformerTest
	{
		...
		public void Setup()
		{
			...
			OvertimeTransformer multiplicatorDefinitionSetTransformer = new OvertimeTransformer(_insertDateTime);
		...
		{
			//BusinessUnit
			Assert.AreEqual(_multiplicatorDefinitionSetCollection[0].BusinessUnit.Id, _table.Rows[0]["business_unit_code"]);
			Assert.AreEqual(_multiplicatorDefinitionSetCollection[1].BusinessUnit.Description.Name, _table.Rows[1]["business_unit_name"]);
			//UpdatedOn
