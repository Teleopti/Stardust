    ...
    public class CommonStateHolderTest
    {
	    ...
	    public void GetSchedulePartPerPersonAndDateShouldNotReturnSchedulePartAfterLeavingDate()
	    {
			...
			using (_mocks.Record())
			{
				...
				Expect.Call(range1.ScheduledDayCollection(new DateOnlyPeriod(2015, 3, 27,2015,3,28))).Return(new [] { scheduleDay1,scheduleDay1});

				//only one day here
				Expect.Call(scheduleDictionary[p2]).Return(range1);
				Expect.Call(range1.ScheduledDayCollection(new DateOnlyPeriod(2015, 3, 27, 2015, 3, 27))).Return(new[] { scheduleDay1 });
