...
namespace Teleopti.Analytics.Portal.PerformanceManager.ViewModel
{
	public class AnalyzerReportViewModel
	{
		...
		{
			PermissionLevel userPermissions = PermissionLevel.None;

			userPermissions = PermissionInformation.IsPmAuthenticationWindows 
				? PermissionInformation.UserPermissions 
				: AuthorizationHelper.CurrentUserPmPermission(AuthorizationHelper.LoggedOnUser);
