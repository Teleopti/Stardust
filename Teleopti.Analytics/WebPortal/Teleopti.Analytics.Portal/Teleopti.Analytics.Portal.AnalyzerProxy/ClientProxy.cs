...
namespace Teleopti.Analytics.Portal.AnalyzerProxy
{
	public class ClientProxy : IClientProxy, IDisposable
	{
		...
		private const int rootNodeId = -1227456569;
		private const int shareReportId = 624687020;
		...
		public bool EnsureDataSourceExists()
		{
			...
			{
				ServerName = _olapServer, 
				InitialCatalog = _olapDatabase, 
				Name = _olapServer
			};
		...
		{
			var userPermissionsOut = PermissionLevel.None;
			var userPermissionsDataSource = PermissionLevel.None;
		...
		public bool SetDefaultRolePermission(int generalUserRoleId, int reportDesignerRoleId)
		{
			...
			if (catalogItem != null && (catalogItem.ParentId == 0) && catalogItem.Success)
			{
				...
				{
					MemberType = SecurityMemberType.Role,
					MemberId = generalUserRoleId,
					ObjectId = rootNodeId,
					Permission = (int) PermissionLevel.GeneralUser
				};
				...
				{
					MemberType = SecurityMemberType.Role,
					MemberId = reportDesignerRoleId,
					ObjectId = rootNodeId,
					Permission = (int) PermissionLevel.ReportDesigner
				};
