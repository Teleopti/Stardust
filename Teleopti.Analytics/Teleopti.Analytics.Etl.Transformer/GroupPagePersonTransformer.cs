...
namespace Teleopti.Analytics.Etl.Transformer
{
    public class GroupPagePersonTransformer : IGroupPagePersonTransformer
    {
        ...
        public IList<IGroupPage> BuiltInGroupPages
        {
            get
            {
                GroupingsCreator groupingsCreator = new GroupingsCreator(_getGroupPageDataProvider());
        ...
        private static void BuildGroupPagePersonRows(IEnumerable<IGroupPage> groupPageCollection, DataTable dataTable)
        {
            ...
            {
                GroupPage gpCasted = groupPage as GroupPage;
                if (gpCasted == null)
                ...
                foreach (IRootPersonGroup rootGroup in groupPage.RootGroupCollection)
                {
                    foreach (IPerson person in rootGroup.PersonCollection)
                    {
        ...
        {
            List<IChildPersonGroup> groupsToRemove = new List<IChildPersonGroup>();
