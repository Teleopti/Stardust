	...
	public class JobResultProvider : IJobResultProvider
	{
				...
				return
					jobResult.Select(
						...
						new JobResultModel
						{
							...
							Timestamp = TimeZoneHelper.ConvertFromUtc(j.Timestamp, TeleoptiPrincipalForLegacy.CurrentPrincipal.Regional.TimeZone).ToString()
						}).ToList();
			}
		}
		...
		public IList<JobResultDetailModel> GetJobResultDetails(JobResultModel jobResultModel, int detailLevel)
        {
	        using (_unitOfWorkFactory.CreateAndOpenUnitOfWork())
            {
				...
				var timeZone = TeleoptiPrincipalForLegacy.CurrentPrincipal.Regional.TimeZone;
