...
namespace Teleopti.Ccc.SmartClientPortal.Shell.WinCode.Grouping.Commands
{
    ...
    public class LoadOrganizationCommand : ILoadOrganizationCommand, IDisposable
    {
		...
		public void Execute()
        {
            ...
            var root = new TreeNodeAdv(((ITeleoptiIdentity)TeleoptiPrincipalForLegacy.CurrentPrincipal.Identity).BusinessUnit.Name) { LeftImageIndices = new[] { 0 }, Expanded = true, Tag = new List<Guid>(), TagObject = new List<Guid>() };
            nodes.Add(root);
