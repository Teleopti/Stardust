	...
	public class ScheduleReportGraphicalDrawTimeline
	{
		...
		public float Draw(IList<DateOnly> dates, ISchedulingResultStateHolder stateHolder, IList<IPerson> persons, DateOnly day)
		{
			...
			for (var i = 0; i <= totalHours; i++)
			{
				...
				if (_rightToLeft)
					timeString = TimeZoneHelper.ConvertFromUtc(timelinePeriod.EndDateTime.AddHours(-i), TeleoptiPrincipalForLegacy.CurrentPrincipal.Regional.TimeZone).ToShortTimeString();
				else
					timeString = TimeZoneHelper.ConvertFromUtc(timelinePeriod.StartDateTime.AddHours(i), TeleoptiPrincipalForLegacy.CurrentPrincipal.Regional.TimeZone).ToShortTimeString();
        ...
        public DateTimePeriod TimelinePeriod(IList<DateOnly> dates, ISchedulingResultStateHolder stateHolder, IList<IPerson> persons, DateOnly day)
        {
            ...
            {
                startDateTime = TimeZoneHelper.ConvertToUtc(day.Date.AddHours(8), TeleoptiPrincipalForLegacy.CurrentPrincipal.Regional.TimeZone);
                endDateTime = TimeZoneHelper.ConvertToUtc(day.Date.AddHours(17), TeleoptiPrincipalForLegacy.CurrentPrincipal.Regional.TimeZone);
            }
