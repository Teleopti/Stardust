	...
	public static class ViewBaseHelper
	{
		...
		public static string GetToolTipAssignments(IScheduleDay scheduleDay)
		{
			...
			if (pa != null)
			{
				...
				{
					sb.Append(ToLocalStartEndTimeString(projectionPeriod.Value, TimeZoneGuardForDesktop.Instance.CurrentTimeZone())); //time
				}
				...
				foreach (var layer in pa.PersonalActivities())
				{
					...
					//name
					sb.Append(": ");
					sb.Append(ToLocalStartEndTimeString(layer.Period, TimeZoneGuardForDesktop.Instance.CurrentTimeZone())); //time
				}
			}
			return sb.ToString();
		}
		...
		public static string GetToolTipAbsences(IScheduleDay cell)
		{
			...
			if (abses.Count > 0)
			{
				foreach (IPersonAbsence pa in abses)
				{
					...
					sb.Append(": ");
					sb.Append(ToLocalStartEndTimeStringAbsences(cell.Period, pa.Layer.Period, TimeZoneGuardForDesktop.Instance.CurrentTimeZone()));
				}
			}
		...
		public static string GetToolTipMeetings(IScheduleDay cell)
		{
			...
			foreach (IPersonMeeting personMeeting in cell.PersonMeetingCollection())
			{
				...
				sb.Append(": ");
				sb.Append(ToLocalStartEndTimeString(personMeeting.Period, TimeZoneGuardForDesktop.Instance.CurrentTimeZone()));
		...
		public static string GetToolTipOvertime(IScheduleDay cell)
		{
			...
			foreach (IVisualLayer layer in proj)
			{
				...
				sb.Append(": ");
				sb.Append(ToLocalStartEndTimeString(layer.Period, TimeZoneGuardForDesktop.Instance.CurrentTimeZone()));
			}
			return sb.ToString();
		}
		...
		public static IList<string> GetInfoTextWeekView(IScheduleDay schedulePart, SchedulePartView significantPart)
		{
			...
			if (significantPart == SchedulePartView.MainShift)
			{
				...
				if (period.HasValue)
				{
					periodText = ToLocalStartEndTimeString(period.Value, TimeZoneGuardForDesktop.Instance.CurrentTimeZone());
				}
			}
