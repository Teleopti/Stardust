...
#pragma warning restore 618
		...
		private void backgroundWorkerResourceCalculatorDoWork(object sender, DoWorkEventArgs e)
		{
			...
			using (_container.Resolve<ISharedResourceContextOldSchedulingScreenBehavior>().MakeSureExists(new DateOnlyPeriod(_schedulerState.DaysToRecalculate.Min(), _schedulerState.DaysToRecalculate.Max())))
			{
				...
				if (scheduleDays.Any())
				{
					...
					var validationResult = _container.Resolve<CheckScheduleHints>()
						.Execute(new HintInput(_schedulerState.Schedules, _scheduleView.AllSelectedPersons(_scheduleView.SelectedSchedules()), 
							selectedPeriod, new FixedBlockPreferenceProvider(_schedulingOptions)));
		...
		private void loadAndOptimizeData(DoWorkEventArgs e)
		{
			...
			if (!_schedulerState.SchedulingResultState.SkipResourceCalculation && !_teamLeaderMode)
			{
				...
				using (_container.Resolve<ISharedResourceContextOldSchedulingScreenBehavior>().MakeSureExists(requestPeriod))
				{
