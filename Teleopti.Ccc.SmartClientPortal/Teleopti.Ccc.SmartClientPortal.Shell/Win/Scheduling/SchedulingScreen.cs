...
namespace Teleopti.Ccc.SmartClientPortal.Shell.Win.Scheduling
{
	...
	public partial class SchedulingScreen : BaseRibbonForm
	{
		...
		private readonly bool _loadRequests;
...
#pragma warning restore 618
		...
		private void invalidateAfterMeetingChange(ModifyMeetingEventArgs e)
		{
			...
			{
				var period = c.Period.ToDateOnlyPeriod(TimeZoneGuardForDesktop.Instance_DONTUSE.CurrentTimeZone());
		...
		private void schedulingScreenLoad(object sender, EventArgs e)
		{
			...
			var publishSchedule = authorization.IsPermitted(DefinedRaptorApplicationFunctionPaths.PublishSchedule);
			toolStripMenuItemPublish.Visible = publishSchedule;
			...
			//No code after the call to RunWorkerAsync
		}
		...
		{
			if (sender is ToolStripButton button && button.Tag != null) zoom((ZoomLevel)button.Tag);
			else
			{
				if (sender is QuickButtonReflectable quickButton && quickButton.ReflectedButton.Tag != null)
		...
		{
			var backgroundWorkersToKill = new[]
			{
			...
			for (var i = 0; i < backgroundWorkersToKill.Length; i++)
			{
				scheduleStatusBarUpdate($"{LanguageResourceHelper.Translate("XXCancellingThreeDots")} ({i + 1}/{backgroundWorkersToKill.Length})");
				cancelBackgroundWorker(backgroundWorkersToKill[i]);
			}
		...
		private void addDayOff()
		{
			if (_scheduleView != null)
			{
				if (SchedulerState.SchedulerStateHolder.FilteredCombinedAgentsDictionary.Count > 0)
				{
					...
					var period =
						new DateTimePeriod(
							DateTime.SpecifyKind(TimeZoneHelper.ConvertFromUtc(first, TimeZoneGuardForDesktop.Instance_DONTUSE.CurrentTimeZone()), DateTimeKind.Utc),
							DateTime.SpecifyKind(TimeZoneHelper.ConvertFromUtc(last.AddDays(1), TimeZoneGuardForDesktop.Instance_DONTUSE.CurrentTimeZone()),
								DateTimeKind.Utc));
					var addDayOffDialog = _scheduleView.CreateAddDayOffViewModel(displayList, TimeZoneGuardForDesktop.Instance_DONTUSE.CurrentTimeZone(), period);
		...
		public void RecalculateResources()
		{
			...
			if (toolStripProgressBar1.CustomLabelControl == null)
		...
		private void backgroundWorkerResourceCalculatorProgressChanged(object sender, ProgressChangedEventArgs e)
		{
			...
			if (e.UserState is ResourceOptimizerProgressEventArgs progress)
			{
		...
		private void afterBackgroundWorkersCompleted(bool canceled)
		{
			...
			if (_schedulingOptions.ShowTroubleshot && _schedulingOptions.ScheduleEmploymentType == ScheduleEmploymentType.FixedStaff)
			{
				...
				if (scheduleDays.Any())
				{
					...
					var specificTimeZone = new SpecificTimeZone(TimeZoneGuardForDesktop.Instance_DONTUSE.CurrentTimeZone());
		...
		private void backgroundWorkerLoadDataRunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
		{
			...
			if (PrincipalAuthorization.Current_DONTUSE().IsPermitted(DefinedRaptorApplicationFunctionPaths.RequestScheduler) && _loadRequests)
			{
		...
		{
			// this is a workaround because if we press cancel before loadPeople has finished the SchedulingScreen will not unload
			...
			if (_scheduleView != null)
			{
				...
				if (_scheduleView.ViewGrid[rowIndex, colIndex].CellValue is IScheduleDay dest)
				{
					IMeeting meeting = _schedulerMeetingHelper.MeetingFromList(dest.Person,
						dest.Period.StartDateTimeLocal(TimeZoneGuardForDesktop.Instance_DONTUSE.CurrentTimeZone()), dest.PersonMeetingCollection());
					if (meeting != null)
					{
		...
		private void skillGridControlGotFocus(object sender, EventArgs e)
		{
		...
		{
			//This one is for use of reOptimize, reOptimize days off and move shifts. We will probably need something else for get back to legal state
			if (e.UserState is ResourceOptimizerProgressEventArgs progress)
			{
				if (_cancelButtonPressed)
				{
		...
		private void backgroundWorkerOvertimeSchedulingDoWork(object sender, DoWorkEventArgs e)
		{
			...
			_container.Resolve<ScheduleOvertime>()
				.Execute(argument.OvertimePreferences, new BackgroundWorkerWrapper(_backgroundWorkerOvertimeScheduling),
								LockManager.UnlockedDays(scheduleDays), TimeZoneGuardForDesktop.Instance_DONTUSE.CurrentTimeZone());
		...
		private void backgroundWorkerOptimizationDoWork(object sender, DoWorkEventArgs e)
		{
			...
			AdvanceLoggingService.LogOptimizationInfo(optimizerPreferences, scheduleDays.Select(x => x.Person).Distinct().Count(),
				dateOnlyList.Count, () => runBackgroundWorkerOptimization(e));
			_undoRedo.CommitBatch();
		}

		private void runBackgroundWorkerOptimization(DoWorkEventArgs e)
		{
			var argument = (SchedulingAndOptimizeArgument)e.Argument;
		...
		private void setEventHandlers()
		{
			...
			schedulerSplitters1.DayGridControl.GotFocus += skillGridControlGotFocus;
			schedulerSplitters1.IntraDayGridControl.GotFocus += skillGridControlGotFocus;
			schedulerSplitters1.WeekGridControl.GotFocus += skillGridControlGotFocus;
			schedulerSplitters1.MonthGridControl.GotFocus += skillGridControlGotFocus;
			schedulerSplitters1.FullPeriodGridControl.GotFocus += skillGridControlGotFocus;
		...
		private void setEventHandlersOff()
		{
			...
			if (flowLayoutExportToScenario?.ContainerControl != null)
			{
				foreach (var control in flowLayoutExportToScenario.ContainerControl.Controls)
				{
					if (control is ButtonAdv button)
						button.Click -= menuItemClick;
				}
			}
			...
			if (_schedulerMessageBrokerHandler != null)
			{
				...
				_schedulerMessageBrokerHandler = null;
			}
			_requestPresenter = null;
		...
		// because called on another thread sometimes
		...
		{
			TimeZoneGuardForDesktop.Instance_DONTUSE.Set(TeleoptiPrincipalLocator_DONTUSE_REALLYDONTUSE.CurrentPrincipal.Regional.TimeZone);
			wpfShiftEditor1.SetTimeZone(TimeZoneGuardForDesktop.Instance_DONTUSE.CurrentTimeZone());
			...
			IEnumerable<PersistConflict> conflicts)
		{
			...
			{
				SchedulerState.SchedulerStateHolder.MarkDateToBeRecalculated(new DateOnly(data.Period.StartDateTimeLocal(TimeZoneGuardForDesktop.Instance_DONTUSE.CurrentTimeZone())));
			...
			ICollection<PersistConflict> conflictsBuffer)
		{
			...
			{
				_schedulerMessageBrokerHandler.Refresh(refreshedEntitiesBuffer, conflictsBuffer, _loadRequests);
			}
		}
		...
		private void toggleShrinkage(bool useShrinkage)
		{
			...
			foreach (ISkillStaffPeriod period in skillStaffPeriods)
			{
				...
				SchedulerState.SchedulerStateHolder.MarkDateToBeRecalculated(new DateOnly(period.Period.StartDateTimeLocal(TimeZoneGuardForDesktop.Instance_DONTUSE.CurrentTimeZone())));
			}
		...
		{
			TimeZoneGuardForDesktop.Instance_DONTUSE.Set(timeZone);
			wpfShiftEditor1.SetTimeZone(TimeZoneGuardForDesktop.Instance_DONTUSE.CurrentTimeZone());
			var selectedSchedules = _scheduleView.SelectedSchedules();
		...
		private void displayTimeZoneInfo()
		{
			...
			toolStripSplitButtonTimeZone.Text = string.Concat(LanguageResourceHelper.Translate("XXViewPointTimeZone"),
				Resources.Colon,
				TimeZoneGuardForDesktop.Instance_DONTUSE.CurrentTimeZone().DisplayName);
		}
		...
		private void replaceActivity()
		{
			...
			using (var view = new ReplaceActivityView(SchedulerState.SchedulerStateHolder.CommonStateHolder.Activities.ToList(), defaultPeriod.TimePeriod(TimeZoneGuardForDesktop.Instance_DONTUSE.CurrentTimeZone()), _replaceActivityParameters))
			{
				...
				replaceActivityService.Replace(schedules, activity, replaceWithActivity, timePeriod, TimeZoneGuardForDesktop.Instance_DONTUSE.CurrentTimeZone());
